...
const DUNGEON_UNIQUE_LOOT={trainingCellar:'ravenClaw',oldCrypt:'cryptWardenSignet',forgottenTower:'towerEye',beastLair:'wyvernScaleCharm',sunkenChapel:'drownedBell',witchBarrow:'witchKnot',blackrootKeep:'blackrootHeart',emberMine:'slagCoreCharm',ashenCitadel:'cinderMatriarchSeal',frostVault:'frostboundSignet',tempestSpire:'tempestEye'};
function dungeonUniqueItem(dungeonId){const id=DUNGEON_UNIQUE_LOOT[dungeonId];return id?itemDef(id):null}
function dungeonUniqueDropChance(dungeonId){return (state?.player?.dungeonClears?.[dungeonId]||0)===0?1:.24}
function dungeonBossDifficulty(d){if(!d)return {hp:1.55,atk:1.22,label:'Trudny'};if(d.id==='trainingCellar')return {hp:1.22,atk:1.08,label:'Wprowadzenie'};if(d.min>=45)return {hp:1.85,atk:1.32,label:'Ekstremalny'};if(d.min>=30)return {hp:1.72,atk:1.28,label:'Bardzo trudny'};return {hp:1.60,atk:1.24,label:'Trudny'}
function rollDungeonUniqueLoot(dungeonId){const id=DUNGEON_UNIQUE_LOOT[dungeonId];if(!id)return null;return Math.random()<dungeonUniqueDropChance(dungeonId)?{id,qty:1,gear:true,unique:true}:null}
...
function monsterLootTable(id){return MONSTER_LOOT[id]||[]}
function monsterLootHTML(m){const rows=monsterLootTable(m.id);if(!rows.length)return '<div class="loot-empty">Brak zarejestrowanych łupów.</div>';return `<div class="bestiary-loot-grid">${rows.map(r=>{const d=itemDef(r.id);return `<div class="bestiary-loot-row"><span class="loot-icon">${d.icon}</span><div><b class="rarity-${d.rarity}">${d.name}</b><small>${lootChanceLabel(r.chance)} • ${r.min||1}${(r.max||r.min||1)!==(r.min||1)?`–${r.max}`:''} szt.</small></div></div>`}).join('')}</div>`}
function rollMonsterMaterials(m,e,c){const drops=[];const elite=!!e?.elite,boss=!!c?.isBoss||!!c?.worldBoss;for(const row of monsterLootTable(m.id)){const chance=Math.min(1,row.chance*(elite?1.12:1)*(boss?1.18:1));if(Math.random()>chance)continue;let qty=rnd(row.min||1,row.max||row.min||1);if(elite&&Math.random()<.30)qty++;if(boss&&Math.random()<.45)qty++;drops.push({id:row.id,qty})}return drops}
function mergeLootDrops(drops=[]){const merged=[];const stackIndex=new Map();for(const drop of drops.filter(Boolean)){const qty=Math.max(1,drop.qty||1);if(isStackable(drop.id)){if(stackIndex.has(drop.id)){merged[stackIndex.get(drop.id)].qty+=qty;if(drop.unique)merged[stackIndex.get(drop.id)].unique=true}else{stackIndex.set(drop.id,merged.length);merged.push({id:drop.id,qty,gear:false,unique:!!drop.unique})}}else{for(let i=0;i<qty;i++)merged.push({id:drop.id,qty:1,gear:true,unique:!!drop.unique})}}return merged}
...
function critChance(){const p=state.player,pet=p.petActive?petDef(p.petActive):null,classBonus=p.class==='hunter'?5:p.class==='ranger'?2:0;return Math.min(70,5+p.stats.agi*.55+gearStat('crit')+(pet?.crit||0)+classBonus)}
function dodgeChance(){const p=state.player,classBonus=p.class==='ranger'?7:p.class==='hunter'?3:p.class==='mage'?1:0;return Math.min(38,2+p.stats.agi*.42+classBonus+(combat?.playerDodgeBuff||0))}
...
function rewardQuest(q){removeQuestEntities(q.id);gainXp(q.xp);state.player.gold+=q.gold;if(!state.quests.done.includes(q.id))state.quests.done.push(q.id);state.quests.active=state.quests.active.filter(id=>id!==q.id);if(q.id==='q13'&&!countItem('blackMedallion'))addItem('blackMedallion');if(q.id==='q25'){addItem(state.player.class==='mage'?'mistCirclet':['hunter','ranger'].includes(state.player.class)?'mistHood':'mistHelm');addItem('wraithEssence',2)}if(q.id==='q30'){addItem(state.player.class==='mage'?'mistRobe':['hunter','ranger'].includes(state.player.class)?'mistLeathers':'mistMail');addItem('bogAmber',2)}if(q.id==='q35'){addItem(state.player.class==='mage'?'mistStaff':['hunter','ranger'].includes(state.player.class)?'mistBow':'mistBlade');addItem('mireCharm');if(['hunter','ranger'].includes(state.player.class)&&!state.player.pets.some(p=>p.id==='mireLynx')){state.player.pets.push({id:'mireLynx',level:1,xp:0});toast('Nowy chowaniec: Ryś Mgieł!')}if(state.story?.flags?.endingDestroy)state.adventure.reputation+=2;if(state.story?.flags?.endingSeal)state.adventure.reputation+=4}if(q.id==='q38'){addItem(state.player.class==='mage'?'ashCowl':['hunter','ranger'].includes(state.player.class)?'ashHood':'ashHelm');addItem('charredIron',2)}if(q.id==='q45'){addItem(state.player.class==='mage'?'ashStaff':['hunter','ranger'].includes(state.player.class)?'ashBow':'ashBlade');addItem('cinderCharm');if(['hunter','ranger'].includes(state.player.class)&&!state.player.pets.some(p=>p.id==='cinderHound')){state.player.pets.push({id:'cinderHound',level:1,xp:0});toast('Nowy chowaniec: Ogar Popiołu!')}}if(q.id==='q48'){addItem(state.player.class==='mage'?'stormCowl':['hunter','ranger'].includes(state.player.class)?'stormHood':'stormHelm');addItem('frostCrystal',2)}if(q.id==='q55'){addItem(state.player.class==='mage'?'stormStaff':['hunter','ranger'].includes(state.player.class)?'stormBow':'stormSpear');addItem('tempestCharm');if(['hunter','ranger'].includes(state.player.class)&&!state.player.pets.some(p=>p.id==='stormHawk')){state.player.pets.push({id:'stormHawk',level:1,xp:0});toast('Nowy chowaniec: Jastrząb Burzy!')}}toast(`Quest ukończony: ${q.name} • +${q.xp} XP • +${q.gold} 🪙`);save()}
...
function updateLiveMapPosition(){
 ...
 if(!playerMapMarker){
  const html=`<div class="leaflet-player-marker rpg-player-marker">${classVisual(state.player.class,'mmo-player-sprite')}<span></span></div>`;
 ...
 }else playerMapMarker.setLatLng(ll);
...
function currentBiomeInfoHTML(){
 const bio=biomeInfoAtPlayer(),pool=biomeMonsterPool(bio.id).map(id=>MONSTERS.find(m=>m.id===id)).filter(Boolean);
 return `<div class="biome-info-modal"><div class="modal-head"><div><h2>${bio.icon} ${bio.name}</h2><div class="muted">Informacje o okolicy</div></div><button class="close" data-close>×</button></div><p>${bio.desc}</p><h3>Potwory, których możesz się spodziewać</h3><div class="biome-monster-list">${pool.map(m=>`<div class="biome-monster-chip"><span>${m.icon}</span><div><b>${m.name}</b><small>lvl ${m.min}–${m.max} • ${m.family}</small></div></div>`).join('')}</div><div class="muted biome-note">Rzadsze eventy i specjalne strefy mogą sprowadzić inne stworzenia.</div></div>`;
}
...
function toggleGps(){
 if(gpsWatch!==null){navigator.geolocation?.clearWatch(gpsWatch);gpsWatch=null;toast('GPS wyłączony.');if(currentTab==='map')selectNav('map');return}
 ...
 const handlePosition=pos=>{
  ...
  state.player.position={x:dx,y:dy,lat,lng,gps:true,accuracy,heading,virtualTravel:false,testWalk:false};
...
function itemCanGoToSlot(i,slot){
 ...
 if(slot==='offhand'&&state.player.class==='berserker'&&d.type==='weapon'&&['sword','axe','hammer'].includes(d.weaponKind))return true;
...
function renderInventory(el){
 ...
 const p=state.player,slots=[['helmet','Hełm','⛑️'],['amulet','Amulet','📿'],['weapon','Broń','⚔️'],['armor','Pancerz','🛡️'],['offhand','Druga ręka','🛡️'],['gloves','Rękawice','🧤'],['ring1','Pierścień I','💍'],['ring2','Pierścień II','💍'],['boots','Buty','🥾']],used=inventoryUsedSlots(),cap=inventoryCapacity();
 const cells=Array.from({length:cap},(_,idx)=>{const i=p.inventory[idx];if(!i)return `<button class="backpack-slot empty" disabled><span>·</span></button>`;const d=itemDef(i.id),qty=i.qty||1,eq=inventoryItemEquippedSlot(i),gear=!!d.slot;return `<button class="backpack-slot dnd-bag-item rarity-border-${d.rarity} ${eq?'is-equipped':''}" data-bag-index="${idx}" ${gear?'draggable="true"':''} data-dnd-gear="${gear?'1':'0'}" title="${itemName(i)}${gear?' • przeciągnij na slot':''}"><span class="backpack-icon">${itemIconVisual(d.id,'backpack-item-svg')}</span>${qty>1?`<b class="stack-badge">${qty}</b>`:''}${(i.upgrade||0)>0?`<em>+${i.upgrade}</em>`:''}${eq?`<i class="bag-eq-badge">${slotLabel(eq)}</i>`:''}</button>`}).join('');
 el.innerHTML=`<div class="section-title gear-page-title"><div><h2>🎒 Ekwipunek + plecak</h2><div class="muted">Sprzęt ma wymagany poziom i klasy. Berserker może trzymać drugą broń zamiast tarczy; Łowca i Tropiciel zużywają strzały.</div></div><span class="pill ${used>=cap?'danger-pill':''}">${used}/${cap}</span></div><div class="gear-bag-layout"><section class="paperdoll drag-paperdoll"><div class="paperdoll-title">WYPOSAŻENIE</div><div class="paperdoll-grid dnd-paperdoll-grid">${slots.map(([slot,label,ico])=>equipmentSlotHTML(slot,label,ico)).join('')}<div class="hero-silhouette drag-hero-silhouette"><div class="hero-pixel">${classVisual(p.class,'sprite-hero')}</div><b>${p.name}</b><span>${CLASSES[p.class].name}</span></div></div><div class="drag-help">Przeciągnij przedmiot na podświetlony slot</div></section><section class="backpack-frame drag-backpack"><div class="backpack-topline"><b>Plecak</b><span>${cap-used} wolnych</span></div><div class="backpack-grid">${cells}</div></section></div>${setStatusHTML()}`;
...
function renderTown(el){
 setAmbient('town');
 const cl=climate();
 const cardCopy={tavern:['Dorian','Odpoczynek • plotki • questy'],shop:['Selma','Handel • towary dnia'],smith:['Ragor','Sklep • enchanty • ulepszanie • rzemiosło'],alchemist:['Ilyra','Mikstury • receptury • składniki'],auction:['Varo','Oferty dnia • rzadkie przedmioty'],guild:['Edrin','Reputacja • kontrakty • gildia']};
...
function shopHTML(){const classGear={knight:['rustySword','shortSword','chainVest','woodenShield','ironArmor'],berserker:['rustySword','shortSword','ironAxe','chainVest','berserkerHarness'],mage:['noviceStaff','apprenticeStaff','apprenticeRobe','mageHood','mageShoes'],hunter:['hunterBow','yewBow','rangerLeather','scoutHood','trailBoots'],ranger:['hunterBow','yewBow','rangerLeather','scoutHood','trailBoots']}[state.player.class]||[];const ids=['potion','manaPotion','herb','scrap',...(['hunter','ranger'].includes(state.player.class)?['primitiveArrow']:[]),...classGear].filter((id,i,a)=>a.indexOf(id)===i).filter(id=>!itemDef(id).reqLevel||itemDef(id).reqLevel<=state.player.level+5);return `${npcCard('Selma','Kupcowa','hunter','Selma pokazuje sprzęt odpowiedni dla Twojej klasy. Najlepszy ekwipunek nadal zdobywa się w świecie i lochach.')}<div class="panel-list">${ids.map(id=>{const i=itemDef(id),price=merchantBuyPrice(id);return `<div class="panel-item shoprow"><div><b class="shop-item-name">${itemIconVisual(i.id,'shop-item-svg')} ${i.name}</b><div class="muted">${i.reqLevel?`lvl ${i.reqLevel} • `:''}${i.damage?`${i.damage[0]}–${i.damage[1]} obr. • `:''}${price} 🪙</div></div><button class="secondary" data-buy="${id}" data-price="${price}">Kup</button></div>`}).join('')}</div>`}
const SMITH_ROTATION_MS=12*60*60*1000;
const SMITH_STARTER_IDS=new Set(['tornRags','holeyBoots','shortBlade','willowStaff','primitiveBow']);
function smithRotationId(){return Math.floor(Date.now()/SMITH_ROTATION_MS)}
function smithTimeLeft(){const left=Math.max(0,(smithRotationId()+1)*SMITH_ROTATION_MS-Date.now()),h=Math.floor(left/3600000),m=Math.floor((left%3600000)/60000);return `${h} godz. ${m} min`}
function smithHash(text){let h=2166136261;for(const ch of String(text)){h^=ch.charCodeAt(0);h=Math.imul(h,16777619)}return h>>>0}
function smithOrder(ids,seed,level=state.player.level){const score=id=>Math.abs((itemDef(id).reqLevel||1)-level)*10+seeded(seed+smithHash(id)*.001)*22;return [...ids].sort((a,b)=>score(a)-score(b))}
function smithEligibleGear(level=state.player.level){
 const min=Math.max(1,level-7),max=level+5;
 const base=Object.values(ITEMS).filter(d=>d.slot&&d.value>0&&!SMITH_STARTER_IDS.has(d.id)&&itemClassAllowed(d)&&!['heroic','legendary'].includes(d.rarity));
 let near=base.filter(d=>(d.reqLevel||1)>=min&&(d.reqLevel||1)<=max);
 if(near.length<8){const ids=new Set(near.map(d=>d.id)),extras=base.filter(d=>!ids.has(d.id)&&(d.reqLevel||1)<=max).sort((a,b)=>Math.abs((a.reqLevel||1)-level)-Math.abs((b.reqLevel||1)-level));near=[...near,...extras.slice(0,8-near.length)]}
 return near.length?near:base;
}
function generateSmithStock(rotation=smithRotationId()){
 const level=state.player.level,seed=rotation*97+level*31+smithHash(state.player.class),pool=smithEligibleGear(level),chosen=[];
 const takeSlot=(slot)=>{const candidates=smithOrder(pool.filter(d=>d.slot===slot&&!chosen.includes(d.id)).map(d=>d.id),seed+chosen.length*19);if(candidates[0])chosen.push(candidates[0])};
 takeSlot('weapon');takeSlot('armor');
 const rest=smithOrder(pool.filter(d=>!chosen.includes(d.id)).map(d=>d.id),seed+311);
 for(const id of rest){if(chosen.length>=6)break;chosen.push(id)}
 return {rotation,classId:state.player.class,levelAtRoll:level,offers:chosen.slice(0,6).map((id,i)=>{const base=merchantBuyPrice(id),factor=.94+seeded(seed+i*53)*.14;return {id,price:Math.max(1,Math.ceil(base*factor)),bought:false}})};
}
function ensureSmithStock(){ensureEconomyState();const r=smithRotationId(),st=state.economy.smithShop;if(!st||st.rotation!==r||st.classId!==state.player.class||!Array.isArray(st.offers)){state.economy.smithShop=generateSmithStock(r);save()}return state.economy.smithShop}
function smithShopHTML(){const stock=ensureSmithStock(),level=state.player.level;return `<div class="smith-shop-head"><div><span class="eyebrow">DOSTAWA CO 12 GODZIN</span><h3>🛒 Sklep Ragora</h3><p class="muted">Sprzęt dobierany głównie do poziomu ${level} i klasy ${CLASSES[state.player.class].name}. Najrzadsze unikaty nadal zdobywa się w świecie i lochach.</p></div><div class="smith-refresh-box"><small>Nowa dostawa za</small><b data-smith-countdown>${smithTimeLeft()}</b></div></div><div class="smith-stock-grid">${stock.offers.map((o,idx)=>{const d=itemDef(o.id),locked=(d.reqLevel||1)>level;const stats=[d.damage?`${d.damage[0]}–${d.damage[1]} obr.`:'',d.armor?`Pancerz +${d.armor}`:'',d.power?`Moc +${d.power}`:'',d.crit?`Kryt +${d.crit}%`:''].filter(Boolean).join(' • ');return `<article class="smith-stock-card ${o.bought?'sold':''}"><div class="smith-stock-art">${itemIconVisual(d.id,'shop-item-svg')}</div><div class="smith-stock-copy"><span class="rarity-${d.rarity}">${rarityName(d.rarity)}</span><b>${d.name}</b><small>Wymagany lvl ${d.reqLevel||1}${stats?` • ${stats}`:''}</small></div><div class="smith-stock-buy"><strong>${o.price} 🪙</strong><button class="secondary" data-smith-buy="${idx}" ${o.bought||locked?'disabled':''}>${o.bought?'SPRZEDANE':locked?`Lvl ${d.reqLevel}`:'Kup'}</button></div></article>`}).join('')}</div>`}
function smithEnchantHTML(){const gear=state.player.inventory.filter(i=>itemDef(i.id).slot),runes=['runePower','runeGuard','runePrecision'];return `<div class="forge-summary"><span>🔹 Odłamki: <b>${countItem('runeShard')}</b></span><span>💎 Kryształy: <b>${countItem('crystal')}</b></span>${runes.map(id=>`<span>${itemDef(id).icon} ${countItem(id)}</span>`).join('')}</div><h3>🔮 Enchanty</h3><p class="muted">Zaklinanie jest osobną usługą. Wybierz przedmiot, aby nadać mu bonus lub przerzucić istniejący enchant.</p><div class="panel-list">${gear.map(i=>{const d=itemDef(i.id),q=enchantQuote(i,!!i.enchant);return `<div class="panel-item smith-item"><div><b class="shop-item-name">${itemIconVisual(d.id,'shop-item-svg')} ${itemName(i)}</b><div class="muted">${rarityName(d.rarity)} • ${i.enchant?`🔮 ${i.enchant.name}`:'bez enchantu'}</div></div><div class="smith-actions"><button class="secondary" data-enchant="${i.uid}">${i.enchant?`Przerzuć • ${q.gold}🪙 + ${q.crystal}💎${q.shard?` + ${q.shard}🔹`:''}`:`Enchant • ${q.gold}🪙 + ${q.crystal}💎`}</button></div></div>`}).join('')||'<div class="panel-item muted">Nie masz sprzętu do zaklinania.</div>'}</div>`}
function smithUpgradeHTML(){const gear=state.player.inventory.filter(i=>itemDef(i.id).slot),runes=['runePower','runeGuard','runePrecision'];return `<div class="forge-summary"><span>🔩 Złom: <b>${countItem('scrap')}</b></span><span>🔹 Odłamki: <b>${countItem('runeShard')}</b></span>${runes.map(id=>`<span>${itemDef(id).icon} ${countItem(id)}</span>`).join('')}</div><h3>⚒️ Ulepszanie i runy</h3><div class="panel-list">${gear.map(i=>{const d=itemDef(i.id),up=i.upgrade||0,q=upgradeQuote(i);return `<div class="panel-item smith-item"><div><b class="shop-item-name">${itemIconVisual(d.id,'shop-item-svg')} ${itemName(i)}</b><div class="muted">${rarityName(d.rarity)} • +${up} • ${i.rune?`${itemDef(i.rune).icon} ${itemDef(i.rune).name}`:'wolne gniazdo runy'}</div></div><div class="smith-actions"><button class="secondary" data-upgrade="${i.uid}" ${up>=5?'disabled':''}>${up>=5?'MAX':`Ulepsz ${q.gold}🪙${q.scrap?` + ${q.scrap}🔩`:''}`}</button>${!i.rune?runes.map(r=>`<button class="ghost" data-socket="${i.uid}" data-rune="${r}" ${countItem(r)?'':'disabled'}>${itemDef(r).icon} Osadź</button>`).join(''):`<button class="ghost" data-unsocket="${i.uid}">↩️ Wyjmij runę</button>`}</div></div>`}).join('')}</div>`}
function smithCraftHTML(){const recipes=RECIPES.filter(r=>r.station==='smith'&&itemClassAllowed(itemDef(r.result)));return `<h3>🧰 Rzemiosło</h3><p class="muted">Tutaj Ragor wykuwa przedmioty z materiałów zdobytych podczas walk i eksploracji.</p><div class="panel-list">${recipes.map(r=>{const fee=craftFee(r);return `<div class="panel-item shoprow"><div><b class="shop-item-name">${itemIconVisual(r.result,'shop-item-svg')} ${r.name}</b><div class="muted">${Object.entries(r.ingredients).map(([id,q])=>`${q}× ${itemDef(id).icon} ${itemDef(id).name}`).join(' + ')} • ${fee} 🪙</div></div><button class="secondary" data-craft="${r.id}">Wykuj</button></div>`}).join('')}</div>`}
function smithHTML(){state.ui ||= {};state.ui.smithView ||= 'shop';const view=state.ui.smithView,content=view==='enchant'?smithEnchantHTML():view==='upgrade'?smithUpgradeHTML():view==='craft'?smithCraftHTML():smithShopHTML();return `${npcCard('Ragor','Kowal i runmistrz','berserker','Kupuj stal, ulepszaj ekwipunek albo zaklinaj go — każda usługa ma teraz własne stanowisko.')}<nav class="smith-service-tabs"><button class="${view==='shop'?'active':''}" data-smith-view="shop">🛒 Sklep</button><button class="${view==='enchant'?'active':''}" data-smith-view="enchant">🔮 Enchanty</button><button class="${view==='upgrade'?'active':''}" data-smith-view="upgrade">⚒️ Ulepszanie</button><button class="${view==='craft'?'active':''}" data-smith-view="craft">🧰 Rzemiosło</button></nav><div class="smith-service-body">${content}</div>`}
...
function bindBuilding(id,view){
 ...
 document.querySelectorAll('[data-smith-view]').forEach(b=>b.onclick=()=>{state.ui.smithView=b.dataset.smithView;save();openBuilding('smith','service')});
 document.querySelectorAll('[data-smith-buy]').forEach(b=>b.onclick=()=>buySmithOffer(Number(b.dataset.smithBuy)));
...
function buySmithOffer(index){const stock=ensureSmithStock(),offer=stock.offers[index];if(!offer||offer.bought)return toast('Ta sztuka została już sprzedana.');const d=itemDef(offer.id);if((d.reqLevel||1)>state.player.level)return toast(`Potrzebujesz poziomu ${d.reqLevel}.`);if(state.player.gold<offer.price)return toast('Za mało złota.');if(!inventoryHasRoom())return toast('🎒 Plecak jest pełny.');state.player.gold-=offer.price;offer.bought=true;addItem(offer.id);save();openBuilding('smith','service');toast(`Ragor sprzedał Ci: ${d.name} • -${offer.price} 🪙`)}
...
function exportSave(){const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='time4heroes-build-3.0.2-save.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
...
function startCombat(entity,opts={}){
 const m=monsterTemplate(entity),lvl=opts.level||monsterLevel(m),scale=.66+lvl*.075,isDungeonBoss=!!opts.dungeon?.boss,isWorldBoss=!!opts.worldBoss,isBoss=isWorldBoss||isDungeonBoss||!!entity.elite,dungeonDef=isDungeonBoss?DUNGEONS.find(x=>x.id===opts.dungeon?.dungeonId):null,dungeonDiff=isDungeonBoss?dungeonBossDifficulty(dungeonDef):null,bossHpMult=isDungeonBoss?dungeonDiff.hp:isWorldBoss?1.58:entity.elite?1.12:1,bossAtkMult=isDungeonBoss?dungeonDiff.atk:isWorldBoss?1.25:entity.elite?1.08:1,maxHp=Math.max(24,Math.floor(m.hp*scale*bossHpMult)),atk=Math.max(4,Math.floor(m.atk*(.62+lvl*.045)*bossAtkMult));
 combat={entity,monster:m,level:lvl,maxHp,hp:maxHp,atk,baseAtk:atk,log:[`${m.name} staje do walki.`],guard:0,debuff:0,debuffTurns:0,poison:0,poisonTurns:0,bleed:0,bleedTurns:0,burn:0,burnTurns:0,freezeTurns:0,mark:0,markTurns:0,dungeon:opts.dungeon||null,worldBoss:isWorldBoss,isBoss,bossPhase:1,enemyTurns:0,intent:null,stagger:0,staggerMax:isBoss?100:0,stunned:0,vulnerableTurns:0,barrier:0,barrierTurns:0,playerWeaken:0,playerWeakenTurns:0,playerCritBuff:0,playerCritBuffTurns:0,playerDodgeBuff:0,playerDodgeBuffTurns:0,playerPowerBuff:0,playerPowerBuffTurns:0,playerBlockBuff:0,playerBlockBuffTurns:0,cooldowns:{},lastPlayerHit:null,lastEnemyHit:null};
 if(isDungeonBoss)combat.log.push(`👑 Boss lochu • ${dungeonDiff.label}. Przygotuj się na długą walkę.`);
 openCombat();
}
function openCombat(){
...
function resolveBossIntent(){
 ...
 if(intent.type==='smash'){
  combat.fx='boss';const reduction=armorPower()*.28,guardMult=combat.guard>0?.28:1,blocked=Math.random()*100<blockChance(),blockMult=blocked?.55:1,dmg=Math.max(1,Math.floor((combat.atk*2.25-reduction)*guardMult*blockMult));state.player.hp=Math.max(0,state.player.hp-dmg);combat.lastEnemyHit=dmg;logCombat(`${combat.guard>0||blocked?'🛡️ Ograniczasz cios':'💥 Trafia Cię'}: ${info.name} za ${dmg}.`);if(combat.guard>0)combat.guard--;
  ...
  let total=0;for(let i=0;i<2;i++){const blocked=Math.random()*100<blockChance(),d=Math.max(1,Math.floor((combat.atk*1.05-armorPower()*.32)*(blocked?.55:1)));total+=d}state.player.hp=Math.max(0,state.player.hp-total);combat.lastEnemyHit=total;logCombat(`⚔️ ${info.name}: dwa uderzenia zadają łącznie ${total}.`);
 }
 return true;
}
...
function enemyTurn(){
 ...
 if(combat.poisonTurns>0){combat.hp-=combat.poison;logCombat(`☠️ Trucizna zadaje ${combat.poison}.`)}
 if(combat.bleedTurns>0){combat.hp-=combat.bleed;logCombat(`🩸 Krwawienie zadaje ${combat.bleed}.`)}
 if(combat.burnTurns>0){combat.hp-=combat.burn;logCombat(`🔥 Podpalenie zadaje ${combat.burn}.`)}
 if(combat.hp<=0)return winCombat();
 ...
 if(combat.freezeTurns>0&&Math.random()<.25){logCombat('❄️ Zamrożenie spowalnia przeciwnika — traci akcję.');tickEnemyStatuses();save();openCombat();return}
 if(combat.intent){resolveBossIntent();tickEnemyStatuses();if(state.player.hp<=0)return loseCombat();save();openCombat();return}
 combat.enemyTurns++;
 const intentEvery=combat.bossPhase>=3?2:3;if(combat.isBoss&&combat.enemyTurns%intentEvery===0){combat.intent=chooseBossIntent();const info=bossIntentInfo(combat.intent);logCombat(`⚠️ Boss przygotowuje: ${info.name}. ${info.hint}`);playSfx('boss');haptic([30,35,30]);tickEnemyStatuses();save();openCombat();return}
 const dodge=Math.random()*100<dodgeChance();if(dodge){combat.lastEnemyHit=null;logCombat('💨 Unikasz ataku przeciwnika.');tickEnemyStatuses();save();openCombat();return}
 combat.fx='enemy';const blocked=Math.random()*100<blockChance(),reduction=armorPower()*.42,guardMult=combat.guard>0?.55:1,freezeMult=combat.freezeTurns>0?.78:1,blockMult=blocked?.52:1,mult=(1-combat.debuff)*guardMult*freezeMult*blockMult,dmg=Math.max(1,Math.floor((combat.atk*(.85+Math.random()*.3)-reduction)*mult));state.player.hp=Math.max(0,state.player.hp-dmg);combat.lastEnemyHit=dmg;logCombat(`${blocked?'🛡️ Blok! ':''}${combat.monster.name} zadaje Ci ${dmg}.`);if(combat.guard>0)combat.guard--;tickEnemyStatuses();if(state.player.hp<=0)return loseCombat();save();openCombat();
}
...
function battleLootRow(drop,index){const d=itemDef(drop.id);return `<label class="battle-loot-row rarity-card-${d.rarity} ${drop.unique?'unique-boss-drop':''}"><input type="checkbox" data-battle-loot="${index}" ${drop.unique?'checked':''}><span class="battle-loot-icon">${d.icon}</span><span class="battle-loot-copy"><b class="rarity-${d.rarity}">${d.name}</b><small>${drop.unique?'👑 UNIKAT BOSSA • ':''}${drop.qty>1?`${drop.qty} szt. • `:''}${d.rarity} • ${d.type==='material'?'materiał':d.type==='consumable'?'zużywalny':d.slot?'ekwipunek':'przedmiot'}</small></span><span class="battle-loot-take">ZABIERZ</span></label>`}
...
function openDungeonLobby(d){
 ...
 const boss=MONSTERS.find(m=>m.id===d.boss),unique=dungeonUniqueItem(d.id),uniqueChance=dungeonUniqueDropChance(d.id),diff=dungeonBossDifficulty(d);
 openModal(`<div class="dungeon-lobby dungeon-theme-${d.id}"><div class="modal-head"><div><span class="eyebrow">ODKRYTY LOCH</span><h2>${d.icon} ${d.name}</h2><div class="muted">Dostępny z dowolnego miejsca</div></div><button class="close" data-close>×</button></div><div class="dungeon-lobby-hero"><div class="dungeon-lobby-icon">${d.icon}</div><div><p>${d.desc}</p><div class="dungeon-rule-pills"><span>⏱️ ${Math.round(dungeonTimeLimit(d)/60)} min</span><span>🎟️ ${a.attempts}/${DUNGEON_DAILY_LIMIT} dziś</span><span>👑 ${diff.label}</span><span>🔒 po sukcesie 1 h</span><span>💀 porażka 15 min</span></div></div></div>${boss?`<div class="dungeon-lobby-boss"><span>${monsterVisual(boss.id,'dungeon-lobby-boss-sprite')}</span><div><small>BOSS LOCHU</small><b>${boss.name}</b><em>HP ×${diff.hp.toFixed(2)} • ATK ×${diff.atk.toFixed(2)}</em></div></div>`:''}${unique?`<div class="dungeon-unique-loot"><div class="unique-loot-icon">${unique.icon}</div><div><small>UNIKAT BOSSA</small><b class="rarity-${unique.rarity}">${unique.name}</b><span>${uniqueChance===1?'Pierwsze zwycięstwo: GWARANTOWANY drop':'Kolejne zwycięstwa: 24% szansy na drop'}</span></div></div>`:''}<div class="dungeon-records"><span><b>${state.player.dungeonClears[d.id]||0}</b> ukończeń</span><span><b>${a.bestTime?formatClock(a.bestTime):'—'}</b> rekord czasu</span><span><b>${a.bestExplore||0}%</b> rekord eksploracji</span></div><div class="dungeon-lobby-status">${!levelOk?`🔒 Wymagany poziom ${d.min}.`:cool?`🔒 Pieczęć odnowi się za <b>${formatCooldown(a.cooldownUntil)}</b>.`:left<=0?'🎟️ Wykorzystano wszystkie 3 wejścia na dziś.':`🟢 Gotowy • pozostało wejść: <b>${left}/${DUNGEON_DAILY_LIMIT}</b>`}</div><button class="primary dungeon-action" data-start-dungeon-run ${levelOk&&!cool&&left>0?'':'disabled'}>Rozpocznij ekspedycję</button></div>`);
...
function dungeonMapHTML(){
 const cells=[];for(let y=0;y<dungeonRun.size;y++)for(let x=0;x<dungeonRun.size;x++){const c=dungeonRun.grid[y][x],current=x===dungeonRun.x&&y===dungeonRun.y;cells.push(`<div class="dungeon-map-cell ${c.wall?'wall':'floor'} type-${c.type} ${c.revealed?'revealed':''} ${c.visited?'visited':''} ${current?'current':''} ${c.type==='boss'?'boss-cell':''}">${dungeonCellIcon(c,x,y)}</div>`)}return `<div class="crawler-map" style="--dungeon-size:${dungeonRun.size}">${cells.join('')}</div>`
}
