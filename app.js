import {CLASSES,MONSTERS,MONSTER_LOOT,ITEMS,QUESTS,SKILLS,PETS,RECIPES,DUNGEONS,BUILDINGS} from './data.js?v=3810';

const SAVE_KEY='time4heroes_build_381';
...
const CITY_RADIUS=200,CITY_VISIT_MS=4*60*60*1000,CITY_RELOCATE_MS=30*24*60*60*1000;
function phoneMode(){return window.matchMedia?.('(max-width: 720px), (display-mode: standalone)')?.matches||false}
function ensureCityState(s=state){if(!s)return;s.city ||= {center:null,placedAt:0,lastVisitAt:0,demoPassUntil:0,levels:{}};s.city.levels ||= {};for(const b of BUILDINGS)s.city.levels[b.id] ||= 1}
function liveCityFix(){const p=state?.player?.position;return !!p?.gps&&Number.isFinite(p.lat)&&Number.isFinite(p.lng)&&p.lat!==null&&p.lng!==null&&p.accuracy<=100&&Date.now()-(p.gpsFixAt||0)<120000}
function cityDistance(){return state?.city?.center?geoDistance(state.player.position,state.city.center):Infinity}
function insideCity(){return liveCityFix()&&cityDistance()<=CITY_RADIUS+Math.min(25,state.player.position.accuracy||0)}
function cityAccess(){ensureCityState();if(phoneMode())return insideCity();return Date.now()-state.city.lastVisitAt<CITY_VISIT_MS||state.settings.demo&&Date.now()<state.city.demoPassUntil}
function cityStatus(){ensureCityState();if(!state.city.center){if(!phoneMode()&&cityAccess())return 'Dostęp próbny • centrum miasta jeszcze nieustawione';return 'Najpierw wyznacz strefę miasta telefonem przy aktywnym GPS.';}if(insideCity())return 'Jesteś w strefie miasta • usługi dostępne';if(!phoneMode()&&cityAccess())return state.city.demoPassUntil>Date.now()?'Dostęp próbny na tym urządzeniu':'Wizyta potwierdzona • usługi dostępne';return !liveCityFix()&&phoneMode()?'Włącz dokładny GPS, aby sprawdzić wejście do miasta.':`Poza strefą miasta • ${Number.isFinite(cityDistance())?Math.round(cityDistance())+' m od centrum':'włącz GPS'}`}
function placeCity(){ensureCityState();if(!phoneMode())return toast('Centrum miasta ustawisz z telefonu.');if(!liveCityFix())return toast('Aby ustawić miasto, włącz GPS na telefonie (dokładność do 100 m).');if(state.city.center&&Date.now()-state.city.placedAt<CITY_RELOCATE_MS)return toast('Lokalizację miasta można zmienić raz na 30 dni.');const p=state.player.position;state.city.center={lat:p.lat,lng:p.lng};state.city.placedAt=Date.now();state.city.lastVisitAt=Date.now();save();selectNav('town');toast('Strefa miasta wyznaczona. Promień: 200 m.')}
const CITY_UPGRADES={tavern:'Tańsze posiłki (−5%/poziom)',shop:'Tańsze zakupy (−5%/poziom)',smith:'Tańsze ulepszanie (−8%/poziom)',alchemist:'Tańsze warzenie (−8%/poziom)',auction:'Tańsze oferty (−5%/poziom)',guild:'Więcej reputacji za kontrakty (+1/poziom)'};
function cityUpgradeQuote(id){const lvl=state.city.levels[id]||1;return {gold:65*lvl*lvl,scrap:id==='alchemist'?0:2*lvl,herb:id==='alchemist'?3*lvl:0}}
function upgradeCityBuilding(id){ensureCityState();if(phoneMode())return toast('Rozbudowa jest dostępna w przeglądarce na komputerze.');if(!cityAccess())return toast('Rozbudowa wymaga ostatniej wizyty w mieście.');if(!BUILDINGS.some(b=>b.id===id)||!buildingUnlock(id).ok)return;const lvl=state.city.levels[id]||1;if(lvl>=3)return toast('Budynek ma już maksymalny poziom.');const q=cityUpgradeQuote(id);if(state.player.gold<q.gold||countItem('scrap')<q.scrap||countItem('herb')<q.herb)return toast('Brakuje złota lub surowców na rozbudowę.');state.player.gold-=q.gold;if(q.scrap)removeItem('scrap',q.scrap);if(q.herb)removeItem('herb',q.herb);state.city.levels[id]=lvl+1;save();selectNav('town');toast(`Rozbudowano: ${BUILDINGS.find(b=>b.id===id).name}, poziom ${lvl+1}.`)}
...
const AFFIXES=[{name:'Ognia',power:3},{name:'Żelaza',armor:3},{name:'Sokoła',crit:3},{name:'Łowcy',power:2,crit:2},{name:'Siłacza',str:2,vit:1},{name:'Zwinności',agi:3},{name:'Mędrca',int:3},{name:'Hartowności',vit:3},{name:'Bohatera',str:2,agi:3}];
...
function createGearInstance(id,affix=undefined){const d=itemDef(id),inst={id,uid:uid(),upgrade:0,rune:null,enchant:null,enchantRolls:0,affix:null};if(d.slot)inst.affix=affix===undefined?(Math.random()<affixChance(d.rarity)?{...pick(AFFIXES)}:null):(affix?{...affix}:null);return inst}
function merchantBuyPrice(id){const d=itemDef(id),typeMult=d.type==='material'?1.18:d.type==='consumable'?1.10:ECONOMY.shopMarkup,discount=1-Math.min(2,(state?.city?.levels?.shop||1)-1)*.05;return Math.max(1,Math.ceil(d.value*typeMult*discount))}
...
function upgradeQuote(inst){const d=itemDef(inst.id),up=inst.upgrade||0,rank=rarityRank(d.rarity),discount=1-Math.min(2,(state?.city?.levels?.smith||1)-1)*.08;return {gold:Math.ceil((16+d.value*.10+rank*8)*Math.pow(up+1,1.28)*discount),scrap:up===0?0:Math.max(1,Math.ceil((up+rank)/2))}}
...
function craftFee(recipe){const d=itemDef(recipe.result),rank=rarityRank(d.rarity),discount=recipe.station==='alchemist'?1-Math.min(2,(state?.city?.levels?.alchemist||1)-1)*.08:1;return Math.ceil((8+(d.value||10)*.08+rank*12)*discount)}
...
function monsterLootHTML(m){const rows=monsterLootTable(m.id);if(!rows.length)return '<div class="loot-empty">Brak zarejestrowanych łupów.</div>';return `<div class="bestiary-loot-grid">${rows.map(r=>{const d=itemDef(r.id);return `<div class="bestiary-loot-row"><span class="loot-icon">${itemIconVisual(r.id,'mini-item-svg')}</span><div><b class="rarity-${d.rarity}">${d.name}</b><small>${lootChanceLabel(r.chance)} • ${r.min||1}${(r.max||r.min||1)!==(r.min||1)?`–${r.max}`:''} szt.</small></div></div>`}).join('')}</div>`}
...
function addItem(id,qty=1,affix=undefined){let left=Math.max(0,qty|0),added=0;if(isStackable(id)){const lim=stackLimit(id);for(const st of state.player.inventory.filter(x=>x.id===id&&isStackable(x.id)&&((x.qty||1)<lim))){if(left<=0)break;const room=lim-(st.qty||1),take=Math.min(room,left);st.qty=(st.qty||1)+take;left-=take;added+=take}while(left>0&&inventoryHasRoom()){const take=Math.min(lim,left);state.player.inventory.push({id,qty:take});left-=take;added+=take}}else{while(left>0&&inventoryHasRoom()){state.player.inventory.push(createGearInstance(id,affix));left--;added++}}if(left>0)toast(`🎒 Plecak pełny — nie zmieściło się ${left}× ${itemDef(id).name}.`);if(added)checkQuestProgress('item',id);save();return added}
...
function effectiveStat(key){return (state.player.stats[key]||0)+gearStat(key)}
...
function chooseWorldEvent(e,choiceId){
 ...
 if(mustFight||riskFight){const m=MONSTERS.find(x=>x.id===monsterId),lvl=clamp(state.player.level+(choice.elite?1:0),m.min,m.max),entity={id:`eventfight_${e.id}_${Date.now()}`,type:'monster',template:m.id,x:e.x,y:e.y,alive:true,elite:!!choice.elite,synthetic:true};closeModal();toast(`⚠️ ${choice.result||'Zostałeś zaatakowany!'}`);setTimeout(()=>startCombat(entity,{level:lvl,worldEvent:{eventId:e.id,choiceId:choice.id}}),260);return}
 completeWorldEvent(e,choice);
}
function openWorldEvent(e){
 if(!e||e.type!=='event')return;ensureLivingWorld();if(state.world.living.completedEvents.includes(e.id)||e.done)return toast('To wydarzenie zostało już ukończone.');
...
function fastTravelTo(id){
 ...
 state.player.position={x:n.x,y:n.y,lat:ll[0],lng:ll[1],gps:false,gpsFixAt:0,accuracy:0,heading:null,virtualTravel:true};state.world.exploration.travelVisited.push(id);save();closeModal();selectNav('map');toast(`Szybka podróż: ${n.name} • -${cost} 🪙. Tryb GPS-interakcji jest zablokowany.`)
}
...
function normalizeState(s){
 ...
 s.player.inventory ||= [];
 s.player.inventory=normalizeInventoryStacks(s.player.inventory);ensureCityState(s);
...
function primaryCombatStat(){const p=state.player;return p.class==='mage'?effectiveStat('int'):['hunter','ranger'].includes(p.class)?effectiveStat('agi'):effectiveStat('str')}
...
function armorPower(){return Math.floor(effectiveStat('vit')*.55+gearStat('armor'))}
function critChance(){const p=state.player,pet=p.petActive?petDef(p.petActive):null,classBonus=p.class==='hunter'?5:p.class==='ranger'?2:0,biomeBonus=combat?.environment?.effect?.crit||0;return Math.min(70,5+effectiveStat('agi')*.55+gearStat('crit')+(pet?.crit||0)+classBonus+biomeBonus)}
function dodgeChance(){const p=state.player,classBonus=p.class==='ranger'?7:p.class==='hunter'?3:p.class==='mage'?1:0,biomeBonus=combat?.environment?.effect?.dodge||0,storyBonus=combat?.monster?.family==='Natura'&&storyChoiceFor('q2')==='spare'?2:0;return Math.min(42,2+effectiveStat('agi')*.42+classBonus+(combat?.playerDodgeBuff||0)+biomeBonus+storyBonus)}
...
function buyTavernStamina(amount,cost,label){cost=Math.ceil(cost*(1-Math.min(2,(state?.city?.levels?.tavern||1)-1)*.05));const p=state.player;p.maxStamina ??=100;p.stamina ??=p.maxStamina;if(p.stamina>=p.maxStamina)return toast('Masz pełną staminę.');if(p.gold<cost)return toast(`Potrzebujesz ${cost} 🪙.`);p.gold-=cost;p.stamina=Math.min(p.maxStamina,p.stamina+amount);save();openBuilding('tavern','keeper');toast(`${label}: +${amount} staminy • -${cost} 🪙`)}
...
function rebuildGameLayers(){
 ...
 if(state.city?.center){const center=[state.city.center.lat,state.city.center.lng],ring=L.circle(center,{radius:CITY_RADIUS,color:'#e2b55e',weight:3,dashArray:'9 5',fillColor:'#e2b55e',fillOpacity:.12}).addTo(realMap),pin=L.marker(center,{icon:makeLeafletIcon('<div class="city-map-pin">🏰</div>','city-map-icon',[48,48])}).addTo(realMap);pin.on('click',()=>{if(cityAccess())selectNav('town');else toast('Wejdź do obszaru miasta i włącz GPS, aby korzystać z usług.')});leafletZoneLayers.push(ring,pin)}
 rebuildBiomeLayers();
 rebuildTrail();
...
function updateLiveMapPosition(){
 ...
 const frame=document.querySelector('.real-map-rpg-frame');let entry=frame?.querySelector('[data-city-enter]');if(frame&&insideCity()&&!entry){frame.insertAdjacentHTML('beforeend','<button class="city-map-entry" data-city-enter>🏰 Wejdź do miasta</button>');entry=frame.querySelector('[data-city-enter]');entry?.addEventListener('click',()=>selectNav('town'))}else if(entry&&!insideCity())entry.remove();
...
function renderMap(el){
 ...
 el.querySelector('[data-city-enter]')?.addEventListener('click',()=>selectNav('town'));
 ...
 el.querySelectorAll('[data-world-event]').forEach(b=>b.onclick=()=>{const e=eventById(b.dataset.worldEvent);if(!e)return;const d=dist(e,state.player.position);if(d<=60)openWorldEvent(e);else toast(`${e.signal||e.name} • ${Math.round(d)} m`)});
...
function moveDemo(dx,dy){
 ...
 pos.gps=false;pos.gpsFixAt=0;pos.accuracy=0;pos.heading=null;pos.virtualTravel=false;pos.testWalk=true;
...
function toggleGps(){
 if(gpsWatch!==null){navigator.geolocation?.clearWatch(gpsWatch);gpsWatch=null;if(state?.player?.position){state.player.position.gps=false;state.player.position.gpsFixAt=0;}save();toast('GPS wyłączony.');if(currentTab==='map'||currentTab==='town')selectNav(currentTab);return}
 if(!navigator.geolocation)return toast('Ta przeglądarka nie udostępnia GPS.');
 followGps=true;toast('Uruchamiam dokładny GPS…');
 const handlePosition=pos=>{
  const now=Date.now();if(now-lastGpsTick<800)return;lastGpsTick=now;
  ...
  state.player.position={x:dx,y:dy,lat,lng,gps:true,gpsFixAt:now,accuracy,heading,virtualTravel:false,testWalk:false};
  if(state.city?.center&&insideCity())state.city.lastVisitAt=now;
  ...
  if(currentTab==='town')selectNav('town');
  if(currentTab==='map'){
...
function equipIndexToSlot(idx,slot){
 ...
 const oldSlot=inventoryItemEquippedSlot(i);if(oldSlot&&oldSlot!==slot)state.player.equipped[oldSlot]=null;
...
function inventoryCard(i,idx){const d=itemDef(i.id),eqSlot=Object.entries(state.player.equipped).find(([,x])=>x?.uid&&x.uid===i.uid)?.[0],eq=!!eqSlot,qty=i.qty||1,up=i.upgrade||0,details=[d.damage?`Obrażenia ${d.damage[0]}–${d.damage[1]}`:'',d.power&&!d.damage?`ATK +${d.power+up*2}`:'',d.armor?`Pancerz +${d.armor+up*2}`:'',d.crit?`Kryt +${d.crit+up}%`:'',d.ammo?`Amunicja: ${itemDef(d.ammo).name}`:''].filter(Boolean).join(' • '),mods=[i.affix?.name?`✨ Afiks: ${i.affix.name}`:'',i.enchant?.name?`🔮 Enchant: ${i.enchant.name}`:'',i.rune?`${itemDef(i.rune).icon} ${itemDef(i.rune).name}`:'',d.set?`⚜️ ${SET_BONUSES[d.set]?.name||d.set}`:'',...['str','agi','int','vit'].filter(k=>(d[k]||0)+(i.affix?.[k]||0)+(i.enchant?.[k]||0)>0).map(k=>`+${(d[k]||0)+(i.affix?.[k]||0)+(i.enchant?.[k]||0)} ${{str:'Siły',agi:'Zręczności',int:'Inteligencji',vit:'Witalności'}[k]}`)].filter(Boolean);return `<div class="item-card rarity-card-${d.rarity}"><div class="item-top"><div class="item-icon">${itemIconVisual(d.id,'inventory-item-svg')}</div><div><b class="rarity-${d.rarity}">${itemName(i)}${qty>1?` ×${qty}`:''}</b><div class="tiny">${rarityName(d.rarity)}${d.reqLevel?` • wymagany lvl ${d.reqLevel}`:''} • ${d.slot?slotLabel(d.slot):d.type}${details?` • ${details}`:''}${d.classes?`<br>Klasa: ${itemClassNames(d)}`:''}</div>${mods.length?`<div class="item-mods">${mods.map(x=>`<span>${x}</span>`).join('')}</div>`:''}</div></div>${eq?`<div class="equipped-tag">ZAŁOŻONE: ${slotLabel(eqSlot)}</div>`:''}<div class="tabs" style="margin-top:8px">${d.slot?(eq?`<button class="secondary" data-unequip="${i.uid}">Zdejmij</button>`:inventoryEquipButtons(d,idx)):''}${d.type==='consumable'?`<button class="secondary" data-use="${d.id}">Użyj</button>`:''}${d.slot&&!eq?`<button class="ghost" data-salvage="${idx}">♻️ Rozbierz</button>`:''}</div></div>`}
...
function unequipItem(uidv){const slot=Object.entries(state.player.equipped||{}).find(([,x])=>x?.uid===uidv)?.[0];if(!slot)return false;const i=equippedInstance(slot);state.player.equipped[slot]=null;playSfx('equip');save();refresh();toast(`Zdjęto: ${i?itemName(i):slotLabel(slot)}`);return true}
...
function renderEvents(el){
 ...
 el.querySelectorAll('[data-world-event]').forEach(b=>b.onclick=()=>{const e=eventById(b.dataset.worldEvent);if(!e)return;const d=dist(e,state.player.position);if(d<=60)openWorldEvent(e);else{state.ui.mapPanelTab='events';state.ui.mapSheetOpen=false;selectNav('map');toast(`${e.signal||e.name} • ${Math.round(d)} m od Ciebie.`)}})
}
...
const CITY_INTERIORS={
 ...
 auction:{title:'Dom aukcyjny',npc:'Varo',role:'Licytator',classId:'ranger',bg:'assets/backgrounds/interior-auction-380.webp',quote:'„Każdy przedmiot ma wartość. Pytanie brzmi: dla kogo?”'},
...
function renderTown(el){
 setAmbient('town');ensureCityState();const cl=climate(),access=cityAccess(),mobile=phoneMode();
 const cardCopy={tavern:['Dorian','Odpoczynek • plotki • questy'],shop:['Selma','Handel • wyposażenie klasowe'],smith:['Ragor','Ulepszanie • runy • rzemiosło'],alchemist:['Ilyra','Mikstury • receptury'],auction:['Varo','Oferty dnia • rzadki sprzęt'],guild:['Edrin','Reputacja • gildia']};
 const canMove=!state.city.center||Date.now()-state.city.placedAt>=CITY_RELOCATE_MS;
 el.innerHTML=`<div class="city-hub-shell"><div class="city-hub-hero"><div><span class="eyebrow">WIOSKA POD KRUKIEM • ${mobile?'STREFA GPS':'ZARZĄDZANIE W PRZEGLĄDARCE'}</span><h2>Twoje miasto</h2><p>${cl.icon} ${cl.weather} • ${cl.phase} • ${cityStatus()}</p></div><div class="city-hub-seal">⚜️</div></div><div class="city-access-panel"><div><b>🏰 Strefa miasta: 200 m</b><p>${mobile?'Ustaw centrum z prawdziwej pozycji GPS. Usługi działają podczas pobytu w strefie.':'Rozbudowa i usługi wymagają wizyty potwierdzonej na telefonie. Zapis jest lokalny — przenieś go eksportem/importem w Menu.'}</p></div><div class="city-access-buttons">${mobile?`<button class="secondary" data-city-place ${canMove&&liveCityFix()?'':'disabled'}>${state.city.center?'Przenieś miasto (co 30 dni)':'Ustaw miasto tutaj'}</button><button class="secondary" data-city-gps>${gpsWatch!==null?'GPS włączony':'Włącz GPS'}</button>`:state.settings.demo?`<button class="ghost" data-city-demo>🧪 ${access?'Odśwież dostęp testowy':'Wypróbuj miasto (30 min)'}</button>`:''}</div></div><div class="city-district-grid">${BUILDINGS.map(b=>{const u=buildingUnlock(b.id),c=CITY_INTERIORS[b.id],copy=cardCopy[b.id]||[c?.npc||'',b.tag||''],lvl=state.city.levels[b.id]||1,q=cityUpgradeQuote(b.id),ready=access&&u.ok&&lvl<3&&state.player.gold>=q.gold&&countItem('scrap')>=q.scrap&&countItem('herb')>=q.herb;return `<article class="city-building-unit"><button class="city-building-card ${u.ok&&access?'':'locked-building'}" data-building="${b.id}" ${u.ok&&access?'':'disabled'} style="--card-bg:url('${c?.bg||''}')"><span class="city-building-art"></span><span class="city-building-shade"></span><span class="city-building-npc">${npcVisual(c.npc,c.classId,'city-card-npc')}</span><span class="city-building-copy"><small>${copy[0]}</small><b>${b.name}</b><em>${u.ok?copy[1]:`🔒 ${u.reason}`}</em><strong>${u.ok&&access?'WEJDŹ DO ŚRODKA →':u.ok?'WYMAGANA WIZYTA':'ZABLOKOWANE'}</strong></span></button>${mobile?`<div class="city-mobile-level">Poziom ${lvl} • ${CITY_UPGRADES[b.id]}</div>`:`<div class="city-building-management"><div><b>Poziom ${lvl}/3</b><small>${CITY_UPGRADES[b.id]}</small></div>${lvl<3?`<button class="secondary" data-city-upgrade="${b.id}" ${ready?'':'disabled'}>Rozbuduj • ${q.gold} 🪙${q.scrap?` + ${q.scrap} 🔩`:''}${q.herb?` + ${q.herb} 🌿`:''}</button>`:'<span class="pill green">Rozbudowano</span>'}</div>`}</article>`}).join('')}</div></div>`;
 el.querySelectorAll('[data-building]:not(:disabled)').forEach(b=>b.onclick=()=>openBuilding(b.dataset.building,'scene'));
 el.querySelector('[data-city-place]')?.addEventListener('click',placeCity);
 el.querySelector('[data-city-gps]')?.addEventListener('click',()=>{if(gpsWatch===null)toggleGps();else toast('GPS jest już aktywny.')});
 el.querySelector('[data-city-demo]')?.addEventListener('click',()=>{state.city.demoPassUntil=Date.now()+30*60*1000;save();renderTown(el)});
...
function buildingSceneHTML(id,view='scene'){
 ...
 return `<div class="tavern-clean-stage clean-room-stage room-${id}">
   ...
   <div class="clean-room-scene-npc" aria-label="${c.npc} — ${c.role}">${npcVisual(c.npc,c.classId,'clean-room-scene-sprite')}</div>
   <div class="clean-room-counter" aria-hidden="true" style="--room-bg:url('${c.bg}')"></div>
   <div class="clean-room-nameplate"><b>${c.npc}</b><small>${c.role}</small></div>
   ${hotspots}${sheet}
 </div>`;
}
...
function openBuilding(id,view='scene'){
 if(!cityAccess())return toast('Usługi miasta wymagają wizyty w strefie GPS. W przeglądarce możesz też włączyć dostęp próbny.');
...
const ATTRIBUTE_LABELS={str:'Siła',agi:'Zręczność',int:'Inteligencja',vit:'Witalność'};
function affixStatsHTML(affix){return affix?Object.entries(ATTRIBUTE_LABELS).filter(([key])=>affix[key]).map(([key,label])=>`<span>✦ +${affix[key]} ${label}</span>`).join(''):''}
function itemStatsHTML(d,affix=null){const stats=[];if(d.damage)stats.push(`⚔️ ${d.damage[0]}–${d.damage[1]}`);if(d.armor!==undefined)stats.push(`🛡️ ${d.armor}`);if(d.power)stats.push(`✦ +${d.power} mocy`);if(d.crit)stats.push(`🎯 +${d.crit}%`);for(const [k,label] of Object.entries(ATTRIBUTE_LABELS))if(d[k])stats.push(`+${d[k]} ${label}`);if(affix)for(const [k,label] of Object.entries(ATTRIBUTE_LABELS))if(affix[k])stats.push(`✨ +${affix[k]} ${label}`);if(d.reqLevel)stats.push(`lvl ${d.reqLevel}`);if(!stats.length)stats.push(d.type==='material'?'Materiał rzemieślniczy':d.type==='ammo'?'Amunicja':'Przedmiot użytkowy');return stats.map(x=>`<span>${x}</span>`).join('')}
function marketItemCardHTML(id,price,action,button='Kup',badge='Towar Selmy',affix=null){const d=itemDef(id),leveled=state.player.level>=(d.reqLevel||1),room=isStackable(id)?countItem(id)%stackLimit(id)!==0||inventoryHasRoom():inventoryHasRoom(),affordable=state.player.gold>=price,available=leveled&&room&&affordable;return `<article class="market-card rarity-frame-${d.rarity} ${!leveled?'market-locked':''}"><div class="market-card-top"><span class="market-badge">${badge}</span><span class="rarity-${d.rarity}">${rarityName(d.rarity)}</span></div><div class="market-product"><div class="market-product-icon">${itemIconVisual(d.id,'shop-item-svg')}</div><div><h3>${d.name}${affix?` <small class="market-affix-name">${affix.name}</small>`:''}</h3><div class="market-stats">${itemStatsHTML(d,affix)}</div><small class="market-class">${d.classes?itemClassNames(d):'Wszystkie klasy'}</small></div></div><footer><div class="market-price"><small>Cena</small><b>${price} 🪙</b></div><button class="secondary market-buy" ${action} ${available?'':'disabled'}>${!leveled?`Od lvl ${d.reqLevel}`:!room?'Brak miejsca':!affordable?'Za mało złota':button}</button></footer></article>`}
function recipeCardHTML(r,verb){const d=itemDef(r.result),fee=craftFee(r),ingredients=Object.entries(r.ingredients),ready=ingredients.every(([id,q])=>countItem(id)>=q)&&state.player.gold>=fee;return `<article class="recipe-card rarity-frame-${d.rarity}"><div class="recipe-output"><div class="market-product-icon">${itemIconVisual(d.id,'shop-item-svg')}</div><div><span>${r.qty>1?`${r.qty}× `:''}${rarityName(d.rarity)}</span><h3>${r.name}</h3><div class="market-stats">${itemStatsHTML(d)}</div></div></div><div class="ingredient-tray">${ingredients.map(([id,q])=>{const have=countItem(id),ok=have>=q;return `<span class="${ok?'ready':'missing'}" title="Masz ${have}">${itemIconVisual(id,'mini-item-svg')} ${q} <small>/${have}</small></span>`}).join('')}</div><footer><div class="market-price"><small>Opłata</small><b>${fee} 🪙</b></div><button class="secondary market-buy" data-craft="${r.id}" ${ready?'':'disabled'}>${ready?verb:'Brakuje składników'}</button></footer></article>`}
...
const SHOP_SPECIAL_AFFIXES={chainVest:'Siłacza',rangerLeather:'Zwinności',apprenticeRobe:'Mędrca',berserkerHarness:'Bohatera',ironArmor:'Hartowności'};
function shopAffix(id){const d=itemDef(id);if(!d.slot||d.rarity==='common')return null;const selected=SHOP_SPECIAL_AFFIXES[id];if(selected)return AFFIXES.find(a=>a.name===selected)||null;const seed=stableTextSeed(`${daySeed()}:${id}`);return seed%4===0?AFFIXES[seed%AFFIXES.length]:null}
function shopPrice(id){const a=shopAffix(id);return Math.ceil(merchantBuyPrice(id)*(a?1.38:1))}
function shopStock(){return Object.values(ITEMS).filter(d=>d.slot&&['common','uncommon','rare'].includes(d.rarity)&&!d.set&&itemClassAllowed(d)).sort((a,b)=>(a.reqLevel||1)-(b.reqLevel||1)||a.value-b.value)}
function shopHTML(){const p=state.player,filter=state.ui.shopFilter||'all',categories=[['all','Wszystko'],['weapon','Broń'],['armor','Pancerze'],['gear','Pozostały sprzęt'],['trinket','Biżuteria'],['supplies','Zaopatrzenie']],basics=['potion','strongPotion','manaPotion','antidote','herb','scrap',...(['hunter','ranger'].includes(p.class)?['primitiveArrow']:[])],stock=shopStock().filter(d=>filter==='all'||(filter==='gear'?d.type==='gear':d.type===filter)),supplies=filter==='all'||filter==='supplies';return `${serviceHeroHTML('shop','SKLEP KUPIECKI','Towary Selmy','Pełny katalog sprzętu dla Twojej klasy. Poziom i pojemność plecaka decydują o zakupie.')}<div class="merchant-quote">„Zwykły sprzęt jest tańszy. Wybrane sztuki mają dodatkowe cechy — sprawdź je przed zakupem.”</div><div class="shop-filter-bar">${categories.map(([key,name])=>`<button class="${filter===key?'active':''}" data-shop-filter="${key}">${name}</button>`).join('')}</div>${supplies?`${sectionTitleHTML('🎒','Zaopatrzenie','Materiały, strzały i mikstury')}<div class="market-grid">${basics.map(id=>marketItemCardHTML(id,shopPrice(id),`data-buy="${id}"`,'Kup','Na szlak')).join('')}</div>`:''}${filter!=='supplies'?`${sectionTitleHTML('⚔️',`Wyposażenie: ${CLASSES[p.class]?.name||p.class}`,`${stock.length} przedmiotów • odblokowywane wraz z poziomem • ${p.gold} 🪙`)}<div class="market-grid">${stock.map(d=>marketItemCardHTML(d.id,shopPrice(d.id),`data-buy="${d.id}"`,'Kup',shopAffix(d.id)?'Wzmocniony egzemplarz':'Zwykły sprzęt',shopAffix(d.id))).join('')}</div>`:''}${sectionTitleHTML('💰','Sprzedaj łupy','Sprzedaż tylko w sklepie; wyposażony sprzęt pozostaje na postaci')}<div class="shop-sell-list">${backpackEntries().filter(({item})=>itemDef(item.id).value>0).map(({item:i,index})=>`<div class="shop-sell-row"><span>${itemIconVisual(i.id,'mini-item-svg')}</span><b>${itemName(i)}${(i.qty||1)>1?` ×${i.qty}`:''}</b><button class="ghost" data-shop-sell="${index}">Sprzedaj 1 • ${itemSellValue(i)} 🪙</button></div>`).join('')||'<div class="service-empty">W plecaku nie ma nic na sprzedaż.</div>'}</div>`}
...
function bindBuilding(id,view){
 ...
 document.querySelectorAll('[data-shop-filter]').forEach(b=>b.onclick=()=>{state.ui.shopFilter=b.dataset.shopFilter;save();openBuilding('shop','service')});
 document.querySelectorAll('[data-shop-sell]').forEach(b=>b.onclick=()=>{if(!cityAccess())return toast('Sprzedaż wymaga wizyty w mieście.');const index=Number(b.dataset.shopSell);if(inventoryItemEquippedSlot(state.player.inventory[index]))return;sellIndex(index);openBuilding('shop','service')}); 
 ...
 document.querySelectorAll('[data-auction-buy]').forEach(b=>b.onclick=()=>{const price=Number(b.dataset.price);if(!cityAccess())return toast('Zakupy wymagają wizyty w mieście.');const offer=itemDef(b.dataset.auctionBuy);if(offer.reqLevel>state.player.level)return toast(`Wymagany poziom ${offer.reqLevel}.`);if(!inventoryHasRoom())return toast('Plecak jest pełny.');if(state.player.gold<price)return toast('Za mało złota.');if(!addItem(b.dataset.auctionBuy))return;state.player.gold-=price;save();openBuilding('auction','service');toast('Kupiono ofertę z rynku.')});
...
function buyItem(id,building){if(!cityAccess())return toast('Zakupy wymagają wizyty w mieście.');const d=itemDef(id),allowed=building==='shop'&&(shopStock().some(x=>x.id===id)||['potion','strongPotion','manaPotion','antidote','herb','scrap','primitiveArrow'].includes(id)&&(['hunter','ranger'].includes(state.player.class)||id!=='primitiveArrow'));if(!allowed)return toast('Tego towaru nie ma w sklepie dla Twojej klasy.');if(d.reqLevel&&state.player.level<d.reqLevel)return toast(`Wymagany poziom ${d.reqLevel}.`);const price=shopPrice(id),free=isStackable(id)?countItem(id)%stackLimit(id)!==0||inventoryHasRoom():inventoryHasRoom();if(!free)return toast('Plecak jest pełny.');if(state.player.gold<price)return toast('Za mało złota.');const got=addItem(id,1,d.slot?shopAffix(id):undefined);if(!got)return;state.player.gold-=price;save();openBuilding(building,'service');toast(`Kupiono: ${d.name}${shopAffix(id)?' '+shopAffix(id).name:''} • -${price} 🪙`)}
...
function exportSave(){const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='time4heroes-build-3.8.0-save.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
...
function queueEnemyTurn(delay=1100){
 if(!combat)return;combat.phase='enemyDelay';const token=++combat.turnToken;save();openCombat();setTimeout(()=>{if(combat&&combat.turnToken===token&&combat.phase==='enemyDelay')enemyTurn()},delay)
}
function finishEnemyTurn(delay=1050){
 if(!combat)return;combat.phase='enemyResult';const token=++combat.turnToken;save();openCombat();setTimeout(()=>{if(combat&&combat.turnToken===token&&combat.phase==='enemyResult'){combat.phase='player';combat.lastPlayerHit=null;combat.lastEnemyHit=null;combat.fx='idle';save();openCombat()}},delay)
}
function enemyTurn(){
...
function combatPotion(){if(!combat||combat.phase!=='player'||!countItem('potion'))return;resetCombatExchange();combat.turnDealtNote='MIKSTURA';playSfx('heal');removeItem('potion');state.player.hp=Math.min(state.player.maxHp,state.player.hp+55);logCombat('Wypijasz Miksturę życia (+55 HP).');queueEnemyTurn()}
function fleeCombat(){if(!combat||combat.phase!=='player')return;if(combat?.dungeon)return toast('Nie możesz uciec z tej komnaty lochu.');if(Math.random()<.7){combat=null;renderShell();toast('Udało Ci się uciec.')}else{logCombat('Nie udało się uciec!');queueEnemyTurn()}}
function battleLootRow(drop,index){const d=itemDef(drop.id);return `<label class="battle-loot-row rarity-card-${d.rarity}"><input type="checkbox" data-battle-loot="${index}"><span class="battle-loot-icon">${itemIconVisual(drop.id,'mini-item-svg')}</span><span class="battle-loot-copy"><b class="rarity-${d.rarity}">${d.name}</b><small>${drop.qty>1?`${drop.qty} szt. • `:''}${d.rarity} • ${d.type==='material'?'materiał':d.type==='consumable'?'zużywalny':d.slot?'ekwipunek':'przedmiot'}</small></span><span class="battle-loot-take">ZABIERZ</span></label>`}
...
function finishBattleResult(result,takeLoot=true){const taken=takeLoot?collectSelectedBattleLoot(result):[];if(result.dungeonInfo&&dungeonRun&&result.pauseStarted){dungeonRun.deadline+=Math.max(0,Date.now()-result.pauseStarted)}closeModal();save();if(taken.length){playSfx('loot');toast(`Zabrano: ${lootToastText(taken)}`)}if(result.dungeonInfo){advanceDungeonAfterCombat(result.dungeonInfo);return}renderShell();queueReadyStoryScene(null,260)}
