import {CLASSES,MONSTERS,MONSTER_LOOT,ITEMS,QUESTS,SKILLS,PETS,RECIPES,DUNGEONS,BUILDINGS} from './data.js?v=3972';
...
const ALCHEMY_READY_STOCK=[
 {id:'potion',price:38},{id:'manaPotion',price:44},{id:'strongPotion',price:92},{id:'antidote',price:34}
];
...
const GROUP_RAIDS={
 hero:{label:'Heros',icon:'⚔️',recommendedParty:'2–4',soloLevelGap:5,levelBand:.55,daily:3,hp:2.8,atk:1.18,rep:12,gold:90,monsterPool:['goblinChampion','pyreKnight','frozenKnight','blackrootGuardian']},
 legend:{label:'Legenda',icon:'👑',recommendedParty:'3–4',soloLevelGap:10,levelBand:.78,daily:1,hp:4.9,atk:1.34,rep:35,gold:260,monsterPool:['graveColossus','stormDrake','tempestLord','abyssHydra']}
};
...
function alchemyReadyPrice(row){return Math.max(1,Math.ceil(row.price*(1-cityDiscount('alchemist'))))}
...
function raidIncomingMultiplier(){if(!combat?.raidTier)return 1;const allies=combat.partyMembers?.length||0;return Math.max(.76,1-allies*.06)}
function raidPartyAssist(){if(!combat?.raidTier||!combat.partyMembers?.length||combat.hp<=0)return 0;let total=0;const details=[];for(const member of combat.partyMembers){const classMult=member.class==='mage'?1.08:member.class==='ranger'?1.02:member.class==='knight'?.90:1;const dmg=Math.max(2,Math.floor(attackPower()*(.27+Math.min(10,Math.max(0,(member.level||state.player.level)-state.player.level))*.008)*classMult*(.88+Math.random()*.24)));total+=dmg;details.push(`${member.name} ${dmg}`)}combat.hp-=total;combat.raidPartyDamage=(combat.raidPartyDamage||0)+total;combat.lastPlayerHit=(combat.lastPlayerHit||0)+total;if(combat.isBoss)addStagger(Math.min(14,4+combat.partyMembers.length*3));logCombat(`🤝 Drużyna atakuje: ${details.join(' • ')} • razem ${total}.`);return total}
...
const TUTORIAL_STEPS=[
 ...
 {id:'kill',title:'Pierwsza walka',text:'Podejdź do słabego przeciwnika i wygraj pierwszą walkę. Pierwszy pokonany potwór gwarantuje sprzęt dla Twojej klasy.',go:'map'},
 {id:'tavern',title:'Bezpieczny powrót',text:'Odwiedź Karczmę „Pod Krukiem”. To Twój pierwszy bezpieczny hub.',go:'town'},
 {id:'inventory',title:'Sprawdź zdobycz',text:'Otwórz Bohater → Ekwipunek. Po pierwszej walce masz gwarantowany klasowy przedmiot z łupu.',go:'inventory'},
...
function itemIconVisual(id,cls='item-svg'){
 const d=itemDef(id),visualId=d.visualId||id,pos=ITEM_ART_ATLAS[visualId],family=/^(mist|storm)/.test(visualId)?'frost':/^(ash|ember|cinder)/.test(visualId)?'ember':/^(wild|forest|wolf)/.test(visualId)?'forest':'neutral';
 const art=pos?`<span class="atlas-sprite item-atlas-sprite ${cls} item-tone-${family}" role="img" aria-label="${d.name}" style="${atlasStyle('assets/atlases/item-atlas-320.png',pos,4,4)}"></span>`:`<img src="assets/icons/items/${visualId}.svg" class="${cls}" alt="${d.name}" loading="lazy">`;
...
const CLASS_SKILL_TRIALS={
 knight:{shield:{type:'kills',count:1,label:'Pokonaj pierwszego przeciwnika'},counter:{type:'kills',count:10,label:'Pokonaj 10 przeciwników'},taunt:{type:'kills',count:18,label:'Pokonaj 18 przeciwników'}},
 mage:{fire:{type:'kills',count:1,label:'Pokonaj pierwszego przeciwnika'},frost:{type:'family',family:'Nieumarli',count:6,label:'Pokonaj 6 Nieumarłych'},spark:{type:'kills',count:14,label:'Pokonaj 14 przeciwników'}},
 hunter:{double:{type:'kills',count:1,label:'Pokonaj pierwszego przeciwnika'},petStrike:{type:'family',family:'Natura',count:12,label:'Pokonaj 12 stworzeń Natury'},camouflage:{type:'kills',count:22,label:'Pokonaj 22 przeciwników'},volley:{type:'kills',count:36,label:'Pokonaj 36 przeciwników'}},
 berserker:{rage:{type:'kills',count:1,label:'Pokonaj pierwszego przeciwnika'},cleave:{type:'kills',count:10,label:'Pokonaj 10 przeciwników'},dualCut:{type:'kills',count:16,label:'Pokonaj 16 przeciwników'}},
 ranger:{poison:{type:'kills',count:1,label:'Pokonaj pierwszego przeciwnika'},trap:{type:'monster',monster:'wolf',count:10,label:'Pokonaj 10 Szarych Wilków'},spirit:{type:'family',family:'Natura',count:15,label:'Pokonaj 15 stworzeń Natury'}}
};
function skillTrialDef(skill){
 const special=CLASS_SKILL_TRIALS[state.player.class]?.[skill.id];if(special)return special;
 return {type:'kills',count:Math.max(6,Math.round((skill.req||1)*4)),label:`Pokonaj ${Math.max(6,Math.round((skill.req||1)*4))} przeciwników`};
}
function skillTrialProgress(skill){
 ...
 else value=Number(state.player.kills||0);
 return {trial:t,value:Math.min(t.count,value),raw:value,done:value>=t.count};
}
function skillTrialText(skill){const x=skillTrialProgress(skill);return `${x.trial.label} • ${x.value}/${x.trial.count}`}
...
function upgradeQuote(inst){const d=itemDef(inst.id),up=inst.upgrade||0,rank=rarityRank(d.rarity);return {gold:Math.ceil((16+d.value*.10+rank*8)*Math.pow(up+1,1.28)*(1-cityDiscount('smith'))*(1-guildForgeDiscount())),scrap:up===0?0:Math.max(1,Math.ceil((up+rank)/2))}}
...
function rollCombatLoot(m,e,c){ensureEconomyState();let gearDrop=null;const drops=rollMonsterMaterials(m,e,c);
 ...
 if(gearDrop)drops.push({id:gearDrop,qty:1,gear:true});
...
function playerVsEnemyLevelMultiplier(){const gap=combatLevelGap();return gap>0?Math.max(.72,1-gap*.03):1}
function enemyHpLevelGapMultiplier(level){const gap=Math.max(0,combatLevelGap(level));return 1+Math.min(.20,gap*.02)}
function enemyAtkLevelGapMultiplier(level){const gap=Math.max(0,combatLevelGap(level));return 1+Math.min(.25,gap*.02)}
...
function ensureShopUiState(){state.ui=state.ui||{};if(!state.ui.shopPage)state.ui.shopPage='3';if(typeof state.ui.shopClassOnly!=='boolean')state.ui.shopClassOnly=false;if(!state.ui.shopMode)state.ui.shopMode='buy'}
function shopInventoryPages(){const classGear={knight:['rustySword','shortSword','chainVest','woodenShield','ironArmor'],berserker:['rustySword','shortSword','ironAxe','chainVest','berserkerHarness'],mage:['noviceStaff','apprenticeStaff','apprenticeRobe','mageHood','mageShoes'],hunter:['hunterBow','yewBow','rangerLeather','scoutHood','trailBoots'],ranger:['hunterBow','yewBow','rangerLeather','scoutHood','trailBoots']}[state.player.class]||[];const basics=['potion','manaPotion','herb','scrap',...(['hunter','ranger'].includes(state.player.class)?['primitiveArrow']:[])],gear=classGear.filter(id=>!itemDef(id).reqLevel||itemDef(id).reqLevel<=state.player.level+5),all=[...basics,...gear.filter(id=>!basics.includes(id))];return {1:basics,2:gear,3:all}}
...
function auctionHTML(){ensureEconomyState();ensureAuctionUiState();processAuctionListings();const offerIds=['blueBlade','forestBow','arcaneStaff','shadowRing','wolfCharm','ironHelm','emberRing','ravenBlade','wildBow','arcaneRobe','mistBlade','mistBow','mistStaff','mistHelm','mistHood','mistCirclet'].filter(id=>itemClassAllowed(itemDef(id)));const ids=[0,1,2,3].map(i=>offerIds[Math.floor(seeded(daySeed()+i*91)*offerIds.length)]);return `${serviceHeroHTML('auction','DOM AUKCYJNY','Katalog Vara','Kupuj oferty dnia albo wystaw własne przedmioty na lokalnym rynku.')}<div class="auction-ribbon"><span>🔨 DOM AUKCYJNY</span><b>Oferty Vara + własne aukcje</b><small>Wystawienie 2% • prowizja 5% • limit ${auctionListingLimit()}</small></div>${auctionListingFormHTML()}${ownAuctionListingsHTML()}${sectionTitleHTML('👑','Oferty kupca','Rzadkie przedmioty wybrane dla Twojej klasy • katalog zmienia się każdego dnia')}<div class="auction-grid">${ids.map((id,i)=>{const d=itemDef(id),factor=ECONOMY.auctionMin+seeded(daySeed()+i*17)*(ECONOMY.auctionMax-ECONOMY.auctionMin)+(rarityRank(d.rarity)>=3?.12:0),price=Math.ceil(d.value*factor);return marketItemCardHTML(id,price,`data-auction-buy="${id}" data-price="${price}"`,'Kup ofertę',i===0?'Oferta dnia':`Gablota ${i+1}`)}).join('')}</div>`}
...
function tryEnemySpecialAttack(){
 ...
 const dodge=Math.random()*100<dodgeChance()*.55;if(dodge){combat.lastEnemyHit=null;setCombatTaken(0,def.name.toUpperCase());logCombat(`💨 Unikasz specjalnego ataku: ${def.name}.`);return true}
 const blocked=Math.random()*100<blockChance(),guardMult=combat.guard>0?.52:1,blockMult=blocked?.60:1,freezeMult=combat.freezeTurns>0?.82:1,reduction=armorPower()*(def.armor??.22),mult=(1-combat.debuff)*guardMult*blockMult*freezeMult,dmg=Math.max(1,Math.floor((combat.atk*def.mult-reduction)*mult*combatIncomingGearMultiplier()));
...
function combatCommandMenuHTML(c,p,combatSkills,potionIds,potionTotal,locked){
 ...
 return `<div class="battle-command-panel main-command-panel"><div class="command-question"><b>Co zrobisz?</b><span>${rangeHint}</span></div><div class="pokemon-command-grid"><button class="pokemon-command fight" data-command="fight" ${locked?'disabled':''}><span>⚔️</span><b>Walka</b><small>${melee&&distance>0?'Ataki wręcz wymagają BLISKO':'Atak i umiejętności'}</small></button><button class="pokemon-command position" data-command="position" ${locked?'disabled':''}><span>${melee?'👣':'🎯'}</span><b>${melee?'Podejście':'Pozycja'}</b><small>${distance===0?'BLISKO':distance===1?'ŚREDNIO':'DALEKO'}</small></button><button class="pokemon-command heal" data-command="heal" ${locked||!potionTotal?'disabled':''}><span>🧪</span><b>Leczenie</b><small>${potionTotal} mikstur</small></button><button class="pokemon-command flee" data-flee ${locked||c.dungeon||c.raidTier?'disabled':''}><span>🏃</span><b>Ucieczka</b><small>${c.dungeon||c.raidTier?'zablokowana':'70% szans'}</small></button></div></div>`;
}
...
function startCombat(entity,opts={}){
 ...
 const levelGap=lvl-p.level;if(levelGap>=3)combat.log.push(`⚠️ Przeciwnik ma +${levelGap} poziomów: więcej HP i obrażeń, a Twoje ciosy są słabsze.`);
...
function openCombat(){
 ...
 if(c.raidTier&&contextHost){const party=document.createElement('div');party.className='raid-party-strip';party.innerHTML=`<b>${c.raidTier==='legend'?'👑 LEGENDA':'⚔️ HEROS'}</b><span>🧙 ${state.player.name}</span>${(c.partyMembers||[]).map(m=>`<span>${m.icon||'⚔️'} ${m.name}</span>`).join('')}<em>${(c.partyMembers||[]).length?`Twój wkład: ${c.raidPlayerDamage||0} dmg • drużyna: ${c.raidPartyDamage||0} dmg`:`Tryb SOLO • przeciwnik lvl ${c.level}`}</em>`;contextHost.before(party)}
...
function winCombat(){
 ...
 state.player.gold+=baseGold;state.player.kills++;recordGuildMonsterKill(c.raidTier||null);playSfx('kill');haptic(18);tutorialEvent('kill');state.player.bestiary[m.id]=(state.player.bestiary[m.id]||0)+1;if(!e.synthetic){e.alive=false;e.respawn=Date.now()+5*60*1000}checkQuestProgress('kill',m.id);const loot=rollCombatLoot(m,e,c);if((c.environment?.effect?.loot||1)>1&&Math.random()<(c.environment.effect.loot-1)){const extra=monsterLootTable(m.id)[0];if(extra)loot.drops=mergeLootDrops([...loot.drops,{id:extra.id,qty:1}])}gainXp(xp);gainPetXp(xp);if(e.elite||c.isBoss)playSfx('loot');if(c.worldBoss){state.adventure.worldBossDay=daySeed();state.adventure.reputation+=25;worldBossRep=25;state.player.gold+=180;gold+=180;loot.drops.push({id:'titanShard',qty:1},{id:'runeShard',qty:2});if(Math.random()<.55)loot.drops.push({id:pick(['runePower','runeGuard','runePrecision']),qty:1});if(Math.random()<.18)loot.drops.push({id:pick(['stormCrown','cryptHeart']),qty:1});loot.drops=mergeLootDrops(loot.drops)}if(c.raidTier){ensureSocialState();const rd=raidDef(c.raidTier),eligible=(c.raidPlayerActions||0)>=2||(c.raidPlayerDamage||0)>=c.maxHp*.05;state.social.raids[`${c.raidTier}Wins`]=(state.social.raids[`${c.raidTier}Wins`]||0)+1;const guildRaidMult=guildExpeditionBonus();raidRep=Math.ceil(rd.rep*guildRaidMult);const raidGold=Math.ceil(rd.gold*guildRaidMult);state.adventure.reputation+=raidRep;state.player.gold+=raidGold;gold+=raidGold;if(eligible){loot.drops.push({id:'runeShard',qty:c.raidTier==='legend'?2:1});if(c.raidTier==='legend'){loot.drops.push({id:'titanShard',qty:2});if(Math.random()<.35)loot.drops.push({id:pick(['stormCrown','cryptHeart']),qty:1})}else if(Math.random()<.28)loot.drops.push({id:pick(['runePower','runeGuard','runePrecision']),qty:1})}loot.drops=mergeLootDrops(loot.drops);state.social.raids.history.unshift({tier:c.raidTier,monster:m.id,at:Date.now(),party:1+(c.partyMembers?.length||0),playerDamage:c.raidPlayerDamage||0,partyDamage:c.raidPartyDamage||0,eligible});state.social.raids.history=state.social.raids.history.slice(0,20)}if(c.storyConsequence?.qid)checkQuestProgress('story',c.storyConsequence.qid);if(c.worldEvent?.eventId){const event=eventById(c.worldEvent.eventId),choice=event?.choices?.find(x=>x.id===c.worldEvent.choiceId);if(event)completeWorldEvent(event,choice||{label:'Walka wygrana'},{deferRender:true})}updateAchievements();const pauseStarted=c.dungeon&&dungeonRun?Date.now():0;if(pauseStarted)clearDungeonTimer();const result={xp,gold,worldBossRep,raidRep,raidTier:c.raidTier||null,raidPartySize:c.raidTier?1+(c.partyMembers?.length||0):0,raidContribution:c.raidTier?Math.round(100*(c.raidPlayerDamage||0)/Math.max(1,(c.raidPlayerDamage||0)+(c.raidPartyDamage||0))):0,drops:loot.drops,dungeonInfo:c.dungeon,pauseStarted};combat=null;battleResult=result;save();showBattleVictory(result);
}
