import {CLASSES,MONSTERS,ITEMS,QUESTS,SKILLS,PETS,RECIPES,DUNGEONS,BUILDINGS} from './data.js?v=1140';

const SAVE_KEY='time4heroes_build_114';
const MIGRATION_KEYS=['time4heroes_build_111','time4heroes_build_110','time4heroes_build_19','time4heroes_build_18','time4heroes_build_17','time4heroes_build_16','time4heroes_build_15','time4heroes_build_14','time4heroes_build_13','time4heroes_build_12_core','time4heroes_build_11','time4heroes_build_10','time4heroes_build_09','time4heroes_build_08','georpg_build_07','georpg_build_06','georpg_build_05','georpg_build_04','georpg_build_03','georpg_build_02','georpg_build_01'];
...
function destroyRealMap(){
 ...
 fogLayer=null;trailLayer=null;playerMapMarker=null;accuracyCircle=null;interactionCircle=null;leafletEntityLayers=[];leafletZoneLayers=[];leafletBiomeLayers=[];
}
...
function rebuildFog(){
 ...
 const radius=100,holes=[];const pts=[...(state.world.explored||[]).slice(-420)];
 if(state.player.position?.lat&&state.player.position?.lng)pts.push([state.player.position.lat,state.player.position.lng]);
 for(const [lat,lng] of pts){if(b.pad(.25).contains([lat,lng]))holes.push(circleRing(lat,lng,radius,28))}
 fogLayer=L.polygon([outer,...holes],{pane:'fogPane',stroke:false,fill:true,fillColor:'#020507',fillOpacity:.88,fillRule:'evenodd',interactive:false}).addTo(realMap);
}
...
function initRealMap(){
 ...
 if(p.lat){
  ...
  accuracyCircle=L.circle([p.lat,p.lng],{pane:'overlayPane',radius:Math.max(8,p.accuracy||15),color:'#65b7ff',weight:1,fillColor:'#4da7ef',fillOpacity:.07,interactive:false}).addTo(realMap);
  interactionCircle=L.circle([p.lat,p.lng],{pane:'overlayPane',radius:60,color:'#e0bb62',weight:2,dashArray:'5 7',fillColor:'#e0bb62',fillOpacity:.035,interactive:false}).addTo(realMap);
 }
...
function renderMap(el){setAmbient('forest');
 destroyRealMap();ensureLivingWorld();const p=state.player,hasGeo=!!(p.position.lat&&p.position.lng),virtual=!!p.position.virtualTravel;
 for(const e of state.world.entities)if(e.type==='monster'&&!e.alive&&e.respawn<=Date.now())e.alive=true;
 el.innerHTML=`<div class="play-map-only"><div class="real-map-shell"><div id="realMap" class="real-map"></div><div class="mmo-map-top map-status-minimal"><span class="mmo-map-badge" data-live-gps>${virtual?'TRYB DOMOWY':hasGeo?`GPS ±${Math.round(p.position.accuracy||0)} m`:'GPS wyłączony'}</span><span class="mmo-map-badge interaction-badge">⚔️ interakcja 60 m</span></div>${tutorialMapOverlay()}${!hasGeo?`<div class="gps-start-card compact-gps-card"><b>📍 Rozpocznij eksplorację</b><p>GPS ustawi Cię na mapie. Odkrywasz teren w promieniu 100 m, a z obiektami wchodzisz w interakcję do 60 m.</p><button class="primary" data-gps-start>Włącz GPS</button></div>`:''}</div></div>`;
 el.querySelector('[data-gps-start]')?.addEventListener('click',toggleGps);bindTutorialControls(el);initRealMap();
}
...
function renderQuests(el){updateStoryConditions();ensureStoryState();const all=[...state.quests.active,...state.quests.done].map(id=>QUESTS.find(q=>q.id===id)).filter(Boolean),active=state.quests.active.map(id=>QUESTS.find(q=>q.id===id)).find(Boolean),chapter=active?.chapter||all.at(-1)?.chapter||'Przygoda';el.innerHTML=`<div class="section-title"><h2>📜 Dziennik zadań</h2><span class="pill">${chapter}</span></div>${tutorialJournalHTML()}${storyProfileHTML()}<div class="panel-list">${all.map(q=>questHTML(q)).join('')||'<div class="panel-item">Brak zadań.</div>'}</div>`;el.querySelectorAll('[data-story-scene]').forEach(b=>b.onclick=()=>openStoryScene(b.dataset.storyScene));bindTutorialControls(el)}
...
function exportSavefunction exportSave(){const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='time4heroes-build-1.14-save.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
