...
const SAVE_KEY='georpg_build_07';
const OLD_SAVE_KEY='georpg_build_06';
const OLDER_SAVE_KEY='georpg_build_05';
const LEGACY_SAVE_KEY='georpg_build_04';
...
function load(){
 ...
 const older=rawLoad(OLDER_SAVE_KEY);if(older){const migrated=normalizeState(older);localStorage.setItem(SAVE_KEY,JSON.stringify(migrated));return migrated}
...
function renderMore(el){el.innerHTML=`<div class="section-title"><h2>⚙️ Ustawienia</h2><span class="pill">Build 0.7</span></div><div class="panel-list"><div class="panel-item"><b>💾 Zapis gry</b><div class="muted">Build 0.7 automatycznie migruje zapis 0.6, 0.5, 0.4, 0.3, 0.2 i 0.1. Zapis działa lokalnie w przeglądarce.</div><div class="tabs" style="margin-top:8px"><button class="secondary" data-export>Eksportuj</button><button class="secondary" data-import>Importuj</button><input type="file" id="saveFile" accept="application/json" hidden></div></div><div class="panel-item"><b>🌙 Symulacja nocy</b><div class="muted">Przydatna do testowania questów zależnych od pory dnia.</div><button class="secondary" data-night>${state.settings.forceNight?'Wyłącz symulację':'Włącz noc'}</button></div><div class="panel-item"><b>🧪 Tryb testowy mapy</b><div class="muted">Strzałki przesuwają bohatera o 20 m bez GPS.</div></div><div class="panel-item"><b>🎨 Galeria Build 0.7</b><div class="muted">Podgląd kierunku graficznego nowej wersji.</div><button class="secondary" data-showcase>Otwórz galerię</button></div><div class="panel-item"><b>🆕 Co nowego w 0.7</b><div class="muted">Nowy ekran startowy, galeria wersji 0.7, większe karty bestiariusza, mikroanimacje mapy i walki oraz dalsze szlify grafiki.</div></div><div class="panel-item"><b>🔄 Reset</b><div class="muted">Usuwa zapis Build 0.7.</div><button class="danger" data-reset>Usuń zapis</button></div></div>`;
 el.querySelector('[data-showcase]')?.addEventListener('click',openShowcase);el.querySelector('[data-export]').onclick=exportSave;el.querySelector('[data-import]').onclick=()=>document.querySelector('#saveFile').click();document.querySelector('#saveFile').onchange=importSave;el.querySelector('[data-night]').onclick=()=>{state.settings.forceNight=!state.settings.forceNight;save();refresh()};el.querySelector('[data-reset]').onclick=()=>{if(confirm('Na pewno usunąć zapis?')){localStorage.removeItem(SAVE_KEY);state=null;render()}};
}

function openShowcase(){openModal(`<div class="modal-head"><div><h2>🎨 GeoRPG 0.7 — Visual Pack II</h2><div class="muted">Kierunek graficzny i interfejs</div></div><button class="close" data-close>×</button></div><img src="assets/showcase07.png" alt="GeoRPG 0.7 showcase" class="showcase07"><p class="muted">Ta plansza pokazuje docelowy klimat kolejnych ekranów: mapy, walki, questów, bestiariusza i miasta.</p>`) }
function exportSave(){const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='georpg-build-0.7-save.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
