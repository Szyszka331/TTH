...
function addStagger(amount){
 ...
 if(combat.stagger>=combat.staggerMax){trialStat('bossBreaks');combat.stagger=0;combat.stunned=1;combat.vulnerableTurns=1;if(combat.intent){logCombat('💥 Przełamanie przerywa zamiar bossa!');combat.intent=null}else logCombat('💥 Boss zostaje PRZEŁAMANY i traci następną akcję!');playSfx('boss');haptic([30,20,45]);return true}return false;
}
function interruptIntent(skill,meta){
 if(!combat?.intent||!meta?.interrupt||!combat.intent.interruptible)return false;const info=bossIntentInfo(combat.intent);combat.intent=null;combat.stunned=1;combat.stagger=Math.min(combat.staggerMax,combat.stagger+20);trialStat('interrupts');logCombat(`✋ ${skill.name} przerywa: ${info.name}. Boss traci akcję.`);playSfx('boss');haptic([20,20,30]);return true;
}
...
function playerAction(skill){
 ...
 else if(skill.kind==='poison'){const h=hitDamage(skill.mult,0,meta.type);lastHit=h;total=h.dmg;combat.hp-=h.dmg;if(!h.miss){combat.poison=Math.max(4,Math.floor(attackPower()*(state.player.class==='ranger'?.29:.24)*(combat.environment?.poison||1)));combat.poisonTurns=skill.turns;trialStat('poisonApplied');logCombat(`${skill.name}: ${h.dmg}${hitSuffix(h)} i trucizna.`)}else logCombat(`${skill.name}: pudło.`)}
...
function petAttack(force){const p=petInstance();if(!p)return;if(!force&&Math.random()>.35)return;const dmg=Math.max(2,Math.floor(petPower()*(.8+Math.random()*.4)));trialStat('petAttacks');combat.hp-=dmg;if(combat.hp<=0)trialStat('petKills');combat.lastPlayerHit=(combat.lastPlayerHit||0)+dmg;addCombatDealt(dmg,'Atak + chowaniec');logCombat(`${petDef(p.id).icon} ${petDef(p.id).name} atakuje za ${dmg}.`)}
