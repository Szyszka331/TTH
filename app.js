...
function itemCanGoToSlot(i,slot){
 if(!i)return false;const d=itemDef(i.id);if(!d?.slot)return false;
...
function equipIndex(idx){const i=state.player.inventory[idx];if(!i)return;const d=itemDef(i.id);if(d.reqLevel&&state.player.level<d.reqLevel)return toast(`Wymagany poziom ${d.reqLevel}.`);let slot=d.slot;if(!slot)return;if(slot==='ring')slot=!state.player.equipped.ring1?'ring1':!state.player.equipped.ring2?'ring2':'ring1';if(equipIndexToSlot(idx,slot))refresh()}
...
function exportSave(){const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='time4heroes-build-2.6-save.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
