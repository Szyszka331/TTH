import {CLASSES,MONSTERS,MONSTER_LOOT,ITEMS,QUESTS,SKILLS,PETS,RECIPES,DUNGEONS,BUILDINGS} from './data.js?v=3970';
...
const CITY_UPGRADE_RULES={
 ...
 alchemist:{name:'Alchemik',icon:'⚗️',costs:{2:{gold:300,scrap:2},3:{gold:760,scrap:5}},effects:{1:'standardowe warzenie',2:'−6% opłat za mikstury',3:'−12% opłat za mikstury'}},
...
function ensureCityState(s=state){if(!s)return;s.city ||= {buildings:{}};s.city.buildings ||= {};for(const b of BUILDINGS)s.city.buildings[b.id]=clamp(Number(s.city.buildings[b.id]||1),1,CITY_MAX_LEVEL)}
...
const TUTORIAL_STEPS=[
 ...
 {id:'inventory',title:'Sprawdź zdobycz',text:'Otwórz Bohater → Ekwipunek. Po pierwszej walce czeka tam gwarantowany przedmiot dla Twojej klasy.',go:'inventory'},
...
function ensureCoreState(s=state){if(!s)return;ensureStoryState(s);ensureCityState(s);s.ui ||= {heroView:'char',adventureView:'quests',menuView:'settings'};s.tutorial ||= {stage:0,complete:false,rewardGiven:false,flags:{},introSeen:true};s.tutorial.flags ||= {};s.tutorial.introSeen ??= true;s.tutorial.mapDismissedStage ??= -1;s.settings ||= {};s.settings.audio ??= true;s.settings.ambient ??= true;s.settings.masterSound ??= (s.settings.audio||s.settings.ambient);s.settings.sfxVolume ??= .68;s.settings.ambientVolume ??= .18;s.settings.haptics ??= true;s.settings.mapMode ||= 'focused';s.settings.mapFilters ||= {};s.settings.mapFilters.trail ??= true;s.player.inventoryCapacity ??= 32;s.world.regionRewards ||= {};s.world.fogRadius=100;ensureDungeonAccessState(s);ensureCombatSkillLoadout(s);ensureBackpackSlots(s);ensurePassiveRegenState(s);}


const PASSIVE_REGEN={staminaMs:6*60*1000,hpPerMinute:.02,manaPerMinute:.02,tickMs:15000};
function ensurePassiveRegenState(target=state,now=Date.now()){
 const p=target?.player;if(!p)return null;
 p.passiveRegen ||= {lastAt:now,hpCarry:0,manaCarry:0,staminaCarry:0};
 const r=p.passiveRegen;
 if(!Number.isFinite(r.lastAt)||r.lastAt<=0)r.lastAt=now;
 r.hpCarry=Math.max(0,Number(r.hpCarry)||0);r.manaCarry=Math.max(0,Number(r.manaCarry)||0);r.staminaCarry=Math.max(0,Number(r.staminaCarry)||0);
 return r;
}
function passiveRegenIsPaused(){return !!combat||!!dungeonRun||!!battleResult}
function applyPassiveRegen(target=state,now=Date.now(),paused=(target===state&&passiveRegenIsPaused())){
 const p=target?.player;if(!p)return {changed:false,hp:0,mana:0,stamina:0};
 p.maxStamina=60;p.stamina=clamp(p.stamina??60,0,60);p.hp=clamp(p.hp??p.maxHp,0,p.maxHp);p.mana=clamp(p.mana??p.maxMana,0,p.maxMana);
 const r=ensurePassiveRegenState(target,now),elapsed=Math.max(0,now-r.lastAt);r.lastAt=now;
 if(paused||elapsed<=0)return {changed:false,hp:0,mana:0,stamina:0};
 let hpGain=0,manaGain=0,staminaGain=0;
 if(p.hp<p.maxHp){const total=r.hpCarry+(elapsed/60000)*p.maxHp*PASSIVE_REGEN.hpPerMinute,gain=Math.floor(total+1e-9);if(gain>0){const before=p.hp;p.hp=Math.min(p.maxHp,p.hp+gain);hpGain=p.hp-before}r.hpCarry=p.hp>=p.maxHp?0:Math.max(0,total-gain)}else r.hpCarry=0;
 if(p.mana<p.maxMana){const total=r.manaCarry+(elapsed/60000)*p.maxMana*PASSIVE_REGEN.manaPerMinute,gain=Math.floor(total+1e-9);if(gain>0){const before=p.mana;p.mana=Math.min(p.maxMana,p.mana+gain);manaGain=p.mana-before}r.manaCarry=p.mana>=p.maxMana?0:Math.max(0,total-gain)}else r.manaCarry=0;
 if(p.stamina<p.maxStamina){const total=r.staminaCarry+elapsed/PASSIVE_REGEN.staminaMs,gain=Math.floor(total+1e-9);if(gain>0){const before=p.stamina;p.stamina=Math.min(p.maxStamina,p.stamina+gain);staminaGain=p.stamina-before}r.staminaCarry=p.stamina>=p.maxStamina?0:Math.max(0,total-gain)}else r.staminaCarry=0;
 return {changed:!!(hpGain||manaGain||staminaGain),hp:hpGain,mana:manaGain,stamina:staminaGain};
}
function refreshPassiveResourceHud(){
 if(!state?.player)return;const p=state.player;
 document.querySelectorAll('[data-hud-hp-bar]').forEach(el=>el.style.width=`${100*p.hp/p.maxHp}%`);document.querySelectorAll('[data-hud-hp-label]').forEach(el=>el.textContent=`HP ${p.hp}/${p.maxHp}`);
 document.querySelectorAll('[data-hud-mana-bar]').forEach(el=>el.style.width=`${100*p.mana/p.maxMana}%`);document.querySelectorAll('[data-hud-mana-label]').forEach(el=>el.textContent=`MANA ${p.mana}/${p.maxMana}`);
 document.querySelectorAll('[data-hud-stamina]').forEach(el=>el.textContent=`${p.stamina}/${p.maxStamina}`);
 document.querySelectorAll('.drawer-meter.hp i').forEach(el=>el.style.width=`${100*p.hp/p.maxHp}%`);document.querySelectorAll('.drawer-meter.hp span').forEach(el=>el.textContent=`Życie ${p.hp} / ${p.maxHp}`);
 document.querySelectorAll('.drawer-meter.mana i').forEach(el=>el.style.width=`${100*p.mana/p.maxMana}%`);document.querySelectorAll('.drawer-meter.mana span').forEach(el=>el.textContent=`Mana ${p.mana} / ${p.maxMana}`);
}
function passiveRegenTick(now=Date.now()){
 if(!state)return;const result=applyPassiveRegen(state,now);if(result.changed){save();refreshPassiveResourceHud()}return result;
}
function startPassiveRegenTimer(){if(passiveRegenTimerId)clearInterval(passiveRegenTimerId);passiveRegenTimerId=setInterval(()=>passiveRegenTick(),PASSIVE_REGEN.tickMs)}
...
function tutorialFirstGearForClass(cls=state?.player?.class){return ({knight:'trainingShield',berserker:'trainingAxe',mage:'trainingStaff',hunter:'trainingBow',ranger:'trainingBow'})[cls]||'leatherGloves'}
...
function tutorialEvent(type,value=0){ensureCoreState();if(state.tutorial.complete)return;const f=state.tutorial.flags; if(type==='move'&&value>=60)f.move=true;else if(type==='kill'){f.kill=true;if(!state.tutorial.rewardGiven){const reward=tutorialFirstGearForClass();if(addItem(reward,1)){state.tutorial.rewardGiven=true;playSfx('loot');toast(`🎁 Pierwszy łup klasowy: ${itemDef(reward).name} trafił do plecaka.`)}}}else if(type==='tavern')f.tavern=true;else if(type==='inventory')f.inventory=true;else if(type==='equip')f.equip=true;else if(type==='skill')f.skill=true;else if(type==='secret')f.secret=true;else if(type==='dungeonDiscover')f.dungeonDiscover=true;else if(type==='dungeonComplete')f.dungeonComplete=true;advanceTutorial();save()}
...
const MAX_ACTIVE_SKILLS=4, SKILL_MAX_LEVEL=5;
const SKILL_RANK_LEVEL_OFFSETS=[0,2,5,9,14];
function ensureSkillLevels(p=state?.player){
 if(!p)return{};p.skills ||= [];p.skillLevels ||= {};
 const learned=new Set(p.skills);
 for(const id of learned){const raw=Math.floor(Number(p.skillLevels[id])||1);p.skillLevels[id]=clamp(raw,1,SKILL_MAX_LEVEL)}
 for(const id of Object.keys(p.skillLevels))if(!learned.has(id))delete p.skillLevels[id];
 return p.skillLevels;
}
function skillLevel(id,p=state?.player){if(!p?.skills?.includes(id))return 0;ensureSkillLevels(p);return clamp(Math.floor(Number(p.skillLevels[id])||1),1,SKILL_MAX_LEVEL)}
function skillRankRequiredLevel(skill,rank){rank=clamp(Math.floor(rank||1),1,SKILL_MAX_LEVEL);return Math.max(skill.req||1,(skill.req||1)+(SKILL_RANK_LEVEL_OFFSETS[rank-1]||0))}
function skillRankCost(skill,rank){if(rank<=1)return skill.cost||1;return rank<=3?1:2}
function effectiveSkill(skill,rank=skill?skillLevel(skill.id):0){
 if(!skill)return skill;rank=clamp(rank||1,1,SKILL_MAX_LEVEL);const bonus=rank-1,out={...skill,_level:rank,_maxLevel:SKILL_MAX_LEVEL};
 if(Number.isFinite(skill.mult))out.mult=Number((skill.mult*(1+bonus*.09)).toFixed(3));
 if(Number.isFinite(skill.value))out.value=Math.round(skill.value+bonus*4);
 if(Number.isFinite(skill.debuff))out.debuff=Math.min(.60,Number((skill.debuff+bonus*.03).toFixed(2)));
 if(Number.isFinite(skill.critBonus))out.critBonus=skill.critBonus+bonus*5;
 if(Number.isFinite(skill.statusChance))out.statusChance=Math.min(1,Number((skill.statusChance+bonus*.05).toFixed(2)));
 if(Number.isFinite(skill.turns))out.turns=skill.turns+Math.floor(bonus/2);
 if(skill.kind==='guard')out.guardStrength=Math.min(.68,.45+bonus*.05);
 if(Number.isFinite(skill.statusTurns))out.statusTurns=skill.statusTurns+Math.floor(bonus/2);
 if(Number.isFinite(skill.cooldown)&&rank>=5)out.cooldown=Math.max(0,skill.cooldown-1);
 if(skill.kind==='multi'&&rank>=5)out.hits=(skill.hits||1)+1;
 if(skill.kind==='mark')out.markValue=25+bonus*5;
 if(skill.kind==='debuff')out.debuffTurns=3+Math.floor(bonus/2);
 if(skill.kind==='poison')out.poisonScale=1+bonus*.12;
 if(skill.kind==='rage'){out.rageBase=1.2+bonus*.07;out.rageMissing=.7+bonus*.06}
 if(rank>=5&&Number.isFinite(skill.mana))out.mana=Math.max(0,skill.mana-2);
 return out;
}
function skillEffectSummary(skill){
 if(!skill)return'';const pct=v=>Math.round((v||0)*100),turns=skill.turns||skill.statusTurns||0;
 if(skill.kind==='damage')return `${pct(skill.mult)}% obrażeń${skill.critBonus?` • +${skill.critBonus}% krytyka`:''}${skill.status?` • efekt: ${skill.status}`:''}`;
 if(skill.kind==='multi')return `${skill.hits||1} traf. × ${pct(skill.mult)}%${skill.status?` • efekt: ${skill.status}`:''}`;
 if(skill.kind==='guard')return `${Math.round((skill.guardStrength||.45)*100)}% redukcji obrażeń przez ${skill.turns||1} tur${(skill.turns||1)===1?'ę':'y'}`;
 if(skill.kind==='buff')return `+${skill.value||0}% ${skill.buff||'premii'} przez ${skill.turns||1} tur`;
 if(skill.kind==='debuff')return `−${pct(skill.debuff)}% ataku wroga przez ${skill.debuffTurns||3} tur`;
 if(skill.kind==='mark')return `+${skill.markValue||25}% krytyka przez ${skill.turns||3} tur`;
 if(skill.kind==='poison')return `${pct(skill.mult)}% obrażeń • trucizna ×${(skill.poisonScale||1).toFixed(2)} przez ${skill.turns||3} tur`;
 if(skill.kind==='pet')return `${pct(skill.mult)}% obrażeń + pewny atak chowańca`;
 if(skill.kind==='rage')return `Cios rośnie z brakującym HP • skala mocy ×${(skill.rageBase||1.2).toFixed(2)}`;
 return skill.desc||'';
}
...
function activeCombatSkills(){ensureSkillLevels();return ensureCombatSkillLoadout().map(skillDef).filter(Boolean).map(s=>effectiveSkill(s))}
...
function ensureBackpackSlots(s=state){
 const inv=s?.player?.inventory||[],cap=s?.player?.inventoryCapacity||32,used=new Set(),pending=[],equippedUids=new Set(Object.values(s?.player?.equipped||{}).map(x=>x?.uid).filter(Boolean));
...
function backpackSortKey(item,mode){const d=itemDef(item.id);if(mode==='rarity')return [-rarityRank(d.rarity),d.name];if(mode==='name')return [d.name];if(mode==='value')return [-itemSellValue(item),d.name];const order={weapon:0,offhand:1,helmet:2,armor:3,gloves:4,boots:5,amulet:6,ring:7,consumable:8,ammo:9,rune:10,material:11};return [d.slot?order[d.slot]??7:order[d.type]??12,-rarityRank(d.rarity),d.name]}
...
const ECONOMY={sell:{common:.28,uncommon:.32,rare:.36,epic:.40,unique:.43,heroic:.46,legendary:.50},shopMarkup:1.12,auctionMin:.95,auctionMax:1.18,travelBase:10,travelPer100m:3};
const GEAR_POOLS={
 common:['rustySword','shortSword','ironAxe','noviceStaff','apprenticeStaff','hunterBow','yewBow','leather','apprenticeRobe','chainVest','rangerLeather','scoutHood','mageHood','leatherGloves','trailBoots','mageShoes','woodenShield'],
 uncommon:['oldTalisman','boneRing'],
 epic:['warHammer','axe','blueBlade','forestBow','emberWand','arcaneStaff','berserkerHarness','ironArmor','runicRobe','ironHelm','ironGauntlets','ironBoots','ironShield','wolfCharm','emberRing'],
 unique:['ravenBlade','ravenMail','ravenHelm','wildBow','wildMail','wildHood','wildBoots','arcaneRobe','shadowRing','mistBlade','mistBow','mistStaff','mistMail','mistLeathers','mistRobe','mistHelm','mistHood','mistCirclet','ashBlade','ashBow','ashStaff','ashMail','ashLeathers','ashRobe','ashHelm','ashHood','ashCowl','stormSpear','stormBow','stormStaff','stormMail','stormLeathers','stormRobe','stormHelm','stormHood','stormCowl'],
 heroic:['mireCharm','cinderCharm','tempestCharm'],legendary:['stormCrown','cryptHeart']
};
function rarityRank(r){return ({common:0,uncommon:1,rare:2,epic:3,unique:4,heroic:5,legendary:6})[r]||0}
function affixChance(r){return ({common:0,uncommon:.12,rare:.24,epic:.45,unique:.68,heroic:.86,legendary:1})[r]||0}
...
function merchantBuyPrice(id){const d=itemDef(id),typeMult=d.type==='material'?1.18:d.type==='consumable'?1.10:ECONOMY.shopMarkup;return Math.max(1,Math.ceil(d.value*typeMult*(1-shopDiscount())))}
function arrowAmmoDefs(){return Object.values(ITEMS).filter(d=>d.type==='ammo'&&(!d.classes||d.classes.some(c=>['hunter','ranger'].includes(c))))}
function totalArrowCount(){return arrowAmmoDefs().reduce((sum,d)=>sum+countItem(d.id),0)}
function arrowPackQty(id){const d=itemDef(id);return d.type==='ammo'?Math.max(1,d.packQty||100):1}
function shopPurchasePrice(id){const d=itemDef(id);if(d.type==='ammo'&&d.packPrice)return Math.max(1,Math.ceil(d.packPrice*(1-shopDiscount())));return merchantBuyPrice(id)}
function equippedBowReady(){const w=equippedInstance('weapon');return !!w&&itemDef(w.id)?.weaponKind==='bow'}
function equippedArrowId(required=1){if(!['hunter','ranger'].includes(state?.player?.class)||!equippedBowReady())return null;const off=state.player.equipped?.offhand,id=off?.ammo?off.id:null;if(!id||itemDef(id)?.type!=='ammo'||countItem(id)<required)return null;return id}
function availableArrowId(required=1){return equippedArrowId(required)}
function activeArrowId(required=1){return equippedArrowId(required)}
function arrowDamageRoll(id){const d=itemDef(id);if(d?.type!=='ammo'||!d.damage)return 0;return rnd(d.damage[0],d.damage[1])}
function equippedAmmoMatches(id){return !!id&&state.player.equipped?.offhand?.ammo&&state.player.equipped.offhand.id===id}
function unequipAmmo(){const off=state.player.equipped?.offhand;if(!off?.ammo)return false;const name=itemDef(off.id).name;state.player.equipped.offhand=null;save();refresh();toast(`Zdjęto amunicję: ${name}`);return true}
...
function upgradeQuote(inst){const d=itemDef(inst.id),up=inst.upgrade||0,rank=rarityRank(d.rarity);return {gold:Math.ceil((16+d.value*.10+rank*8)*Math.pow(up+1,1.28)*(1-cityDiscount('smith'))),scrap:up===0?0:Math.max(1,Math.ceil((up+rank)/2))}}
...
function craftFee(recipe){const d=itemDef(recipe.result),rank=rarityRank(d.rarity),disc=recipe.station==='smith'?cityDiscount('smith'):recipe.station==='alchemist'?cityDiscount('alchemist'):0;return Math.ceil((8+(d.value||10)*.08+rank*12)*(1-disc))}
...
function rollCombatLoot(m,e,c){ensureEconomyState();let gearDrop=null;const drops=rollMonsterMaterials(m,e,c);
 ...
 const elite=!!e.elite&&!c.worldBoss&&!c.dungeon?.boss,hero=!!c.dungeon?.boss&&!c.worldBoss,legend=!!c.worldBoss;
 const variantGearChance={normal:.012,young:.006,hardened:.024,corrupted:.040,ancient:.075}[m.variantId]??.012;
 // Zwykłe potwory mogą dać wyłącznie podstawowy ekwipunek. Najwyższe jakości są zarezerwowane dla specjalnych przeciwników.
 if(!elite&&!hero&&!legend&&Math.random()<variantGearChance)gearDrop=randomGearFrom(Math.random()<.22?'uncommon':'common');
 // Elity: epicki lub unikatowy przedmiot. Pity gwarantuje łup co kilka elit.
 if(elite){state.economy.elitePity++;if(Math.random()<.20||state.economy.elitePity>=4){gearDrop=randomGearFrom(Math.random()<.30?'unique':'epic');state.economy.elitePity=0}if(Math.random()<.18)drops.push({id:'runeShard',qty:1})}
 // Herosi / bossowie lochów: unikatowe i heroiczne przedmioty.
 if(hero){state.economy.bossPity++;if(Math.random()<.62||state.economy.bossPity>=3){gearDrop=randomGearFrom(Math.random()<.28?'heroic':'unique');state.economy.bossPity=0}else gearDrop=randomGearFrom('unique');if(Math.random()<.62)drops.push({id:'runeShard',qty:rnd(1,2)});if(Math.random()<.10)drops.push({id:pick(['runePower','runeGuard','runePrecision']),qty:1})}
 // Legendarny boss świata: może dać unikat, heroik lub legendę.
 if(legend){state.economy.bossPity++;const roll=Math.random();gearDrop=randomGearFrom(roll<.16?'legendary':roll<.48?'heroic':'unique');state.economy.bossPity=0;if(Math.random()<.75)drops.push({id:'runeShard',qty:rnd(1,2)});if(Math.random()<.16)drops.push({id:pick(['runePower','runeGuard','runePrecision']),qty:1})}
 if(gearDrop)drops.push({id:gearDrop,qty:1,gear:true});return {gearDrop,drops:mergeLootDrops(drops)};
}
function addItem(id,qty=1){if(!ITEMS[id]||!Number.isInteger(qty)||qty<=0)return 0;let left=Math.max(0,qty|0),added=0;if(isStackable(id)){const lim=stackLimit(id);for(const st of state.player.inventory.filter(x=>x.id===id&&isStackable(x.id)&&((x.qty||1)<lim))){if(left<=0)break;const room=lim-(st.qty||1),take=Math.min(room,left);st.qty=(st.qty||1)+take;left-=take;added+=take}while(left>0&&inventoryHasRoom()){const take=Math.min(lim,left);state.player.inventory.push({id,qty:take});left-=take;added+=take}}else{while(left>0&&inventoryHasRoom()){state.player.inventory.push(createGearInstance(id));left--;added++}}if(left>0)toast(`🎒 Plecak pełny — nie zmieściło się ${left}× ${itemDef(id).name}.`);if(added)checkQuestProgress('item',id,added);save();return added}
function removeItem(id,qty=1){let left=qty;for(let i=state.player.inventory.length-1;i>=0&&left>0;i--){const x=state.player.inventory[i];if(x.id!==id)continue;const q=x.qty||1;if(q>left){x.qty=q-left;left=0}else{left-=q;state.player.inventory.splice(i,1)}}if(state.player.equipped?.offhand?.ammo&&state.player.equipped.offhand.id===id&&countItem(id)<=0)state.player.equipped.offhand=null;save();return left===0}
...
const BOUNTY_GATHER_POOL=[
 {key:'herb',name:'Zielarskie zamówienie',icon:'🌿',target:'herb',minLevel:1},
 {key:'moonHerb',name:'Zioła na nocny wywar',icon:'🌙',target:'moonHerb',minLevel:12},
 {key:'mireMoss',name:'Mech dla alchemika',icon:'🌱',target:'mireMoss',minLevel:18},
 {key:'blackroot',name:'Korzenie z mokradeł',icon:'🪵',target:'blackroot',minLevel:24}
];
function bountyEligibleMonsters(level=1){
 const low=Math.max(1,level-5),high=level<6?level+1:level+3;
 return MONSTERS.filter(m=>m.min<=high&&m.max>=low&&!['graveColossus','stormDrake','mireMother','cinderMatriarch','tempestLord','abyssHydra'].includes(m.id));
}
function lootBountyCandidates(level=1){
 const out=[];
 for(const m of bountyEligibleMonsters(level)){
  for(const row of monsterLootTable(m.id)){
   const d=itemDef(row.id);
   if(!d||d.type!=='material'||row.chance<.30||['scrap','crystal','runeShard'].includes(row.id))continue;
   out.push({key:`loot:${m.id}:${row.id}`,type:'loot',target:row.id,source:m.id,dropChance:row.chance,icon:d.icon||'🎒',name:`Potrzebne: ${d.name}`});
  }
 }
 return out;
}
function bountyRewardScale(level=1,difficulty=1){
 const lv=Math.max(1,Number(level)||1),share=.25+Math.min(.10,Math.max(0,difficulty-1)*.035);
 const xp=Math.max(45,Math.round(xpNeed(lv)*share/5)*5);
 const gold=Math.max(25,Math.round((24+lv*7+Math.pow(lv,1.18)*1.7+difficulty*8)/2)*2);
 const rep=Math.max(2,Math.min(18,2+Math.floor(lv/8)+Math.round(difficulty)));
 return {xp,gold,rep};
}
function bountyNeedFor(type,level=1,dropChance=.5){
 const lv=Math.max(1,Number(level)||1);
 if(type==='loot')return Math.min(5,Math.max(2,2+Math.floor(lv/20)-(dropChance<.42?1:0)));
 if(type==='gather')return Math.min(9,4+Math.floor(lv/14));
 return Math.min(8,3+Math.floor(lv/12));
}
function makeScaledBounty(day,level=1,serial=0,usedKeys=[],preferredType=null){
 const lv=Math.max(1,Number(level)||1),used=new Set(usedKeys||[]),kill=bountyEligibleMonsters(lv).map(m=>({key:`kill:${m.id}`,type:'kill',target:m.id,icon:m.icon||'⚔️',name:`Polowanie: ${m.name}`,monster:m}));
 const gather=BOUNTY_GATHER_POOL.filter(x=>x.minLevel<=lv+2&&ITEMS[x.target]).map(x=>({...x,key:`gather:${x.key}`,type:'gather'}));
 const loot=lootBountyCandidates(lv);
 let pool=preferredType==='loot'?loot:preferredType==='gather'?gather:preferredType==='kill'?kill:[...kill,...gather,...loot];
 pool=pool.filter(x=>!used.has(x.key));if(!pool.length)pool=[...kill,...gather,...loot];
 const roll=seeded(day*97+lv*131+serial*977+17),tpl=pool[Math.floor(roll*pool.length)]||kill[0]||gather[0]||loot[0];
 if(!tpl)return null;
 const m=tpl.monster||MONSTERS.find(x=>x.id===(tpl.source||tpl.target));
 const danger=m?Math.max(1,Math.min(3,1+(m.min-lv)/5)):tpl.type==='loot'?1.4:1;
 const rewards=bountyRewardScale(lv,danger),need=bountyNeedFor(tpl.type,lv,tpl.dropChance||.5);
 return {id:`board_${day}_${serial}_${String(tpl.key).replace(/[^a-z0-9_-]/gi,'_')}`,templateKey:tpl.key,name:tpl.name,icon:tpl.icon||'📌',type:tpl.type,target:tpl.target,source:tpl.source||null,dropChance:tpl.dropChance||null,need,minLevel:Math.max(1,m?.min||tpl.minLevel||1),boardLevel:lv,xp:rewards.xp,gold:rewards.gold,rep:rewards.rep,progress:0,claimed:false,accepted:false,scaled3968:true};
}
function fillBountyBoard(target=state,count=3){
 if(!target?.adventure||!target?.player)return;
 target.adventure.bountySerial=Math.max(0,Number(target.adventure.bountySerial)||0);
 target.adventure.bounties=(target.adventure.bounties||[]).filter(b=>b&&!b.claimed).slice(0,count);
 while(target.adventure.bounties.length<count){
  const used=target.adventure.bounties.map(b=>b.templateKey).filter(Boolean),slot=target.adventure.bounties.length,preferred=['kill','loot','gather'][(slot+target.adventure.bountySerial)%3];
  const serial=++target.adventure.bountySerial,b=makeScaledBounty(target.adventure.day||daySeed(),target.player.level||1,serial,used,preferred);
  if(!b)break;target.adventure.bounties.push(b);
 }
}
function makeDailyBounties(day,level=state?.player?.level||1){
 const fake={player:{level:Math.max(1,Number(level)||1)},adventure:{day,bountySerial:0,bounties:[]}};fillBountyBoard(fake,3);return fake.adventure.bounties;
}
function ensureAdventureState(s=state){
 if(!s)return;
 s.adventure ||= {day:daySeed(),reputation:0,bounties:[],worldBossDay:0,achievements:{}};
 s.adventure.bountySerial=Math.max(0,Number(s.adventure.bountySerial)||0);s.adventure.bounties ||= [];
 const today=daySeed(),lv=s.player?.level||1,needsMigration=s.adventure.boardVersion!==3968;
 if(needsMigration){s.adventure.bounties=s.adventure.bounties.filter(b=>b?.accepted&&!b.claimed).slice(0,3);s.adventure.day=today;s.adventure.boardVersion=3968;fillBountyBoard(s,3)}
 else if(s.adventure.day!==today){s.adventure.day=today;s.adventure.bounties=s.adventure.bounties.filter(b=>b.accepted&&!b.claimed).slice(0,3);fillBountyBoard(s,3)}
 else{
  // Otwarte kartki zmieniają poziom razem z graczem. Przyjęte zachowują warunki z chwili przyjęcia.
  for(let i=0;i<s.adventure.bounties.length;i++){
   const b=s.adventure.bounties[i];if(b.accepted||b.claimed||b.boardLevel===lv)continue;
   const used=s.adventure.bounties.filter((_,j)=>j!==i).map(x=>x.templateKey).filter(Boolean),preferred=['kill','loot','gather'][(i+s.adventure.bountySerial)%3];
   s.adventure.bounties[i]=makeScaledBounty(today,lv,++s.adventure.bountySerial,used,preferred)||b;
  }
  fillBountyBoard(s,3);
 }
 for(const b of s.adventure.bounties||[])b.accepted ??= false;
 s.adventure.achievements ||= {};s.adventure.reputation ||= 0;s.adventure.worldBossDay ||= 0;
}
...
function progressBounties(type,target,amount=1){
 ensureAdventureState();
 for(const b of state.adventure.bounties){
  if(!b.accepted||b.claimed)continue;
  const itemTask=['gather','loot'].includes(b.type),matches=itemTask?b.target===target:monsterTargetMatches(b.target,target);
  if(!matches)continue;
  if((itemTask&&type==='item')||((b.type||'kill')==='kill'&&type==='kill'))b.progress=Math.min(b.need,(b.progress||0)+Math.max(1,amount||1));
 }
 updateAchievements();
}
...
function replaceBoardBounty(id){
 ensureAdventureState();const idx=state.adventure.bounties.findIndex(x=>x.id===id);if(idx<0)return null;
 const old=state.adventure.bounties[idx],used=state.adventure.bounties.filter((_,i)=>i!==idx).map(x=>x.templateKey).filter(Boolean),preferred=['kill','loot','gather'][(idx+state.adventure.bountySerial+1)%3];
 const next=makeScaledBounty(daySeed(),state.player.level||1,++state.adventure.bountySerial,used,preferred);if(next)state.adventure.bounties[idx]=next;else state.adventure.bounties.splice(idx,1);fillBountyBoard(state,3);
 if(state.ui?.questGuideId===`bounty:${old.id}`)state.ui.questGuideId=null;return next;
}
function claimBounty(id){
 ...
 if(['gather','loot'].includes(b.type)&&countItem(b.target)<b.need){toast(`Do oddania potrzebujesz ${b.need}× ${itemDef(b.target).name}.`);return false}
 state.world.entities=state.world.entities.filter(e=>e.bountyId!==b.id);
 if(['gather','loot'].includes(b.type))removeItem(b.target,b.need);
 state.player.gold+=b.gold;const rep=Math.max(1,Math.round(b.rep*guildRepMultiplier()));state.adventure.reputation+=rep;gainXp(b.xp);const next=replaceBoardBounty(b.id);save();renderShell();toast(`Kontrakt wykonany: +${b.xp} XP • +${b.gold} 🪙 • +${rep} reputacji${next?' • na tablicy pojawiło się nowe zlecenie':''}`);return true;
}
...
function generateLivingMonsters(day,origin=state?.player?.position||{x:0,y:0},source=state){
 const out=[];
 const {cx,cy}=localChunkCoords(origin.x||0,origin.y||0),respawns=source?.world?.living?.monsterRespawns||{},level=source?.player?.level||1,now=Date.now(),starterMode=level<=3&&!source?.tutorial?.complete,starterPool=['slime','rat','beetle'];
 for(let gx=cx-LOCAL_CHUNK_RADIUS;gx<=cx+LOCAL_CHUNK_RADIUS;gx++)for(let gy=cy-LOCAL_CHUNK_RADIUS;gy<=cy+LOCAL_CHUNK_RADIUS;gy++){
  ...
  for(let i=0;i<LOCAL_MOBS_PER_CHUNK;i++){
   const seed=baseSeed+i*193,forcedStarter=starterMode&&gx===cx&&gy===cy&&i<6,starterAngle=seeded(seed+7)*Math.PI*2,starterRadius=80+seeded(seed+13)*110,x=forcedStarter?(origin.x||0)+Math.cos(starterAngle)*starterRadius:gx*LOCAL_CHUNK_SIZE+55+seeded(seed+17)*(LOCAL_CHUNK_SIZE-110),y=forcedStarter?(origin.y||0)+Math.sin(starterAngle)*starterRadius:gy*LOCAL_CHUNK_SIZE+55+seeded(seed+53)*(LOCAL_CHUNK_SIZE-110),starterSpawn=starterMode&&Math.hypot(x-(origin.x||0),y-(origin.y||0))<=230;
   const goblinPatrol=!starterSpawn&&['crossroads','settlement'].includes(habitat.id)&&i<Math.min(3,goblinPool.length),template=starterSpawn?starterPool[Math.floor(seeded(seed+101)*starterPool.length)]:goblinPatrol?goblinPool[(i+Math.floor(seeded(baseSeed+71)*goblinPool.length))%goblinPool.length]:pool[Math.floor(seeded(seed+101)*pool.length)]||'wolf',id=`local_${day}_${gx}_${gy}_${i}`;let respawn=respawns[id]||0;
   const entity={id,type:'monster',template,variant:starterSpawn?'normal':monsterVariantFromRoll(seeded(seed+173),level),x,y,alive:respawn<=now,elite:starterSpawn?false:seeded(seed+149)>.92,biome:habitat.biome,habitat:habitat.id,dynamicLocal:true,chunk:`${gx}:${gy}`,tutorialStarter:starterSpawn};
...
function normalizeState(s){
 ...
 s.player.maxStamina=60; s.player.stamina ??= s.player.maxStamina; s.player.stamina=clamp(s.player.stamina,0,60); if(['hunter','ranger'].includes(s.player.class)){const current=s.player.equipped?.offhand,hasAmmo=id=>id&&ITEMS[id]?.type==='ammo'&&s.player.inventory.some(x=>x.id===id&&(x.qty||1)>0);if(!current?.ammo||!hasAmmo(current.id)){const preferred=hasAmmo(s.player.activeAmmoId)?s.player.activeAmmoId:(hasAmmo('primitiveArrow')?'primitiveArrow':s.player.inventory.find(x=>ITEMS[x.id]?.type==='ammo'&&(x.qty||1)>0)?.id||null);s.player.equipped.offhand=preferred?{id:preferred,ammo:true}:null}delete s.player.activeAmmoId}else if(s.player.equipped?.offhand?.ammo)s.player.equipped.offhand=null
 s.player.kills ||= 0;
...
function generateWorld(){
 ...
 for(let i=0;i<30;i++){
  const starterArea=i<6,angle=seeded(seed+i*31)*Math.PI*2,r=starterArea?75+seeded(seed+i*73)*125:230+seeded(seed+i*73)*285,x=Math.cos(angle)*r,y=Math.sin(angle)*r,zone=zoneAt(x,y);
  let pool=starterArea?MONSTERS.filter(m=>m.zone==='green'&&m.min<=2):MONSTERS.filter(m=>m.zone===zone||(zone!=='green'&&m.zone==='green'));if(!pool.length)pool=MONSTERS.filter(m=>m.zone==='green');
  const template=pool[Math.floor(seeded(seed+i*97)*pool.length)];
  ents.push({id:`m${seed}_${i}`,type:'monster',template:template.id,x,y,alive:true,respawn:0,elite:starterArea?false:seeded(seed+i*111)>.91,variant:starterArea?'normal':undefined,tutorialStarter:starterArea});
 }
 const poi=[
...
function bountyMonsterTemplate(b,index){if(b?.type==='loot'&&b.source)return b.source;if(b?.target!=='goblin')return b?.target;const level=state.player.level||1,pool=['goblin','goblinWarrior','goblinMage','goblinChampion'].filter(id=>(MONSTERS.find(m=>m.id===id)?.min||1)<=level+3);return pool[Math.floor(seeded(state.adventure.day+index*67)*pool.length)]||'goblin'}
function spawnBountyTargets(b){
 ...
 const remaining=Math.max(0,b.need-(b.progress||0));
 const desired=b.type==='loot'?Math.min(8,Math.max(2,Math.ceil(remaining/Math.max(.28,b.dropChance||.5)))):remaining;
 if(present>=desired)return;const need=Math.max(0,desired-present);
 for(let i=0;i<need;i++){
  const idx=present+i,pt=bountySpawnPoint(idx,Math.max(1,desired));
...
function topbar(){
 ...
 return `<header class="topbar topbar-fantasy topbar-compact"><div class="brand-side"><div class="identity"><div class="mini-avatar">${classVisual(p.class,'sprite-mini')}</div><div><b>${p.name}</b><div class="tiny">${c.name} • poziom ${p.level} <span class="build-chip">${SAVE_KEY===DEMO_SAVE_KEY?'TEST • osobny zapis':BUILD_VERSION}</span></div></div></div><div class="bars"><div><div class="barwrap"><div class="bar hp" data-hud-hp-bar style="width:${100*p.hp/p.maxHp}%"></div><div class="barlabel" data-hud-hp-label>HP ${p.hp}/${p.maxHp}</div></div><div class="barwrap"><div class="bar mana" data-hud-mana-bar style="width:${100*p.mana/p.maxMana}%"></div><div class="barlabel" data-hud-mana-label>MANA ${p.mana}/${p.maxMana}</div></div></div><div><div class="barwrap"><div class="bar xp" style="width:${100*p.xp/need}%"></div><div class="barlabel">XP ${p.xp}/${need}</div></div><div class="tiny">ATK ${attackPower()} • Pancerz ${armorPower()} • Kryt ${critChance().toFixed(0)}%${pet?` • 🐾 lvl ${pet.level}`:''}</div></div></div></div><div class="hud-right"><div class="resource resource-fantasy"><span>🪙 <strong>${p.gold}</strong></span><span>💎 <strong>${countItem('crystal')}</strong></span><span>⚡ <strong data-hud-stamina>${p.stamina??60}/${p.maxStamina??60}</strong></span>${['hunter','ranger'].includes(p.class)?`<span title="${itemDef(availableArrowId()||'primitiveArrow').name}">➶ <strong>${totalArrowCount()}</strong></span>`:''}</div><div class="world-meta"><span>${region}</span><span>${cl.icon} ${cl.weather}</span><span>🕒 ${now}</span></div></div></header>`;
}
...
function acceptStoryQuest(qid){const q=QUESTS.find(x=>x.id===qid);if(!q||state.quests.done.includes(qid)||state.quests.active.includes(qid))return;if(!canAcceptTask())return toast(`Możesz mieć maksymalnie ${activeTaskLimit()} aktywnych questów. Samouczek nie liczy się do limitu.`);state.quests.active.push(qid);state.quests.progress[qid] ||= q.steps.map(()=>0);syncQuestWorld();save();toast(`Przyjęto: ${q.name} • cel pojawił się na mapie`);closeModal();state.ui.adventureView='quests';currentTab='adventureHub';selectNav('adventureHub')}
function acceptBounty(id){ensureAdventureState();const b=state.adventure.bounties.find(x=>x.id===id);if(!b||b.claimed||b.accepted)return;if(!canAcceptTask())return toast(`Możesz mieć maksymalnie ${activeTaskLimit()} aktywnych questów. Samouczek nie liczy się do limitu.`);b.accepted=true;b.progress=0;b.acceptedLevel=state.player.level;spawnBountyTargets(b);save();toast(b.type==='loot'?`Przyjęto: ${b.name} • Dorian podpowiada: ${bountySourceHint(b)}`:`Przyjęto zlecenie: ${b.name} • cele pojawiły się na mapie`);openBuilding('tavern','board')}
...
function bindDrawerBagDragDrop(root){
 if(drawerState().gearDrawerFilter!=='all')return;let dragIdx=null,ghost=null,start=null,moved=false;
 const targets=()=>root.querySelectorAll('[data-drawer-bag-pos]');
 const clear=()=>{targets().forEach(t=>t.classList.remove('bag-drop-valid','bag-drop-hover'));ghost?.remove();ghost=null;dragIdx=null;start=null;moved=false};
 const hoverAt=(x,y)=>{const hit=document.elementFromPoint?.(x,y)?.closest?.('[data-drawer-bag-pos]');targets().forEach(t=>t.classList.toggle('bag-drop-hover',t===hit));return hit};
 root.querySelectorAll('.drawer-dnd-item').forEach(item=>{
  item.addEventListener('dragstart',e=>{dragIdx=Number(item.dataset.drawerItem);targets().forEach(t=>t.classList.add('bag-drop-valid'));e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/plain',String(dragIdx));item.classList.add('dragging')});
  item.addEventListener('dragend',()=>{item.classList.remove('dragging');clear()});
  item.addEventListener('pointerdown',e=>{if(e.pointerType==='mouse')return;dragIdx=Number(item.dataset.drawerItem);start={x:e.clientX,y:e.clientY};moved=false;item.setPointerCapture?.(e.pointerId)});
  item.addEventListener('pointermove',e=>{if(dragIdx===null||!start||e.pointerType==='mouse')return;const distPx=Math.hypot(e.clientX-start.x,e.clientY-start.y);if(distPx<8&&!moved)return;if(!moved){moved=true;targets().forEach(t=>t.classList.add('bag-drop-valid'));ghost=document.createElement('div');ghost.className='touch-drag-ghost';ghost.innerHTML=item.querySelector('.item-icon-shell')?.outerHTML||item.innerHTML;document.body.appendChild(ghost);window.__t4hSuppressBagClickUntil=Date.now()+700}ghost.style.left=`${e.clientX}px`;ghost.style.top=`${e.clientY}px`;hoverAt(e.clientX,e.clientY)});
  item.addEventListener('pointerup',e=>{if(dragIdx===null||e.pointerType==='mouse')return;const target=hoverAt(e.clientX,e.clientY),idx=dragIdx;if(moved&&target){moveBackpackItemToSlot(idx,Number(target.dataset.drawerBagPos));window.__t4hSuppressBagClickUntil=Date.now()+700;clear();drawerState().gearDrawerSelection=null;renderGearDrawer();return}clear()});
  item.addEventListener('pointercancel',clear);
 });
 targets().forEach(target=>{target.addEventListener('dragover',e=>{if(dragIdx===null)return;e.preventDefault();target.classList.add('bag-drop-hover')});target.addEventListener('dragleave',()=>target.classList.remove('bag-drop-hover'));target.addEventListener('drop',e=>{e.preventDefault();const idx=dragIdx??Number(e.dataTransfer.getData('text/plain'));if(Number.isFinite(idx)){moveBackpackItemToSlot(idx,Number(target.dataset.drawerBagPos));window.__t4hSuppressBagClickUntil=Date.now()+400;clear();drawerState().gearDrawerSelection=null;renderGearDrawer()}else clear()})})
}
...
function renderSkillsTree(el){
 ...
 el.innerHTML=`<div class="section-title"><div><h2>🌳 Umiejętności</h2><div class="muted">${c.name} • odblokuj umiejętność, a potem rozwijaj ją do poziomu ${SKILL_MAX_LEVEL}.</div></div><span class="pill gold">${p.skillPoints} pkt</span></div><div class="class-passive-card"><b>Pasyw klasy</b><span>${classPassiveText()}</span></div>${combatLoadoutHTML()}<div class="skill-branches standalone-skills">${branches.map(branch=>`<section class="skill-branch"><h4>${branch}</h4>${(SKILLS[p.class]||[]).filter(s=>(s.branch||'Umiejętności')===branch).map(s=>skillCard(s)).join('<div class="skill-link">↓</div>')}</section>`).join('')}</div>`;
 el.querySelectorAll('[data-learn]').forEach(b=>b.onclick=()=>learnSkill(b.dataset.learn));
 el.querySelectorAll('[data-skill-upgrade]').forEach(b=>b.onclick=()=>upgradeSkill(b.dataset.skillUpgrade));
...
function renderCharacter(el){
 const p=state.player,c=CLASSES[p.class],slots=[['helmet','Hełm','⛑️'],['amulet','Amulet','📿'],['weapon','Broń główna','⚔️'],['armor','Pancerz','🛡️'],['offhand',p.class==='berserker'?'Druga broń':['hunter','ranger'].includes(p.class)?'Strzały':'Druga ręka',p.class==='berserker'?'🪓':['hunter','ranger'].includes(p.class)?'➶':'🛡️'],['gloves','Rękawice','🧤'],['ring1','Pierścień I','💍'],['ring2','Pierścień II','💍'],['boots','Buty','🥾']];
...
function equipmentSlotHTML(slot,label,ico){
 const i=equippedInstance(slot),d=i?itemDef(i.id):null,ammoQty=d?.type==='ammo'?countItem(d.id):null;
 return `<button class="gear-slot dnd-equip-slot ${i?`rarity-border-${d.rarity}`:'empty'}" data-equip-drop="${slot}" data-slot="${slot}" title="${i?itemName(i):`Przeciągnij tutaj: ${label}`}"><span>${d?itemIconVisual(d.id,'gear-item-svg'):ico}</span><b>${label}</b><small>${d?`${itemName(i)}${ammoQty!==null?` • ${ammoQty} szt.`:''}`:'pusty'}</small>${i?'<em class="equipped-dot">ZAŁOŻONE</em>':''}</button>`;
}
function skillCard(s){
 const p=state.player,learned=p.skills.includes(s.id),level=learned?skillLevel(s.id):0,active=learned&&ensureCombatSkillLoadout().includes(s.id),reqSkill=s.requires,canLevel=p.level>=s.req,canPrev=!reqSkill||p.skills.includes(reqSkill),can=canLevel&&canPrev&&p.skillPoints>=s.cost;
 const current=learned?effectiveSkill(s,level):s,nextRank=Math.min(SKILL_MAX_LEVEL,level+1),next=learned&&level<SKILL_MAX_LEVEL?effectiveSkill(s,nextRank):null,nextReq=learned?skillRankRequiredLevel(s,nextRank):s.req,nextCost=learned?skillRankCost(s,nextRank):s.cost,nextReady=learned&&level<SKILL_MAX_LEVEL&&p.level>=nextReq&&p.skillPoints>=nextCost;
 const pips=Array.from({length:SKILL_MAX_LEVEL},(_,i)=>`<i class="${i<level?'filled':''}"></i>`).join('');
 return `<div class="skill-node ${learned?'learned':!can?'locked':''} ${active?'active-loadout':''}"><div class="skill-orb">${skillIconVisual(s.id,'skill-node-svg')}<span class="skill-rank-badge">${level||'—'}/${SKILL_MAX_LEVEL}</span></div><div class="skill-copy"><b>${s.name}</b><div class="skill-rank-pips">${pips}</div><div class="tiny">Odblokowanie: lvl ${s.req} • ${s.cost} pkt • mana ${s.mana}${s.cooldown?` • CD ${s.cooldown}`:''}</div><p>${s.desc}</p>${learned?`<div class="skill-current-effect"><strong>Poziom ${level}:</strong> ${skillEffectSummary(current)}</div>`:''}${next?`<div class="skill-next-effect"><strong>Poziom ${nextRank}:</strong> ${skillEffectSummary(next)}<br><span>Wymaga lvl ${nextReq} • ${nextCost} pkt</span></div>`:''}${reqSkill&&!canPrev?`<div class="tiny danger-text">Wymaga: ${skillDef(reqSkill)?.name||reqSkill}</div>`:''}</div>${learned?`<div class="skill-node-actions"><span class="pill ${level>=SKILL_MAX_LEVEL?'gold':'green'}">LVL ${level}/${SKILL_MAX_LEVEL}</span>${level<SKILL_MAX_LEVEL?`<button class="primary mini skill-upgrade-btn" data-skill-upgrade="${s.id}" ${nextReady?'':'disabled'}>Ulepsz → ${nextRank}</button>`:'<span class="skill-maxed">MAX</span>'}<button class="secondary mini" data-active-skill="${s.id}">${active?'Usuń z paska':'Dodaj do walki'}</button></div>`:`<button class="secondary" data-learn="${s.id}" ${can?'':'disabled'}>Odblokuj</button>`}</div>`
}
function learnSkill(id){const p=state.player,s=(SKILLS[p.class]||[]).find(x=>x.id===id);if(!s||p.skills.includes(id))return;if(p.level<s.req)return toast(`Wymagany poziom ${s.req}.`);if(s.requires&&!p.skills.includes(s.requires))return toast(`Najpierw odblokuj: ${skillDef(s.requires)?.name||s.requires}.`);if(p.skillPoints<s.cost)return toast('Za mało punktów umiejętności.');p.skillPoints-=s.cost;p.skills.push(id);ensureSkillLevels(p);p.skillLevels[id]=1;const active=ensureCombatSkillLoadout();if(active.length<MAX_ACTIVE_SKILLS&&!active.includes(id))p.activeSkills=[...active,id];tutorialEvent('skill');save();refresh();toast(`Odblokowano: ${s.name} • poziom 1${p.activeSkills.includes(id)?' • dodano do paska walki':''}`)}
function upgradeSkill(id){
 const p=state.player,s=skillDef(id);if(!s||!p.skills.includes(id))return toast('Najpierw odblokuj tę umiejętność.');ensureSkillLevels(p);const current=skillLevel(id);if(current>=SKILL_MAX_LEVEL)return toast(`${s.name} ma już maksymalny poziom.`);const next=current+1,req=skillRankRequiredLevel(s,next),cost=skillRankCost(s,next);if(p.level<req)return toast(`Poziom ${next} umiejętności wymaga poziomu postaci ${req}.`);if(p.skillPoints<cost)return toast(`Potrzebujesz ${cost} pkt umiejętności.`);p.skillPoints-=cost;p.skillLevels[id]=next;playSfx('level');haptic([12,20,12]);save();refresh();toast(`⬆️ ${s.name}: poziom ${next}/${SKILL_MAX_LEVEL} • ${skillEffectSummary(effectiveSkill(s,next))}`)
}
...
function itemCanGoToSlot(i,slot){
 if(!i)return false;const d=itemDef(i.id);if(!itemClassAllowed(d))return false;
 if(d.type==='ammo')return ['hunter','ranger'].includes(state.player.class)&&slot==='offhand'&&equippedBowReady();
...
function equipIndexToSlot(idx,slot){
 ...
 if(d.type==='ammo'){
  if(!['hunter','ranger'].includes(state.player.class)||slot!=='offhand')return false;
  if(!equippedBowReady()){toast('Najpierw załóż łuk do głównej ręki.');return false}
  state.player.equipped.offhand={id:i.id,ammo:true};tutorialEvent('equip');playSfx('equip');haptic(12);save();toast(`➶ Założono do drugiej ręki: ${d.name} • ${countItem(d.id)} szt. • +${d.damage?.[0]||0}–${d.damage?.[1]||0} obr.`);return true;
 }
...
function renderInventory(el){
 ...
 const p=state.player,slots=[['helmet','Hełm','⛑️'],['amulet','Amulet','📿'],['weapon','Broń główna','⚔️'],['armor','Pancerz','🛡️'],['offhand',p.class==='berserker'?'Druga broń':['hunter','ranger'].includes(p.class)?'Strzały':'Druga ręka',p.class==='berserker'?'🪓':['hunter','ranger'].includes(p.class)?'➶':'🛡️'],['gloves','Rękawice','🧤'],['ring1','Pierścień I','💍'],['ring2','Pierścień II','💍'],['boots','Buty','🥾']],used=inventoryUsedSlots(),cap=inventoryCapacity(),bag=backpackEntries(),bySlot=new Map(bag.map(e=>[e.slot,e]));
...
function slotLabel(slot){return ({weapon:'Broń główna',helmet:'Hełm',armor:'Pancerz',gloves:'Rękawice',boots:'Buty',amulet:'Amulet',ring1:'Pierścień I',ring2:'Pierścień II',offhand:state?.player?.class==='berserker'?'Druga broń':['hunter','ranger'].includes(state?.player?.class)?'Strzały':'Druga ręka',ring:'Pierścień'})[slot]||slot}
...
function inventoryCard(i,idx){const d=itemDef(i.id),eqSlot=Object.entries(state.player.equipped).find(([,x])=>x?.uid&&x.uid===i.uid)?.[0],eq=!!eqSlot,qty=i.qty||1,up=i.upgrade||0,details=[d.damage?`Obrażenia ${d.damage[0]}–${d.damage[1]}`:'',d.power&&!d.damage?`ATK +${d.power+up*2}`:'',d.armor?`Pancerz +${d.armor+up*2}`:'',d.crit?`Kryt +${d.crit+up}%`:'',d.weaponKind==='bow'?'Amunicja: strzały':d.ammo?`Amunicja: ${itemDef(d.ammo).name}`:'',itemCombatPerkText(d.id)?`◆ ${itemCombatPerkText(d.id)}`:''].filter(Boolean).join(' • '),mods=[i.affix?.name?`✨ Afiks: ${i.affix.name}`:'',i.enchant?.name?`🔮 Enchant: ${i.enchant.name}`:'',i.rune?`${itemDef(i.rune).icon} ${itemDef(i.rune).name}`:'',d.set?`⚜️ ${SET_BONUSES[d.set]?.name||d.set}`:''].filter(Boolean);return `<div class="item-card rarity-card-${d.rarity}"><div class="item-top"><div class="item-icon">${itemIconVisual(d.id,'inventory-item-svg')}</div><div><b class="rarity-${d.rarity}">${itemName(i)}${qty>1?` ×${qty}`:''}</b><div class="tiny">${rarityName(d.rarity)}${d.reqLevel?` • wymagany lvl ${d.reqLevel}`:''} • ${d.slot?slotLabel(d.slot):d.type}${details?` • ${details}`:''}${d.classes?`<br>Klasa: ${itemClassNames(d)}`:''}</div>${mods.length?`<div class="item-mods">${mods.map(x=>`<span>${x}</span>`).join('')}</div>`:''}</div></div>${eq?`<div class="equipped-tag">ZAŁOŻONE: ${slotLabel(eqSlot)}</div>`:d.type==='ammo'&&equippedAmmoMatches(d.id)?`<div class="equipped-tag">ZAŁOŻONE: STRZAŁY W DRUGIEJ RĘCE</div>`:''}<div class="tabs" style="margin-top:8px">${d.slot?(eq?`<button class="secondary" data-unequip="${i.uid}">Zdejmij</button>`:inventoryEquipButtons(d,idx)):''}${d.type==='consumable'?`<button class="secondary" data-use="${d.id}">Użyj</button>`:''}${d.type==='ammo'&&['hunter','ranger'].includes(state.player.class)?(equippedAmmoMatches(d.id)?`<button class="secondary" data-unequip-ammo>Zdejmij strzały</button>`:`<button class="secondary" data-equip-slot="offhand">➶ Załóż do drugiej ręki</button>`):''}${d.slot&&!eq?`<button class="ghost" data-salvage="${idx}">♻️ Rozbierz</button>`:''}</div>${d.value>0&&!eq?`<div class="inventory-sell-note">🪙 Sprzedaż tego przedmiotu jest dostępna u kupca w mieście.</div>`:''}</div>`}
function rarityName(r){return ({common:'Zwykły',uncommon:'Niezwykły',rare:'Rzadki',epic:'Epicki',unique:'Unikatowy',heroic:'Heroiczny',legendary:'Legendarny'})[r]||r}
...
function sellIndex(idx,qty=1,rerender=true){const i=state.player.inventory[idx];if(!i)return null;if(inventoryItemEquippedSlot(i)){toast('Najpierw zdejmij przedmiot.');return null}const d=itemDef(i.id),available=i.qty||1,amount=isStackable(i.id)?Math.max(1,Math.min(available,Number(qty)||1)):1,price=itemSellValue(i)*amount;state.player.gold+=price;if(available>amount)i.qty=available-amount;else state.player.inventory.splice(idx,1);if(state.player.equipped?.offhand?.ammo&&state.player.equipped.offhand.id===d.id&&countItem(d.id)<=0)state.player.equipped.offhand=null;ensureEconomyState();state.economy.totalSold+=amount;save();if(rerender)refresh();toast(`🪙 Sprzedano: ${d.name}${amount>1?` ×${amount}`:''} • +${price} 🪙`);return {id:d.id,name:d.name,amount,price}}
...
function bountyTargetName(b){return ['gather','loot'].includes(b.type)?itemDef(b.target).name:(MONSTERS.find(m=>m.id===b.target)?.name||b.target)}
function bountyVerb(b){return b.type==='gather'?'Zbierz':b.type==='loot'?'Zdobądź':'Pokonaj'}
function bountyObjective(b){return `${bountyVerb(b)} ${b.need}× ${bountyTargetName(b)}`}
function bountySourceHint(b){if(b?.type!=='loot'||!b.source)return '';const m=MONSTERS.find(x=>x.id===b.source),chance=Math.round((b.dropChance||monsterLootTable(b.source).find(x=>x.id===b.target)?.chance||0)*100);return m?`${itemDef(b.target).name} wypada z: ${m.name}${chance?` • ok. ${chance}% szansy`:''}.`:''}
function tavernLootHintsHTML(){ensureAdventureState();const rows=(state.adventure.bounties||[]).filter(b=>b.accepted&&!b.claimed&&b.type==='loot');if(!rows.length)return '';return `<div class="tavern-loot-hints"><b>🧭 Wskazówki Doriana</b>${rows.map(b=>`<div><span>${b.icon||'🎒'}</span><p><strong>${bountyTargetName(b)}</strong><small>${bountySourceHint(b)}</small></p></div>`).join('')}</div>`}
...
function tavernBoardHTML(){
 ensureAdventureState();const bounties=state.adventure.bounties||[],active=activeTaskCount(),lv=state.player.level;
 return `<div class="building-panel parchment-panel"><button class="ghost panel-back" data-building-home>← Wróć do karczmy</button><div class="board-head"><div><span>TABLICA OGŁOSZEŃ</span><h2>📌 Zlecenia dla poziomu ${lv}</h2></div><b>${active}/${activeTaskLimit()} aktywne</b></div><div class="quest-board">${bounties.map(b=>{const complete=(b.progress||0)>=b.need;return `<article class="quest-paper contract-paper ${b.accepted?'accepted-paper':''}"><i></i><span>${b.type==='loot'?'ZLECENIE NA ŁUP':'KONTRAKT'} • lvl ${b.boardLevel||lv}</span><h3>${b.icon} ${b.name}</h3><p>${bountyObjective(b)}.${b.type==='loot'&&!b.accepted?' Dorian może podpowiedzieć, z jakiego potwora zdobyć ten łup.':' Cel pojawi się na mapie po przyjęciu.'}</p><strong>${b.xp} XP • ${b.gold} 🪙 • ${b.rep} rep.</strong>${b.accepted?(complete?`<button class="secondary" data-claim-bounty="${b.id}">✓ Odbierz nagrodę</button>`:`<button disabled>Przyjęte • ${b.progress||0}/${b.need}</button>`):`<button class="secondary" data-accept-bounty="${b.id}" ${canAcceptTask()?'':'disabled'}>${canAcceptTask()?'Przyjmij':`Limit ${activeTaskLimit()}/${activeTaskLimit()}`}</button>`}</article>`}).join('')}</div><div class="quest-board-foot"><b>Główny wątek fabularny jest w zakładce „Zadania”.</b> Zlecenia z tej tablicy skalują się z Twoim poziomem, a po wykonaniu na miejsce starego pojawia się nowe.</div></div>`;
}
function tavernKeeperHTML(){ensureStaminaCap();const p=state.player,daily=ensureTavernDaily(),st=p.stamina??60,max=p.maxStamina??60,limit=daily.mealUses>=2;return `<div class="building-panel"><button class="ghost panel-back" data-building-home>← Wróć do sali</button>${npcCard('Dorian','Karczmarz • były wojownik','knight','Dorian walczył kiedyś na północy. Zna nawyki potworów, a dziś pilnuje, żeby podróżni wracali na szlak w jednym kawałku.')}<div class="dialogue-bubble">${tavernAnecdote()}</div>${tavernLootHintsHTML()}<div class="stamina-card"><b>⚡ Stamina ${st}/${max}</b><div class="mini-progress"><span style="width:${st/max*100}%"></span></div><small>Posiłki i napitki: ${daily.mealUses}/2 dzisiaj. Każda walka zużywa 3 staminy.</small></div><div class="tavern-menu"><button class="secondary" data-anecdote>🗣️ Kolejna anegdota</button><button class="secondary" data-stamina="10" data-cost="10" data-label="Piwo" ${limit?'disabled':''}>🍺 Piwo • +10 staminy • 10 🪙</button><button class="secondary" data-stamina="25" data-cost="25" data-label="Solidny posiłek" ${limit?'disabled':''}>🍲 Posiłek • +25 • 25 🪙</button><button class="secondary" data-stamina="50" data-cost="50" data-label="Karczemna uczta" ${limit?'disabled':''}>🍗 Uczta • +50 • 50 🪙</button></div></div>`}
...
function itemCombatPerk(id){return ITEM_COMBAT_PERKS[id]||null}
function itemCombatPerkText(id){return itemCombatPerk(id)?.label||''}
function equippedCombatPerks(){const ids=Object.values(state.player.equipped||{}).map(x=>x?.id).filter(Boolean),out={farDamage:0,approachDamage:0,farMana:0,closeGuard:0};for(const id of ids){const p=itemCombatPerk(id);if(!p)continue;for(const k of Object.keys(out))out[k]=Math.max(out[k],Number(p[k]||0))}return out}
...
function combatGearDamageMultiplier(){const perk=equippedCombatPerks();let m=1;if(combatDistance()===2&&combatIsRangedClass()&&perk.farDamage)m*=1+perk.farDamage;if(combatIsMeleeClass()&&combat.approachBuffTurns>0&&perk.approachDamage)m*=1+perk.approachDamage;return m}
function combatIncomingGearMultiplier(){const perk=equippedCombatPerks();return combatDistance()===0&&perk.closeGuard?1-perk.closeGuard:1}
function itemStatsHTML(d){const stats=[];if(d.damage)stats.push(`⚔️ ${d.damage[0]}–${d.damage[1]}`);if(d.armor!==undefined)stats.push(`🛡️ ${d.armor}`);if(d.power)stats.push(`✦ +${d.power} mocy`);if(d.crit)stats.push(`🎯 +${d.crit}%`);if(d.reqLevel)stats.push(`lvl ${d.reqLevel}`);const perk=itemCombatPerkText(d.id);if(perk)stats.push(`◆ ${perk}`);if(!stats.length)stats.push(d.type==='material'?'Materiał rzemieślniczy':d.type==='ammo'?'Amunicja':'Przedmiot użytkowy');return stats.map(x=>`<span>${x}</span>`).join('')}
...
function ensureShopUiState(){state.ui=state.ui||{};state.ui.shopMode='buy'}
function currentShopStockIds(){ensureShopUiState();const ranged=['hunter','ranger'].includes(state.player.class);return Object.values(ITEMS).filter(d=>((d.slot&&d.rarity==='common'&&(d.value||0)>5&&(!d.reqLevel||d.reqLevel<=state.player.level+6))||(ranged&&d.type==='ammo'&&d.rarity==='common'&&(!d.reqLevel||d.reqLevel<=state.player.level)))&&itemClassAllowed(d)).sort((a,b)=>(a.type==='ammo')-(b.type==='ammo')||(a.reqLevel||1)-(b.reqLevel||1)||(a.value||0)-(b.value||0)).map(d=>d.id)}
function shopSelectionState(){ensureShopUiState();const stock=currentShopStockIds(),sellEntries=backpackEntries().filter(({item})=>itemDef(item.id).value>0);if(!stock.length)state.ui.shopSelectedBuy=null;else if(!state.ui.shopSelectedBuy||!stock.includes(state.ui.shopSelectedBuy))state.ui.shopSelectedBuy=stock[0];const sellIndices=sellEntries.map(e=>e.index);if(!sellIndices.length)state.ui.shopSelectedSell=null;else if(state.ui.shopSelectedSell==null||!sellIndices.includes(state.ui.shopSelectedSell))state.ui.shopSelectedSell=sellIndices[0];return {stock,sellEntries}}
function shopPageLabel(){return 'Zwykły sprzęt'}

function shopStockGridHTML(items){if(!items.length)return '<div class="retro-grid-empty">Brak towaru w tej kategorii.</div>';return items.map(id=>{const d=itemDef(id),selected=state.ui.shopMode!=='sell'&&state.ui.shopSelectedBuy===id,qty=arrowPackQty(id),price=shopPurchasePrice(id);return `<button class="retro-shop-slot ${selected?'selected':''} rarity-frame-${d.rarity}" data-shop-select-buy="${id}" data-price="${price}" title="${d.name}${qty>1?` ×${qty}`:''} • ${price} 🪙"><span class="retro-shop-slot-icon">${itemIconVisual(d.id,'shop-item-svg')}</span></button>`}).join('')}
function retroSelectedBuyPanel(){const id=state.ui.shopSelectedBuy;if(!id)return `<div class="retro-tray muted-slot">Brak wyboru</div>`;const d=itemDef(id),qty=arrowPackQty(id),price=shopPurchasePrice(id),room=canReceiveItems([{id,qty}]),ok=state.player.gold>=price&&room;return `<div class="retro-tray filled ${ok?'ready':''}"><div class="retro-tray-icon">${itemIconVisual(d.id,'shop-item-svg')}</div><div><b>${d.name}${qty>1?` ×${qty}`:''}</b><small>${d.type==='ammo'?`+${d.damage[0]}–${d.damage[1]} obrażeń • `:''}${rarityName(d.rarity)} • ${price} 🪙</small></div></div>`}
...
function retroShopSummaryHTML(stock,sellEntries){const mode=state.ui.shopMode||'buy';if(mode==='sell'&&state.ui.shopSelectedSell!=null){const row=sellEntries.find(e=>e.index===state.ui.shopSelectedSell);if(row){const item=row.item,d=itemDef(item.id),qty=item.qty||1,unit=itemSellValue(item);return `<div class="retro-summary-copy"><b>Sprzedaż</b><p>${itemName(item)}${qty>1?` ×${qty}`:''}</p><small>${rarityName(d.rarity)} • ${d.slot?slotLabel(d.slot):d.type}</small></div><div class="retro-summary-stats"><span>Prof: —</span><span>LVL: ${d.reqLevel||1}</span><span>Cena: ${unit} 🪙${qty>1?` / szt.`:''}</span></div><div class="retro-summary-totals"><em class="minus">0</em><em class="plus">+${unit}</em><em class="net">${qty>1?`Stos: ${unit*qty} 🪙`:''}</em></div>`}}const id=state.ui.shopSelectedBuy||stock[0];if(!id)return `<div class="retro-summary-copy"><b>Sklep</b><p>Brak towaru.</p></div>`;const d=itemDef(id),qty=arrowPackQty(id),price=shopPurchasePrice(id);return `<div class="retro-summary-copy"><b>Kupno</b><p>${d.name}${qty>1?` ×${qty}`:''}</p><small>${itemStatsHTML(d).replace(/<span>|<\/span>/g,'').replace(/<[^>]+>/g,' • ')}</small></div><div class="retro-summary-stats"><span>Prof: ${d.classes?itemClassNames(d):'Wsz.'}</span><span>LVL: ${d.reqLevel||1}</span><span>Cena: ${price} 🪙</span></div><div class="retro-summary-totals"><em class="minus">-${price}</em><em class="plus">0</em><em class="net">${state.player.gold>=price?'Gotowe do zakupu':'Za mało złota'}</em></div>`}
function retroShopButtonsHTML(sellEntries,stock){if((state.ui.shopMode||'buy')==='sell'&&state.ui.shopSelectedSell!=null){const row=sellEntries.find(e=>e.index===state.ui.shopSelectedSell);if(row){const item=row.item,qty=item.qty||1;return `<button class="primary retro-accept" data-shop-accept>${qty>1?'Sprzedaj 1':'Sprzedaj'}</button>${qty>1?`<button class="secondary retro-accept-all" data-shop-accept-all>Sprzedaj cały stos</button>`:''}<button class="danger retro-exit" data-building-home>Wyjdź</button>`}}const id=state.ui.shopSelectedBuy||stock[0];if(!id)return `<button class="primary retro-accept" disabled>Akceptuj</button><button class="danger retro-exit" data-building-home>Wyjdź</button>`;const qty=arrowPackQty(id),price=shopPurchasePrice(id),room=canReceiveItems([{id,qty}]),ok=state.player.gold>=price&&room;return `<button class="primary retro-accept" data-shop-accept ${ok?'':'disabled'}>${!room?'Pełny plecak':state.player.gold>=price?'Akceptuj':'Za mało złota'}</button><button class="danger retro-exit" data-building-home>Wyjdź</button>`}
...
function shopHTML(){const {stock,sellEntries}=shopSelectionState();return `<div class="retro-shop-shell"><div class="retro-shop-banner"><div class="retro-shop-title">Sklep</div><button class="close retro-shop-close" data-building-home title="Wyjdź">×</button></div><div class="retro-shop-awning" aria-hidden="true"></div><div class="retro-shop-body shop-basic-body"><section class="retro-shop-left shop-basic-buy"><div class="shop-basic-head"><b>⚔️ Zwykły sprzęt${['hunter','ranger'].includes(state.player.class)?' i strzały':''}</b><small>Selma sprzedaje podstawowe wyposażenie dla Twojej klasy${['hunter','ranger'].includes(state.player.class)?' oraz strzały w paczkach po 100 sztuk':''}. Epickie, unikatowe, heroiczne i legendarne przedmioty zdobywa się w walce.</small></div><div class="retro-shop-grid shop-basic-grid">${shopStockGridHTML(stock)}</div><section class="retro-shop-pane shop-buy-preview"><h4>Kup</h4>${retroSelectedBuyPanel()}</section><div class="retro-shop-summary compact-buy-summary">${retroShopSummaryHTML(stock,sellEntries)}</div><div class="retro-shop-actions shop-buy-actions">${retroShopButtonsHTML(sellEntries,stock)}</div></section><aside class="retro-shop-right shop-basic-sell"><section class="retro-shop-pane"><h4>Sprzedaj z plecaka</h4><small class="shop-sell-hint">Kliknij swój przedmiot — pojawi się szybka opcja sprzedaży bez przewijania strony.</small><div class="retro-sell-grid">${retroSellGridHTML(sellEntries)}</div></section><div class="shop-quality-rules"><b>Skąd bierze się lepszy sprzęt?</b><span>⭐ Elity: epicki / unikatowy</span><span>👑 Herosi: unikatowy / heroiczny</span><span>🌍 Legendarni bossowie: unikatowy / heroiczny / legendarny</span></div></aside></div><div class="shop-sell-sheet" data-shop-sell-sheet aria-live="polite"></div></div>`}
function smithGearCardHTML(i,runes){const d=itemDef(i.id),up=i.upgrade||0,q=upgradeQuote(i);return `<article class="workshop-card rarity-frame-${d.rarity}"><div class="workshop-item"><div class="market-product-icon">${itemIconVisual(d.id,'shop-item-svg')}</div><div><span class="rarity-${d.rarity}">${rarityName(d.rarity)}</span><h3>${itemName(i)}</h3><div class="market-stats">${itemStatsHTML(d)}</div></div></div><div class="upgrade-track" aria-label="Poziom ulepszenia">${[1,2,3].map(n=>`<i class="${n<=up?'lit':''}"></i>`).join('')}<b>+${up}</b></div><div class="socket-line"><span>${i.rune?`${itemDef(i.rune).icon} ${itemDef(i.rune).name}`:'◇ Wolne gniazdo runy'}</span><span>⚒️ Maks. ulepszenie +3</span></div><div class="workshop-actions smith-simple-actions"><button class="secondary" data-upgrade="${i.uid}" ${up>=3?'disabled':''}>${up>=3?'Ulepszenie MAX':`⚒️ +${up+1} • ${q.gold}🪙${q.scrap?` + ${q.scrap}🔩`:''}`}</button>${!i.rune?runes.map(r=>`<button class="ghost rune-button" data-socket="${i.uid}" data-rune="${r}" ${countItem(r)?'':'disabled'}>${itemDef(r).icon} ${itemDef(r).name.replace('Runa ','')} (${countItem(r)})</button>`).join(''):`<button class="ghost" data-unsocket="${i.uid}">↩️ Wyjmij runę</button>`}</div></article>`}
function smithHTML(){const gear=state.player.inventory.filter(i=>itemDef(i.id).slot),recipes=RECIPES.filter(r=>{if(r.station!=='smith')return false;const d=itemDef(r.result);return itemClassAllowed(d)&&(d.type==='rune'||d.type==='ammo'||(!!d.slot&&['common','uncommon','rare'].includes(d.rarity)))}),runes=['runePower','runeGuard','runePrecision'];return `${serviceHeroHTML('smith','KUŹNIA I RUNY','Warsztat Ragora','Kuźnia lekko wzmacnia znaleziony sprzęt, pozwala osadzać runy i tworzyć podstawowe wyposażenie. Najlepszy sprzęt zdobywa się z elit, herosów i legend.')}<div class="resource-belt"><span>🔩 Złom <b>${countItem('scrap')}</b></span><span>💎 Kryształ <b>${countItem('crystal')}</b></span><span>🔹 Odłamki <b>${countItem('runeShard')}</b></span>${runes.map(id=>`<span>${itemDef(id).icon} ${itemDef(id).name.replace('Runa ','')} <b>${countItem(id)}</b></span>`).join('')}</div>${sectionTitleHTML('🔥','Podrasowanie sprzętu',gear.length?'Maksymalnie +3 oraz jedno gniazdo runy':'Nie masz sprzętu do obróbki')}<div class="workshop-grid">${gear.map(i=>smithGearCardHTML(i,runes)).join('')||'<div class="service-empty">Przynieś Ragorowi broń albo pancerz znaleziony na szlaku.</div>'}</div>${sectionTitleHTML('🧰','Proste receptury','Kuźnia nie tworzy przedmiotów epickich, unikatowych, heroicznych ani legendarnych')}<div class="recipe-grid">${recipes.map(r=>recipeCardHTML(r,'Wykuj')).join('')}</div>`}
...
function auctionListingFee(price,qty=1){return Math.max(1,Math.ceil(Math.max(1,price)*Math.max(1,qty)*.02))}
function auctionSalePayout(listing){return Math.max(1,Math.floor(listing.unitPrice*listing.qty*.95))}
...
function auctionListingFormHTML(){
 ...
 const item=row.item,d=itemDef(item.id),max=item.qty||1,suggested=auctionSuggestedUnitPrice(item),note=`Cena za sztukę • 2% opłaty • 5% prowizji • ${activeCount}/${auctionListingLimit()} aktywnych`;
...
function auctionHTML(){ensureEconomyState();ensureAuctionUiState();processAuctionListings();return `${serviceHeroHTML('auction','DOM AUKCYJNY','Rynek Vara','Wystawiaj własne przedmioty i odbieraj zakończone aukcje. Dom aukcyjny nie generuje rzadkiego sprzętu — najlepsze przedmioty pochodzą z walki.')}<div class="auction-ribbon"><span>🔨 DOM AUKCYJNY</span><b>Własne aukcje</b><small>Wystawienie 2% • prowizja 5% • limit ${auctionListingLimit()}</small></div>${auctionListingFormHTML()}${ownAuctionListingsHTML()}<div class="auction-drop-note"><b>⚔️ Najlepszy sprzęt zdobywasz w świecie</b><span>Elity → epickie i unikatowe</span><span>Herosi → unikatowe i heroiczne</span><span>Legendarni bossowie → unikatowe, heroiczne i legendarne</span></div>`}
...
function bindBuilding(id,view){
 ...
 document.querySelectorAll('[data-claim-bounty]').forEach(b=>b.onclick=()=>{if(claimBounty(b.dataset.claimBounty))openBuilding('tavern','board')});
 ...
 document.querySelectorAll('[data-shop-select-sell]').forEach(b=>b.onclick=()=>{ensureShopUiState();state.ui.shopSelectedSell=Number(b.dataset.shopSelectSell);save();openShopSellSheet(Number(b.dataset.shopSelectSell))});
 ...
 document.querySelector('[data-shop-accept]')?.addEventListener('click',()=>{ensureShopUiState();if(state.ui.shopMode==='sell'&&state.ui.shopSelectedSell!=null){sellAtShop(Number(state.ui.shopSelectedSell),1);return}const id=state.ui.shopSelectedBuy;if(id)buyItem(id,'shop',shopPurchasePrice(id))});
...
function buyItem(id,building,forcedPrice){if(!ITEMS[id])return;const d=itemDef(id),isShopAmmo=building==='shop'&&d.type==='ammo'&&['hunter','ranger'].includes(state.player.class);if(building==='shop'&&!(d.slot&&d.rarity==='common')&&!isShopAmmo)return toast('Kupiec sprzedaje wyłącznie zwykły sprzęt i strzały dla klas dystansowych.');const qty=isShopAmmo?arrowPackQty(id):1;if(!requireItemRoom([{id,qty}]))return;const price=Number.isFinite(forcedPrice)&&forcedPrice>0?forcedPrice:(building==='shop'?shopPurchasePrice(id):merchantBuyPrice(id));if(state.player.gold<price)return toast('Za mało złota.');state.player.gold-=price;addItem(id,qty);save();openBuilding(building,'service');toast(`🛒 Kupiono: ${d.name}${qty>1?` ×${qty}`:''} • -${price} 🪙`)}
function upgradeItem(uidv){const i=state.player.inventory.find(x=>x.uid===uidv);if(!i)return;const up=i.upgrade||0;if(up>=3)return;const q=upgradeQuote(i);if(state.player.gold<q.gold)return toast(`Potrzebujesz ${q.gold} złota.`);if(countItem('scrap')<q.scrap)return toast(`Potrzebujesz ${q.scrap}× Żelazny złom.`);state.player.gold-=q.gold;if(q.scrap)removeItem('scrap',q.scrap);i.upgrade=up+1;save();openBuilding('smith','service');toast(`${itemDef(i.id).name} ulepszono do +${i.upgrade} • -${q.gold} 🪙`)}
...
function combatSkillMeta(skill){if(!skill)return {type:'fizyczne',stagger:10};const base=SKILL_COMBAT_META[skill.id]||{type:'fizyczne',stagger:18};return {...base,stagger:(base.stagger||18)+Math.max(0,(skill._level||skillLevel(skill.id)||1)-1)*3}}
...
function enemyRangedAttack(mult=.88,label='ATAK DYSTANSOWY'){const dodge=Math.random()*100<dodgeChance();if(dodge){combat.lastEnemyHit=null;setCombatTaken(0,'UNIK');logCombat(`💨 Unikasz ataku dystansowego: ${combat.monster.name}.`);return 0}const blocked=Math.random()*100<blockChance(),reduction=armorPower()*.30,guardMult=combat.guard>0?(combat.guardStrength?Math.min(.62,1-combat.guardStrength):.62):1,blockMult=blocked?.58:1,gearMult=combatIncomingGearMultiplier(),dmg=Math.max(1,Math.floor((combat.atk*mult-reduction)*guardMult*blockMult*gearMult));state.player.hp=Math.max(0,state.player.hp-dmg);combat.lastEnemyHit=dmg;setCombatTaken(dmg,blocked?'BLOK':label);logCombat(`${blocked?'🛡️ Blok! ':''}${combat.monster.name} atakuje z dystansu za ${dmg}.`);if(combat.guard>0)combat.guard--;return dmg}
...
function combatMove(delta){
 ...
 logCombat(after<before?`👣 Podchodzisz do przeciwnika. Dystans: ${combatDistanceLabel()}.`:`↩️ Cofasz się. Dystans: ${combatDistanceLabel()}.`);haptic(10);queueEnemyTurn(700);
}
function combatCommandMenuHTML(c,p,combatSkills,potionIds,potionTotal,locked){
 ...
 if(menu==='fight'){
  const selectedAmmo=availableArrowId(),basicBlocked=locked||combatNeedsClose(null)||(['hunter','ranger'].includes(p.class)&&!selectedAmmo);
  return `<div class="battle-command-panel"><div class="command-panel-head"><div><span>⚔️ WALKA</span><small>${rangeHint}</small></div>${back}</div><div class="skill-hotbar command-fight-grid"><button class="battle-skill basic" data-attack ${basicBlocked?'disabled':''}><span>${['hunter','ranger'].includes(p.class)?'🏹':p.class==='mage'?'🔮':'⚔️'}</span><b>Atak</b><small>${combatNeedsClose(null)?'Podejdź na BLISKO':['hunter','ranger'].includes(p.class)?(selectedAmmo?`➶ ${itemDef(selectedAmmo).name} • ${countItem(selectedAmmo)} szt. • +${itemDef(selectedAmmo).damage[0]}–${itemDef(selectedAmmo).damage[1]}`:'Załóż strzały w drugiej ręce'):'+10 przeł.'}</small></button><button class="battle-skill defend-skill" data-defend ${locked?'disabled':''}><span>🛡️</span><b>Obrona</b><small>na 1 turę</small></button>${combatSkills.map(s=>{const meta=combatSkillMeta(s),cd=c.cooldowns?.[s.id]||0,needsClose=combatNeedsClose(s),manaCost=combatSkillManaCost(s),blocked=locked||needsClose||p.mana<manaCost||cd>0||(['hunter','ranger'].includes(p.class)&&!availableArrowId(rangedAmmoCost(s)));return `<button class="battle-skill ${c.intent&&meta.interrupt?'interrupt-ready':''} ${cd?'skill-cooldown':''}" data-skill="${s.id}" ${blocked?'disabled':''}><span>${skillIconVisual(s.id,'combat-skill-svg')}</span><b>${s.name} <em class="combat-skill-level">Lv.${s._level||skillLevel(s.id)}</em></b><small>${needsClose?'Podejdź na BLISKO':cd?`⏳ CD ${cd}`:`${manaCost} many • ${meta.type}${manaCost<s.mana?' • premia sprzętu':''}${meta.interrupt?' • PRZERWIJ':''}`}</small></button>`}).join('')}</div></div>`;
 }
...
function playerAction(skill){
 if(skill&&skill.id&&!skill._level)skill=effectiveSkill(skill);
 if(!combat||combat.phase!=='player')return;if(combatNeedsClose(skill)){combat.menu='position';openCombat();return toast('Musisz podejść na BLISKO, aby wykonać ten atak.')}resetCombatExchange();const ammoCost=rangedAmmoCost(skill),ammoId=ammoCost?activeArrowId(ammoCost):null,cd=skill?(combat.cooldowns?.[skill.id]||0):0;if(cd)return toast(`Umiejętność gotowa za ${cd} tur.`);const manaCost=combatSkillManaCost(skill);if(skill&&state.player.mana<manaCost)return toast('Za mało many.');if(ammoCost&&!ammoId)return toast(`Brak odpowiedniej liczby strzał. Potrzebujesz ${ammoCost}.`);combat.shotAmmoId=ammoId;if(ammoCost)removeItem(ammoId,ammoCost);if(skill){state.player.mana-=manaCost;if(skill.cooldown)combat.cooldowns[skill.id]=(skill.cooldown||0)+1}const meta=combatSkillMeta(skill);let total=0,hits=1,lastHit=null;
 ...
 else if(skill.kind==='guard'){combat.guard=Math.max(combat.guard,skill.turns);combat.guardStrength=Math.max(combat.guardStrength||0,skill.guardStrength||.45);logCombat(`${skill.name}: redukcja obrażeń ${Math.round((combat.guardStrength||.45)*100)}% przez ${skill.turns} tury.`)}
 else if(skill.kind==='buff'){applyPlayerBuff(skill)}
 else if(skill.kind==='debuff'){combat.debuff=Math.max(combat.debuff,skill.debuff);combat.debuffTurns=skill.debuffTurns||3;logCombat(`${skill.name}: przeciwnik zostaje osłabiony na ${combat.debuffTurns} tury.`)}
 else if(skill.kind==='mark'){combat.mark=skill.markValue||25;combat.markTurns=skill.turns;logCombat(`${skill.name}: oznaczasz cel • +${combat.mark}% krytyka.`)}
 else if(skill.kind==='poison'){const h=hitDamage(skill.mult,0,meta.type);lastHit=h;total=h.dmg;combat.hp-=h.dmg;if(!h.miss){combat.poison=Math.max(4,Math.floor(attackPower()*(state.player.class==='ranger'?.29:.24)*(skill.poisonScale||1)*(combat.environment?.poison||1)));combat.poisonTurns=skill.turns;logCombat(`${skill.name}: ${h.dmg}${hitSuffix(h)} i trucizna.`)}else logCombat(`${skill.name}: pudło.`)}
 ...
 else if(skill.kind==='rage'){const missing=1-state.player.hp/state.player.maxHp,mult=(skill.rageBase||1.2)+missing*(skill.rageMissing||.7),h=hitDamage(mult,0,meta.type);lastHit=h;total=h.dmg;combat.hp-=h.dmg;logCombat(h.miss?`${skill.name}: pudło.`:`${skill.name}: ${h.dmg}${hitSuffix(h)}.`);applySkillStatus(skill,h)}
 ...
 if(combat.approachBuffTurns>0&&combatOffensiveSkill(skill))combat.approachBuffTurns=0;if(skill?.kind!=='pet')petAttack(false);combat.shotAmmoId=null;if(combat.hp<=0)return winCombat();queueEnemyTurn();
}
function defendAction(){if(!combat||combat.phase!=='player')return;resetCombatExchange();combat.turnDealtNote='OBRONA';combat.guard=Math.max(combat.guard,1);combat.guardStrength=0;logCombat('🛡️ Przyjmujesz postawę obronną. Najbliższy cios zada znacznie mniej obrażeń.');haptic(12);queueEnemyTurn()}
...
function resolveBossIntent(){
 ...
 if(intent.type==='smash'){
  combat.fx='boss';const reduction=armorPower()*.28,guardMult=combat.guard>0?(combat.guardStrength?Math.min(.28,1-combat.guardStrength):.28):1,blocked=Math.random()*100<blockChance(),blockMult=blocked?.55:1,dmg=Math.max(1,Math.floor((combat.atk*2.25-reduction)*guardMult*blockMult));state.player.hp=Math.max(0,state.player.hp-dmg);combat.lastEnemyHit=dmg;setCombatTaken(dmg,blocked||combat.guard>0?'BLOK / OBRONA':info.name);logCombat(`${combat.guard>0||blocked?'🛡️ Ograniczasz cios':'💥 Trafia Cię'}: ${info.name} za ${dmg}.`);if(combat.guard>0)combat.guard--;
...
// Mobile 1.10: oszczędzanie baterii w tle + pasywna regeneracja czasu rzeczywistego.
window.addEventListener('pagehide',()=>{if(state)applyPassiveRegen(state,Date.now());save()});
document.addEventListener('visibilitychange',()=>{
 if(document.hidden){if(state)applyPassiveRegen(state,Date.now());save()}
 else if(state){const regen=applyPassiveRegen(state,Date.now());if(regen.changed){save();refreshPassiveResourceHud()}}
...
state=load();restoreSession();if(state){const offlineRegen=applyPassiveRegen(state,Date.now());if(offlineRegen.changed)save()}if(Number.isFinite(state?.ui?.mapHeading))playerMotion.heading=state.ui.mapHeading;
...
render();resumeSession();startPassiveRegenTimer();
