...
const SAVE_KEY='georpg_build_06';
const OLD_SAVE_KEY='georpg_build_05';
const OLDER_SAVE_KEY='georpg_build_04';
const LEGACY_SAVE_KEY='georpg_build_03';
...
function renderMore(el){el.innerHTML=`<div class="section-title"><h2>⚙️ Ustawienia</h2><span class="pill">Build 0.6</span></div><div class="panel-list"><div class="panel-item"><b>💾 Zapis gry</b><div class="muted">Build 0.6 automatycznie migruje zapis 0.5, 0.4, 0.3, 0.2 i 0.1. Zapis działa lokalnie w przeglądarce.</div><div class="tabs" style="margin-top:8px"><button class="secondary" data-export>Eksportuj</button><button class="secondary" data-import>Importuj</button><input type="file" id="saveFile" accept="application/json" hidden></div></div><div class="panel-item"><b>🌙 Symulacja nocy</b><div class="muted">Przydatna do testowania questów zależnych od pory dnia.</div><button class="secondary" data-night>${state.settings.forceNight?'Wyłącz symulację':'Włącz noc'}</button></div><div class="panel-item"><b>🧪 Tryb testowy mapy</b><div class="muted">Strzałki przesuwają bohatera o 20 m bez GPS.</div></div><div class="panel-item"><b>🆕 Co nowego w 0.6</b><div class="muted">Lepsze grafiki plus więcej klimatu: różne tła walki, bogatsze wnętrza miasta i bestiariusz z ilustracjami.</div></div><div class="panel-item"><b>🔄 Reset</b><div class="muted">Usuwa zapis Build 0.6.</div><button class="danger" data-reset>Usuń zapis</button></div></div>`;
...
function exportSave(){const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='georpg-build-0.6-save.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
