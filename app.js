import {CLASSES,MONSTERS,MONSTER_LOOT,ITEMS,QUESTS,SKILLS,PETS,RECIPES,DUNGEONS,BUILDINGS} from './data.js?v=2900';
...
function rollMonsterMaterials(m,e,c){const drops=[];const elite=!!e?.elite,boss=!!c?.isBoss||!!c?.worldBoss;for(const row of monsterLootTable(m.id)){const chance=Math.min(1,row.chance*(elite?1.12:1)*(boss?1.18:1));if(Math.random()>chance)continue;let qty=rnd(row.min||1,row.max||row.min||1);if(elite&&Math.random()<.30)qty++;if(boss&&Math.random()<.45)qty++;const added=addItem(row.id,qty);if(added)drops.push({id:row.id,qty:added})}return drops}
...
function rollCombatLoot(m,e,c){ensureEconomyState();let gearDrop=null;const drops=rollMonsterMaterials(m,e,c);
 if(Math.random()<.018){const added=addItem('crystal');if(added)drops.push({id:'crystal',qty:added})}
 const elite=!!e.elite,boss=!!c.isBoss||!!c.worldBoss;
 if(!elite&&!boss&&Math.random()<.018)gearDrop=randomGearFrom('uncommon');
 if(elite&&!boss){state.economy.elitePity++;if(Math.random()<.18||state.economy.elitePity>=4){gearDrop=randomGearFrom(Math.random()<.22?'epic':'rare');state.economy.elitePity=0}if(Math.random()<.16){const a=addItem('runeShard');if(a)drops.push({id:'runeShard',qty:a})}}
 if(boss){state.economy.bossPity++;const epicGuaranteed=state.economy.bossPity>=3;if(Math.random()<.58||epicGuaranteed){gearDrop=randomGearFrom(Math.random()<.18?'heroic':'epic');state.economy.bossPity=0}else gearDrop=randomGearFrom('rare');if(Math.random()<.55){const q=rnd(1,2),a=addItem('runeShard',q);if(a)drops.push({id:'runeShard',qty:a})}if(Math.random()<.08){const r=pick(['runePower','runeGuard','runePrecision']),a=addItem(r);if(a)drops.push({id:r,qty:a})}}
 if(gearDrop){const a=addItem(gearDrop);if(a)drops.push({id:gearDrop,qty:a,gear:true})}return {gearDrop,drops};
}
...
function exportSave(){const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='time4heroes-build-2.8-save.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
...
function hitDamage(mult=1,critBonus=0,type='fizyczne'){
 const miss=false,crit=Math.random()*100<critChance()+combat.mark+critBonus,weak=weaknessMod(type),weaken=1-(combat.playerWeaken||0),vulnerable=combat.vulnerableTurns?1.35:1,barrier=combat.barrierTurns?.55:1,base=rollPlayerBaseDamage()*mult,dmg=Math.max(1,Math.floor(base*(crit?1.65:1)*weak.mult*weaken*vulnerable*barrier));
...
function rangedAmmoCost(skill){if(!['hunter','ranger'].includes(state.player.class))return 0;if(!skill)return 1;if(skill.kind==='multi')return Math.max(1,skill.hits||1);return 1}
function playerAction(skill){
 if(!combat)return;const ammoCost=rangedAmmoCost(skill);if(skill&&state.player.mana<skill.mana)return toast('Za mało many.');if(ammoCost&&countItem('primitiveArrow')<ammoCost)return toast(`Brak strzał. Potrzebujesz ${ammoCost}.`);if(ammoCost)removeItem('primitiveArrow',ammoCost);if(skill)state.player.mana-=skill.mana;const meta=combatSkillMeta(skill);let total=0,hits=1;
...
function winCombat(){
 ...
 state.player.gold+=gold;state.player.kills++;playSfx('kill');haptic(18);tutorialEvent('kill');state.player.bestiary[m.id]=(state.player.bestiary[m.id]||0)+1;if(!e.synthetic){e.alive=false;e.respawn=Date.now()+5*60*1000}checkQuestProgress('kill',m.id);const loot=rollCombatLoot(m,e,c);gainXp(xp);gainPetXp(xp);if(e.elite||c.isBoss)playSfx('loot');if(c.worldBoss){state.adventure.worldBossDay=daySeed();state.adventure.reputation+=25;state.player.gold+=180;addItem('titanShard');addItem('runeShard',2);if(Math.random()<.55)addItem(pick(['runePower','runeGuard','runePrecision']));if(Math.random()<.18)addItem(pick(['stormCrown','cryptHeart']));toast('Boss świata pokonany! +25 reputacji • +180 🪙 • dodatkowy łup')}updateAchievements();const dungeonInfo=c.dungeon,lootText=lootToastText(loot.drops);combat=null;save();if(dungeonInfo){renderShell();if(lootText)toast(`Łup: ${lootText}`);advanceDungeonAfterCombat(dungeonInfo);return}renderShell();toast(`Zwycięstwo! +${xp} XP • +${gold} 🪙${lootText?` • ${lootText}`:''}`);
}
function loseCombat(){const info=combat?.dungeon,loss=Math.min(250,Math.max(10,Math.floor(state.player.gold*.04)));state.player.hp=Math.max(1,Math.floor(state.player.maxHp*.35));state.player.mana=Math.floor(state.player.maxMana*.35);state.player.gold=Math.max(0,state.player.gold-loss);combat=null;if(info?.type==='guardian'){save();renderShell();toast(`Strażnik odparł atak. Tracisz ${loss} 🪙. Możesz wrócić do wejścia i spróbować ponownie.`);return}if(dungeonRun){save();failDungeonRun('death');return}save();renderShell();toast(`Porażka. Tracisz ${loss} 🪙 i budzisz się w wiosce.`)}
...
function openModal(html){document.body.classList.add('modal-open');document.body.classList.toggle('dungeon-open',!!dungeonRun);let wrap=document.querySelector('.modal-back');if(!wrap){wrap=document.createElement('div');wrap.className='modal-back';document.body.appendChild(wrap)}wrap.innerHTML=`<div class="modal">${html}</div>`;wrap.onclick=e=>{if(e.target===wrap&&combat===null&&dungeonRun===null)closeModal()};wrap.querySelectorAll('[data-close]').forEach(b=>b.onclick=closeModal)}
