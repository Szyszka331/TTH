import {CLASSES,MONSTERS,MONSTER_LOOT,ITEMS,QUESTS,SKILLS,PETS,RECIPES,DUNGEONS,BUILDINGS} from './data.js?v=3953';
...
function drawerBagHTML(){
 ...
 let cells='';
 if(all){const bySlot=new Map(entries.map(e=>[e.slot,e]));cells=Array.from({length:Math.ceil(cap/6)*6},(_,i)=>{const e=i<cap?bySlot.get(i):null;if(i>=cap)return '<span class="drawer-bag-cell empty filler" aria-hidden="true"></span>';if(!e)return `<button class="drawer-bag-cell empty drawer-drop-target" data-drawer-bag-pos="${i}" aria-label="Pusty slot ${i+1}"></button>`;const d=itemDef(e.item.id);return `<button class="drawer-bag-cell drawer-dnd-item drawer-drop-target rarity-border-${d.rarity} ${ui.gearDrawerSelection===e.index?'selected':''}" data-drawer-item="${e.index}" data-drawer-bag-pos="${i}" draggable="true" title="${itemName(e.item)} • przeciągnij, aby zmienić miejsce" aria-label="${itemName(e.item)}${e.item.qty>1?', '+e.item.qty+' sztuk':''}">${itemIconVisual(d.id,'drawer-item-art')}${e.item.qty>1?`<b class="drawer-qty">${e.item.qty}</b>`:''}${e.item.upgrade?`<em class="drawer-upgrade">+${e.item.upgrade}</em>`:''}</button>`}).join('')}else{cells=Array.from({length:Math.max(6,Math.ceil(entries.length/6)*6)},(_,i)=>{const e=entries[i];if(!e)return '<span class="drawer-bag-cell empty" aria-hidden="true"></span>';const d=itemDef(e.item.id);return `<button class="drawer-bag-cell rarity-border-${d.rarity} ${ui.gearDrawerSelection===e.index?'selected':''}" data-drawer-item="${e.index}" title="${itemName(e.item)}" aria-label="${itemName(e.item)}${e.item.qty>1?', '+e.item.qty+' sztuk':''}">${itemIconVisual(d.id,'drawer-item-art')}${e.item.qty>1?`<b class="drawer-qty">${e.item.qty}</b>`:''}${e.item.upgrade?`<em class="drawer-upgrade">+${e.item.upgrade}</em>`:''}</button>`}).join('')}
 return `<div class="drawer-section-heading"><b>PLECAK</b><span>${used} / ${cap}</span></div><div class="drawer-bag-tools">${bagSortOptionsHTML()}</div><div class="drawer-filters" aria-label="Filtr przedmiotów">${[['all','Wszystko'],['gear','Sprzęt'],['usable','Zapasy'],['materials','Materiały']].map(([id,label])=>`<button data-drawer-filter="${id}" aria-pressed="${ui.gearDrawerFilter===id}" class="${ui.gearDrawerFilter===id?'active':''}">${label}</button>`).join('')}</div><div class="drawer-bag-grid">${cells}</div>${!entries.length?'<p class="drawer-hint">Brak przedmiotów w tej kategorii.</p>':''}<p class="drawer-hint">${all?'Przytrzymaj i przeciągnij przedmiot na inne pole, aby go przesunąć.':'Ręczne przesuwanie jest dostępne w widoku „Wszystko”.'}</p>`;
}
...
function shopStockGridHTML(items){const cells=items.map(id=>{const d=itemDef(id),selected=state.ui.shopMode!=='sell'&&state.ui.shopSelectedBuy===id,price=merchantBuyPrice(id);return `<button class="retro-shop-slot ${selected?'selected':''} rarity-frame-${d.rarity}" data-shop-select-buy="${id}" data-price="${price}" title="${d.name} • ${price} 🪙"><span class="retro-shop-slot-icon">${itemIconVisual(d.id,'shop-item-svg')}</span></button>`});while(cells.length<54)cells.push('<div class="retro-shop-slot empty" aria-hidden="true"></div>');return cells.join('')}
...
function retroSellGridHTML(entries){const cells=entries.slice(0,25).map(({item,index})=>{const d=itemDef(item.id),selected=state.ui.shopMode==='sell'&&state.ui.shopSelectedSell===index;return `<button class="retro-sell-slot ${selected?'selected':''} rarity-frame-${d.rarity}" data-shop-select-sell="${index}" data-shop-sell="${index}" title="${itemName(item)}"><span>${itemIconVisual(d.id,'shop-item-svg')}</span>${(item.qty||1)>1?`<i>${item.qty||1}</i>`:''}</button>`});while(cells.length<25)cells.push('<div class="retro-sell-slot empty" aria-hidden="true"></div>');return cells.join('')}
...
function bindBuilding(id,view){
 ...
 document.querySelectorAll('[data-shop-select-sell]').forEach(b=>b.onclick=()=>{ensureShopUiState();state.ui.shopMode='sell';state.ui.shopSelectedSell=Number(b.dataset.shopSelectSell);save();openBuilding('shop','service')});
