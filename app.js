import {CLASSES,MONSTERS,MONSTER_LOOT,ITEMS,QUESTS,SKILLS,PETS,RECIPES,DUNGEONS,BUILDINGS} from './data.js?v=3965';
...
function normalizeState(s){
 ...
 s.player.maxStamina=60; s.player.stamina ??= s.player.maxStamina; s.player.stamina=clamp(s.player.stamina,0,60);
 s.player.kills ||= 0;
...
function renderSkillsTree(el){
 ...
 el.innerHTML=`<div class="section-title"><div><h2>🌳 Umiejętności</h2><div class="muted">${c.name} • rozwijaj wybraną ścieżkę bohatera.</div></div><span class="pill gold">${p.skillPoints} pkt</span></div><div class="class-passive-card"><b>Pasyw klasy</b><span>${classPassiveText()}</span></div>${combatLoadoutHTML()}<div class="skill-branches standalone-skills">${branches.map(branch=>`<section class="skill-branch"><h4>${branch}</h4>${(SKILLS[p.class]||[]).filter(s=>(s.branch||'Umiejętności')===branch).map(s=>skillCard(s)).join('<div class="skill-link">↓</div>')}</section>`).join('')}</div>`;
...
function skillCard(s){const p=state.player,learned=p.skills.includes(s.id),active=learned&&ensureCombatSkillLoadout().includes(s.id),reqSkill=s.requires,canLevel=p.level>=s.req,canPrev=!reqSkill||p.skills.includes(reqSkill),can=canLevel&&canPrev&&p.skillPoints>=s.cost;return `<div class="skill-node ${learned?'learned':!can?'locked':''} ${active?'active-loadout':''}"><div class="skill-orb">${skillIconVisual(s.id,'skill-node-svg')}</div><div class="skill-copy"><b>${s.name}</b><div class="tiny">lvl ${s.req} • ${s.cost} pkt • mana ${s.mana}${s.cooldown?` • CD ${s.cooldown}`:''}</div><p>${s.desc}</p>${reqSkill&&!canPrev?`<div class="tiny danger-text">Wymaga: ${skillDef(reqSkill)?.name||reqSkill}</div>`:''}</div>${learned?`<div class="skill-node-actions"><span class="pill ${active?'gold':'green'}">${active?'NA PASKU':'NAUCZONE'}</span><button class="secondary mini" data-active-skill="${s.id}">${active?'Usuń z paska':'Dodaj do walki'}</button></div>`:`<button class="secondary" data-learn="${s.id}" ${can?'':'disabled'}>Odblokuj</button>`}</div>`}
...
function renderInventory(el){
 ...
 const p=state.player,slots=[['helmet','Hełm','⛑️'],['amulet','Amulet','📿'],['weapon','Broń główna','⚔️'],['armor','Pancerz','🛡️'],['offhand',p.class==='berserker'?'Druga broń':'Druga ręka',p.class==='berserker'?'🪓':'🛡️'],['gloves','Rękawice','🧤'],['ring1','Pierścień I','💍'],['ring2','Pierścień II','💍'],['boots','Buty','🥾']],used=inventoryUsedSlots(),cap=inventoryCapacity(),bag=backpackEntries(),bySlot=new Map(bag.map(e=>[e.slot,e]));
...
function currentShopStockIds(){ensureShopUiState();return Object.values(ITEMS).filter(d=>d.slot&&d.rarity==='common'&&(d.value||0)>5&&itemClassAllowed(d)&&(!d.reqLevel||d.reqLevel<=state.player.level+6)).sort((a,b)=>(a.reqLevel||1)-(b.reqLevel||1)||(a.value||0)-(b.value||0)).map(d=>d.id)}
...
function buyItem(id,building,forcedPrice){if(!ITEMS[id])return;const d=itemDef(id);if(building==='shop'&&(!d.slot||d.rarity!=='common'))return toast('Kupiec sprzedaje wyłącznie zwykły sprzęt.');if(!requireItemRoom([{id,qty:1}]))return;const price=Number.isFinite(forcedPrice)&&forcedPrice>0?forcedPrice:merchantBuyPrice(id);if(state.player.gold<price)return toast('Za mało złota.');state.player.gold-=price;addItem(id);save();openBuilding(building,'service');toast(`Kupiono: ${d.name} • -${price} 🪙`)}
...
function enemyRangedAttack(mult=.88,label='ATAK DYSTANSOWY'){const dodge=Math.random()*100<dodgeChance();if(dodge){combat.lastEnemyHit=null;setCombatTaken(0,'UNIK');logCombat(`💨 Unikasz ataku dystansowego: ${combat.monster.name}.`);return 0}const blocked=Math.random()*100<blockChance(),reduction=armorPower()*.30,guardMult=combat.guard>0?.62:1,blockMult=blocked?.58:1,gearMult=combatIncomingGearMultiplier(),dmg=Math.max(1,Math.floor((combat.atk*mult-reduction)*guardMult*blockMult*gearMult));state.player.hp=Math.max(0,state.player.hp-dmg);combat.lastEnemyHit=dmg;setCombatTaken(dmg,blocked?'BLOK':label);logCombat(`${blocked?'🛡️ Blok! ':''}${combat.monster.name} atakuje z dystansu za ${dmg}.`);if(combat.guard>0)combat.guard--;return dmg}
...
function playerAction(skill){
 ...
 if(combat.approachBuffTurns>0&&combatOffensiveSkill(skill))combat.approachBuffTurns=0;if(skill?.kind!=='pet')petAttack(false);if(combat.hp<=0)return winCombat();queueEnemyTurn();
}
