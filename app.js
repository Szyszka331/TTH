...
function connectionBanner(){return isOnline?'':'<div class=\"offline-banner\" data-offline-banner>📴 Tryb offline — zapis i większość gry działa, ale ulice na mapie mogą być niedostępne.</div>'}
...
const TUTORIAL_STEPS=[
 {id:'move',title:'Pierwszy krok',text:'Odejdź 60 m od punktu startowego. GPS odsłania mgłę wojny, a tryb testowy pozwala sprawdzić grę bez wychodzenia.',go:'map'},
 ...
 {id:'secret',title:'Coś poza szlakiem',text:'Odkryj pierwszy sekret. Mgła wojny ukrywa miejsca, których jeszcze nie znasz.',go:'map'},
...
function playSfx(type='click'){
 if(!state?.settings?.audio)return;const file=SFX_FILES[type];if(!file)return fallbackTone(type);
 try{const a=new Audio(`assets/audio/${file}`);a.volume=clamp(state.settings.sfxVolume??.68,0,1);a.playbackRate=type==='hit'?(.96+Math.random()*.08):1;a.play().catch(()=>fallbackTone(type))}catch{fallbackTone(type)}
}
function stopAmbient(){if(ambientPlayer){ambientPlayer.pause();ambientPlayer.src='';ambientPlayer=null}ambientKey=null}
function setAmbient(key){ambientWanted=key;if(!state?.settings?.ambient||!audioUnlocked){if(!state?.settings?.ambient)stopAmbient();return}const file=AMBIENT_FILES[key];if(!file){stopAmbient();return}if(ambientPlayer&&ambientKey===key&&!ambientPlayer.paused)return;stopAmbient();try{const a=new Audio(`assets/audio/${file}`);a.loop=true;a.volume=clamp(state.settings.ambientVolume??.18,0,.45);ambientPlayer=a;ambientKey=key;a.play().catch(()=>{})}catch{}}
function refreshAmbient(){if(state?.settings?.ambient&&ambientWanted)setAmbient(ambientWanted);else stopAmbient()}
...
function ensureCoreState(s=state){if(!s)return;ensureStoryState(s);s.ui ||= {heroView:'char',adventureView:'quests',menuView:'settings'};s.tutorial ||= {stage:0,complete:false,rewardGiven:false,flags:{},introSeen:true};s.tutorial.flags ||= {};s.tutorial.introSeen ??= true;s.settings ||= {};s.settings.audio ??= true;s.settings.ambient ??= true;s.settings.sfxVolume ??= .68;s.settings.ambientVolume ??= .18;s.settings.haptics ??= true;s.settings.mapMode ||= 'focused';s.settings.mapFilters ||= {};s.settings.mapFilters.trail ??= true;s.world.regionRewards ||= {};}
...
function tutorialBanner(){const t=tutorialInfo();if(!t)return'';const pct=Math.round((state.tutorial.stage/TUTORIAL_STEPS.length)*100);return `<div class="tutorial-banner npe-banner"><div class="npe-progress"><i style="width:${pct}%"></i></div><div><span>PIERWSZA GODZINA • ${state.tutorial.stage+1}/${TUTORIAL_STEPS.length}</span><b>${t.title}</b><small>${t.text}</small></div><div class="tutorial-actions"><button class="secondary" data-tutorial-go>Pokaż</button><button class="ghost" data-skip-tutorial>Pomiń</button></div></div>`}
...
function bindTutorialSkip(root=document){root.querySelector('[data-tutorial-go]')?.addEventListener('click',tutorialNavigate);root.querySelector('[data-skip-tutorial]')?.addEventListener('click',()=>{state.tutorial.complete=true;save();refresh();toast('Samouczek pominięty. Możesz uruchomić go ponownie w Menu.')})}
...
function addItem(id,qty=1){if(isStackable(id)){const f=state.player.inventory.find(x=>x.id===id);if(f)f.qty=(f.qty||1)+qty;else state.player.inventory.push({id,qty});}else{for(let i=0;i<qty;i++)state.player.inventory.push(createGearInstance(id));}checkQuestProgress('item',id);save()}
...
function discoverSecret(e){
 ensureExplorationState();if(state.world.exploration.secretsFound.includes(e.id))return toast(`${e.name} — już odkryte.`);if(dist(e,state.player.position)>40)return toast(`Sekret jest blisko. Podejdź jeszcze ${Math.max(1,Math.round(dist(e,state.player.position)-40))} m.`);
...
function topbar(){
 ...
 return `<header class="topbar"><div class="identity"><div class="mini-avatar">${classVisual(p.class,'sprite-mini')}</div><div><b>${p.name}</b><div class="tiny">${c.name} • poziom ${p.level} <span class="build-chip">1.13.1</span></div></div></div><div class="bars"><div><div class="barwrap"><div class="bar hp" style="width:${100*p.hp/p.maxHp}%"></div><div class="barlabel">HP ${p.hp}/${p.maxHp}</div></div><div class="barwrap"><div class="bar xp" style="width:${100*p.xp/need}%"></div><div class="barlabel">XP ${p.xp}/${need}</div></div></div><div><div class="barwrap"><div class="bar mana" style="width:${100*p.mana/p.maxMana}%"></div><div class="barlabel">MANA ${p.mana}/${p.maxMana}</div></div><div class="tiny">ATK ${attackPower()} • Pancerz ${armorPower()} • Kryt ${critChance().toFixed(0)}%${pet?` • 🐾 lvl ${pet.level}`:''}</div></div></div><div class="resource"><span>🪙 <strong>${p.gold}</strong></span><span class="weather">${cl.icon} ${cl.weather} • ${cl.phase}</span></div></header>`;
}
function bottomNav(){const tabs=[['map','🗺️','Mapa'],['hero','🧙','Bohater'],['adventureHub','🧭','Przygoda'],['town','🏘️','Miasto'],['menu','☰','Menu']];return `<nav class="bottom core-nav">${tabs.map(([id,ico,name])=>`<button class="navbtn ${id===currentTab?'active':''}" data-nav="${id}"><span>${ico}</span>${name}</button>`).join('')}</nav>`}
function renderShell(){destroyRealMap();app.innerHTML=`<div class="shell">${connectionBanner()}${topbar()}${tutorialBanner()}<div class="main"><main class="viewport" id="viewport"></main><aside class="side" id="side"></aside></div>${bottomNav()}</div>`;document.querySelectorAll('[data-nav]').forEach(b=>b.onclick=()=>selectNav(b.dataset.nav));selectNav(currentTab,false)}
function selectNav(id,rebuild=true){if(id!=='map')destroyRealMap();currentTab=id;if(rebuild)document.querySelectorAll('[data-nav]').forEach(b=>b.classList.toggle('active',b.dataset.nav===id));const el=document.querySelector('#viewport'),side=document.querySelector('#side');if(!el||!side)return;({map:renderMap,hero:renderHeroHub,adventureHub:renderAdventureHub,town:renderTown,menu:renderMenuHub}[id]||renderMap)(el);renderSide(side,id);bindTutorialSkip(document)}
function renderHeroHub(el){ensureCoreState();el.innerHTML=`<div class="hub-tabs"><button class="secondary ${state.ui.heroView==='char'?'active':''}" data-hero-view="char">🧙 Postać</button><button class="secondary ${state.ui.heroView==='inv'?'active':''}" data-hero-view="inv">🎒 Ekwipunek</button></div><div id="hubContent"></div>`;const body=el.querySelector('#hubContent');if(state.ui.heroView==='inv'){tutorialEvent('inventory');renderInventory(body)}else renderCharacter(body);el.querySelectorAll('[data-hero-view]').forEach(b=>b.onclick=()=>{state.ui.heroView=b.dataset.heroView;save();renderHeroHub(el)})}
function renderAdventureHub(el){ensureCoreState();const canTrips=state.player.level>=4||state.quests.done.includes('q3'),canBest=state.player.kills>0;el.innerHTML=`<div class="hub-tabs"><button class="secondary ${state.ui.adventureView==='quests'?'active':''}" data-adventure-view="quests">📜 Questy</button><button class="secondary ${state.ui.adventureView==='trips'?'active':''}" data-adventure-view="trips" ${canTrips?'':'disabled'}>🧭 Wyprawy${canTrips?'':' 🔒'}</button><button class="secondary ${state.ui.adventureView==='bestiary'?'active':''}" data-adventure-view="bestiary" ${canBest?'':'disabled'}>📖 Bestiariusz${canBest?'':' 🔒'}</button></div><div id="hubContent"></div>`;const body=el.querySelector('#hubContent');if(state.ui.adventureView==='trips'&&canTrips)renderAdventure(body);else if(state.ui.adventureView==='bestiary'&&canBest)renderBestiary(body);else{state.ui.adventureView='quests';renderQuests(body)}el.querySelectorAll('[data-adventure-view]').forEach(b=>b.onclick=()=>{if(b.disabled)return;state.ui.adventureView=b.dataset.adventureView;save();renderAdventureHub(el)})}
function renderMenuHub(el){renderMore(el)}
function refresh(){renderShell()}

function renderSide(el,id){const active=state.quests.active.map(q=>QUESTS.find(x=>x.id===q)).filter(Boolean).slice(0,2),cl=climate(),sec=regionSecretStats(),pct=regionDiscoveryPercent();el.innerHTML=`<h3>Świat</h3><div class="panel-list"><div class="panel-item"><b>${cl.icon} ${cl.weather}</b><div class="muted">${cl.phase} • ${biomeInfoAtPlayer().icon} ${biomeInfoAtPlayer().name}</div></div><div class="panel-item"><b>🗺️ Odkrycie ${pct}%</b><div class="muted">Sekrety regionu: ${sec.done}/${sec.total}</div></div>${active.length?`<div class="panel-item"><b>📜 Cel</b>${active.map(q=>`<div class="tiny" style="margin-top:6px">• ${q.name}</div>`).join('')}</div>`:''}${state.player.dungeons.length?`<div class="panel-item"><b>🕳️ Odkryte lochy</b>${state.player.dungeons.slice(0,3).map(id=>{const d=DUNGEONS.find(x=>x.id===id);return d?`<button class="ghost" style="width:100%;margin-top:6px" data-home-dungeon="${id}">${d.icon} ${d.name}</button>`:''}).join('')}</div>`:''}</div>`;el.querySelectorAll('[data-home-dungeon]').forEach(b=>b.onclick=()=>startDungeon(DUNGEONS.find(d=>d.id===b.dataset.homeDungeon)))}

function geoDistance(a,b){
 const R=6371000,toRad=v=>v*Math.PI/180,dLat=toRad(b.lat-a.lat),dLng=toRad(b.lng-a.lng),lat1=toRad(a.lat),lat2=toRad(b.lat);
...
function destroyRealMap(){
 ...
 fogLayer=null;trailLayer=null;playerMapMarker=null;accuracyCircle=null;leafletEntityLayers=[];leafletZoneLayers=[];leafletBiomeLayers=[];
}
...
function rebuildFog(){
 if(!realMap||!window.L)return;ensureExplorationState();
 if(fogLayer){realMap.removeLayer(fogLayer);fogLayer=null}
 const b=realMap.getBounds().pad(.7),outer=[[b.getSouth(),b.getWest()],[b.getNorth(),b.getWest()],[b.getNorth(),b.getEast()],[b.getSouth(),b.getEast()]];
 const holes=[];for(const key of Object.keys(state.world.exploration.sectors)){const ring=sectorLatLngRing(key);if(!ring)continue;const c=ring[0];if(b.pad(.25).contains(c)||b.pad(.25).contains(ring[2]))holes.push(ring)}
 if(!holes.length){const radius=state.world.fogRadius||90;for(const [lat,lng] of (state.world.explored||[]).slice(-300)){if(b.contains([lat,lng]))holes.push(circleRing(lat,lng,radius,22))}}
 fogLayer=L.polygon([outer,...holes],{pane:'fogPane',stroke:false,fill:true,fillColor:'#020507',fillOpacity:.87,fillRule:'evenodd',interactive:false}).addTo(realMap);
}
...
function initRealMap(){
 ...
 if(!window.L){target.innerHTML='<div class="map-error">Mapa bazowa jest niedostępna. Gra i zapis nadal działają — połącz się z internetem, aby zobaczyć ulice.</div>';return}
 destroyRealMap();
 ...
 realMap.createPane('fogPane');realMap.getPane('fogPane').style.zIndex=650;realMap.getPane('fogPane').style.pointerEvents='none';
 realMap.createPane('playerPane');realMap.getPane('playerPane').style.zIndex=700;
 L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap contributors',updateWhenIdle:true,keepBuffer:1}).addTo(realMap);
 ...
 if(origin){rebuildGameLayers();rebuildFog()}
 if(p.lat){
  const html=`<div class="leaflet-player-marker">${classVisual(p.class,'mmo-player-sprite')}<span></span></div>`;
  playerMapMarker=L.marker([p.lat,p.lng],{pane:'playerPane',icon:makeLeafletIcon(html,'player-leaflet-icon',[58,58]),zIndexOffset:1000}).addTo(realMap);
  accuracyCircle=L.circle([p.lat,p.lng],{pane:'overlayPane',radius:Math.max(8,p.accuracy||15),color:'#65b7ff',weight:1,fillColor:'#4da7ef',fillOpacity:.09,interactive:false}).addTo(realMap);
 }
 realMap.on('dragstart',()=>{followGps=false});
 realMap.on('moveend zoomend',()=>{rebuildFog();rebuildTrail()});
...
function renderMap(el){setAmbient('forest');
 destroyRealMap();
 ensureLivingWorld();const p=state.player,filters=state.settings.mapFilters,activeQ=state.quests.active.map(id=>QUESTS.find(q=>q.id===id)).filter(Boolean)[0],hasGeo=!!(p.position.lat&&p.position.lng),hasOrigin=!!state.world.gpsOrigin;
 for(const e of state.world.entities)if(e.type==='monster'&&!e.alive&&e.respawn<=Date.now())e.alive=true;
 const currentZone=zoneAt(p.position.x,p.position.y),fogPoints=(state.world.explored||[]).length,bio=biomeInfoAtPlayer(),exp=explorationStats(),reg=regionDiscoveryStats(),regionPct=reg.percent,sec=regionSecretStats(reg.id),cart=cartographyStats(),virtual=!!p.position.virtualTravel;
 el.innerHTML=`<div class="section-title"><div><h2>🗺️ Mapa świata</h2><div class="muted">Odkrywaj realny teren. W trybie skupionym mapa pokazuje przede wszystkim rzeczy ważne tu i teraz.</div></div><span class="pill ${currentZone}">strefa ${currentZone}</span></div><div class="map-toolbar real-map-toolbar"><button class="primary" data-gps>${gpsWatch?'📍 GPS aktywny':'📍 Uruchom GPS'}</button><button class="secondary" data-center-player ${hasGeo?'':'disabled'}>🎯 Do mnie</button><button class="secondary" data-map-mode>👁️ ${state.settings.mapMode==='focused'?'Skupiona':'Pełna'}</button><button class="secondary" data-explorer-journal>🧭 Dziennik</button><button class="secondary" data-fast-travel>⚡ Podróż</button><details class="layer-details"><summary>⚙️ Warstwy</summary><div class="layer-popover">${[['monster','👹 Potwory'],['poi','📌 Miejsca'],['dungeon','🕳️ Lochy'],['event','✨ Eventy'],['biome','🌿 Biomy'],['trail','👣 Ślad']].map(([k,n])=>`<button class="filter-btn ${filters[k]?'active':''}" data-filter="${k}">${n}</button>`).join('')}</div></details></div><div class="living-world-strip compact-world"><div><b>${bio.icon} ${bio.name}</b><small>${bio.desc}</small></div><div><b>${reg.icon} ${reg.name} ${regionPct}%</b><small>${reg.done}/${reg.total} sektorów • sekrety ${sec.done}/${sec.total}</small></div><div><b>🧭 Kartograf ${cart.level}</b><small>${cart.rank} • ${cart.sectors} sektorów</small></div><div><b>🎯 Dzisiaj ${exp.count}/${exp.goal}</b><small>${exp.claimed?'Nagroda odebrana':'Nowe sektory 100 m'}</small></div></div>${activeQ?`<div class="quest-ribbon">📜 <b>${activeQ.name}</b> — ${activeQ.steps.find((s,i)=>(state.quests.progress[activeQ.id]?.[i]||0)<(s.count||1))?.label||'Cel ukończony'}</div>`:''}<div class="real-map-shell"><div id="realMap" class="real-map"></div><div class="mmo-map-top"><span class="mmo-map-badge" data-live-gps>${virtual?'TRYB DOMOWY':hasGeo?`GPS ±${Math.round(p.position.accuracy||0)} m`:'GPS wyłączony'}</span><span class="mmo-map-badge fog-badge">🌫️ <span data-fog-count>${regionPct}% odkryte</span></span></div><div class="mmo-map-bottom"><div><b>TIME4HEROES</b><small>${hasOrigin?'Odkrywaj nieznane sektory i sekrety':'Włącz GPS, aby zakotwiczyć świat gry'}</small></div><span class="zone-rune ${currentZone}">${reg.icon} ${reg.name}</span></div>${!hasGeo?`<div class="gps-start-card"><div class="gps-radar">⌖</div><b>Włącz lokalizację</b><p>GPS ustawi Twoją postać na prawdziwej mapie i zacznie odsłaniać mgłę wojny.</p><button class="primary" data-gps-start>📍 Zezwól na GPS</button></div>`:''}</div><div class="map-legend"><span>🔵 Ty</span><span>⭐ Elity</span><span>📜 Cel questa</span><span>✨ Event</span><span>❔ Sekret</span><span>👣 Twój ślad</span><span>🌫️ Mgła wojny</span></div>`;
 el.querySelector('[data-gps]').onclick=toggleGps;el.querySelector('[data-gps-start]')?.addEventListener('click',toggleGps);el.querySelector('[data-center-player]')?.addEventListener('click',()=>{if(!realMap||!state.player.position.lat)return;followGps=true;realMap.setView([state.player.position.lat,state.player.position.lng],17,{animate:true})});el.querySelector('[data-map-mode]').onclick=()=>{state.settings.mapMode=state.settings.mapMode==='focused'?'full':'focused';save();renderMap(el)};el.querySelector('[data-explorer-journal]')?.addEventListener('click',openExplorerJournal);el.querySelector('[data-fast-travel]')?.addEventListener('click',openFastTravel);el.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{filters[b.dataset.filter]=!filters[b.dataset.filter];save();renderMap(el)});initRealMap();
}
...
function interactEntity(e){if(!e)return;const d=dist(e,state.player.position);if(state.player.position.virtualTravel&&e.type!=='dungeon')return toast('Tryb podróży domowej nie pozwala na interakcje GPS. Włącz GPS, aby wrócić do świata.');if(e.type==='secret'){discoverSecret(e);return}if(e.type==='event'){resolveWorldEvent(e);return}if(e.type==='monster'){if(d>75)return toast(`Podejdź bliżej. ${Math.round(d)} m.`);startCombat(e);return}if(d>40&&!(e.type==='dungeon'&&state.player.dungeons.includes(e.id)))return toast(`Musisz podejść bliżej. ${Math.round(d)} m.`);if(e.type==='poi'){if(e.id==='nightGuest'&&climate().phase!=='Noc')return toast('To miejsce ma znaczenie nocą. Włącz symulację nocy w Więcej albo wróć później.');const fresh=!state.player.discovered.includes(e.id);if(fresh){state.player.discovered.push(e.id);playSfx('discover');haptic(20)}checkQuestProgress('discover',e.id);save();toast(fresh?`Odkryto: ${e.name}`:e.name);selectNav('map');return}if(e.type==='dungeon'){if(!state.player.dungeons.includes(e.id)){state.player.dungeons.push(e.id);state.player.discovered.push(e.id);playSfx('discover');haptic([20,25,20]);save();toast(`Odkryto loch: ${e.name}. Od teraz wejdziesz także z domu.`);tutorialEvent('dungeonDiscover',e.id)}startDungeon(DUNGEONS.find(x=>x.id===e.id)||e)}}
...
function renderCharacter(el){
 ...
 el.querySelectorAll('[data-slot]').forEach(b=>b.onclick=()=>{state.ui.heroView='inv';currentTab='hero';selectNav('hero');toast(`Wybierz przedmiot do slotu: ${b.dataset.slot}`)});
} 
...
function renderInventory(el){
 ...
 el.innerHTML=`<div class="section-title"><div><h2>🎒 Ekwipunek</h2><div class="muted">Sprzęt może mieć afiks, enchant i jedną runę. Aktywuj bonusy zestawów.</div></div><span class="pill">${p.inventory.length} wpisów</span></div><div class="equip-strip">${slots.map(slot=>{const i=equippedInstance(slot),d=i?itemDef(i.id):null;return `<div class="equip-chip"><span>${d?itemIconVisual(d.id,'equip-item-svg'):'·'}</span><small>${slotLabel(slot)}</small><b>${d?itemName(i):'—'}</b></div>`}).join('')}</div>${setStatusHTML()}<h3 style="margin-top:18px">Plecak</h3><div class="inventory-grid">${p.inventory.map((i,idx)=>inventoryCard(i,idx)).join('')}</div>`;
...
function renderQuests(el){updateStoryConditions();ensureStoryState();const all=[...state.quests.active,...state.quests.done].map(id=>QUESTS.find(q=>q.id===id)).filter(Boolean),active=state.quests.active.map(id=>QUESTS.find(q=>q.id===id)).find(Boolean),chapter=active?.chapter||all.at(-1)?.chapter||'Przygoda';el.innerHTML=`<div class="section-title"><h2>📜 Dziennik zadań</h2><span class="pill">${chapter}</span></div>${storyProfileHTML()}<div class="panel-list">${all.map(q=>questHTML(q)).join('')||'<div class="panel-item">Brak zadań.</div>'}</div>`;el.querySelectorAll('[data-story-scene]').forEach(b=>b.onclick=()=>openStoryScene(b.dataset.storyScene))}
function questHTML(q){const done=state.quests.done.includes(q.id),prog=state.quests.progress[q.id]||q.steps.map(()=>0),scene=storyScene(q.id),choice=storyChoiceFor(q.id),chosen=choice&&scene?.choices.find(c=>c.id===choice),canChoose=storyCanChoose(q.id);return `<div class="panel-item quest ${done?'done':''} ${scene?'story-quest':''}"><div class="quest-title-row"><b>${done?'✅':'📜'} ${q.name}</b>${scene?'<span class="story-badge">FABUŁA</span>':''}</div><div class="tiny">Sugerowany poziom ${q.level} • ${q.xp} XP • ${q.gold} 🪙</div><p>${q.desc}</p>${q.steps.map((s,i)=>{const val=done?(s.count||1):(prog[i]||0),label=s.type==='condition'?'🕰️ '+s.label:s.type==='story'?'💬 '+s.label:s.label;return `<div class="quest-step ${val>=(s.count||1)?'step-done':''}"><span>${label}</span><b>${done||val>=(s.count||1)?'✓':`${Math.min(val,s.count||1)}/${s.count||1}`}</b></div>`}).join('')}${scene?`<div class="story-quest-actions"><button class="secondary" data-story-scene="${q.id}" ${!canChoose&&!choice?'disabled':''}>${choice?'💬 Zobacz decyzję':canChoose?'💬 Rozmowa / decyzja':'🔒 Decyzja później'}</button>${chosen?`<small>Wybrano: <b>${chosen.label}</b></small>`:''}</div>`:''}</div>`}
...
function renderTown(el){setAmbient('town');const cl=climate();el.innerHTML=`<div class="section-title"><div><h2>🏘️ Wioska Startowa</h2><div class="muted">${cl.icon} ${cl.weather} • ${cl.phase}. Kolejne usługi odblokowują się wraz z rozwojem bohatera.</div></div><span class="pill">HUB 1.13.1</span></div><div class="town-scene"><div class="town-sky"></div><div class="town-banner"><b>WIOSKA POD KRUKIEM</b><span>Najpierw karczma. Reszta świata otwiera się stopniowo.</span></div><div class="town-grid v03-town">${BUILDINGS.map(b=>{const u=buildingUnlock(b.id);return `<button class="building ${u.ok?'':'locked-building'}" data-building="${b.id}" ${u.ok?'':'disabled'}><span class="building-icon">${b.icon}</span><b>${b.name}</b><div class="tiny">${u.ok?(b.tag||'Wejdź'):`🔒 ${u.reason}`}</div><em>${u.ok?'WEJDŹ →':'ZABLOKOWANE'}</em></button>`}).join('')}</div></div>`;el.querySelectorAll('[data-building]:not(:disabled)').forEach(b=>b.onclick=()=>openBuilding(b.dataset.building))}
function openBuilding(id){setAmbient(id==='tavern'?'tavern':'town');const unlock=buildingUnlock(id);if(!unlock.ok)return toast(`Odblokujesz to: ${unlock.reason}.`);if(id==='tavern')tutorialEvent('tavern');const map={tavern:['🍺 Karczma „Pod Krukiem”',tavernHTML,'tavern'],shop:['🛒 Sklep kupiecki',shopHTML,'shop'],smith:['⚒️ Kuźnia',smithHTML,'smith'],alchemist:['⚗️ Alchemik',alchemistHTML,'alchemist'],auction:['🏛️ Dom aukcyjny',auctionHTML,'auction'],guild:['🏰 Sala gildii',guildHTML,'guild']};const [title,fn,scene]=map[id]||map.tavern;openModal(`<div class="location-scene scene-${scene}"><div class="location-overlay"><div class="modal-head"><div><h2>${title}</h2><div class="muted">Wioska Startowa • Build 1.13.1</div></div><button class="close" data-close>×</button></div><div id="buildingBody">${fn()}</div></div></div>`);bindBuilding(id)}

function tavernHTML(){
 const next=QUESTS.find(q=>!state.quests.done.includes(q.id)&&!state.quests.active.includes(q.id)&&q.level<=state.player.level+1);
 return `${npcCard('Dorian','Karczmarz','knight','Pod dachem „Pod Krukiem” odpoczniesz, zbierzesz plotki i podejmiesz kolejne wyprawy.')}<div class="panel-list"><div class="panel-item"><b>🛏️ Odpoczynek</b><div class="muted">Pełne HP i mana. Cena rośnie łagodnie wraz z poziomem bohatera.</div><button class="secondary" data-rest>Odpocznij</button></div>${next?`<div class="panel-item"><b>📜 Tablica: ${next.name}</b><div>${next.desc}</div><button class="secondary" data-accept="${next.id}">Przyjmij</button></div>`:''}<div class="panel-item"><b>🗣️ Plotka dnia</b><div class="muted">„Rzadkie strefy przesuwają się o świcie. Wczorajsza droga może dziś prowadzić do czegoś zupełnie innego.”</div></div></div>`}


function shopHTML(){const ids=['potion','manaPotion','herb','scrap','leather','ironArmor'];return `${npcCard('Selma','Kupcowa','hunter','Ceny kupna są wyższe niż wartość odsprzedaży — najlepszy sprzęt nadal zdobywa się w świecie.')}<div class="panel-list">${ids.map(id=>{const i=itemDef(id),price=merchantBuyPrice(id);return `<div class="panel-item shoprow"><div><b class="shop-item-name">${itemIconVisual(i.id,'shop-item-svg')} ${i.name}</b><div class="muted">${price} 🪙 • sprzedaż u kupca: ${Math.max(1,Math.floor(i.value*(ECONOMY.sell[i.rarity]??.28)))} 🪙</div></div><button class="secondary" data-buy="${id}" data-price="${price}">Kup</button></div>`}).join('')}</div>`}

function smithHTML(){const gear=state.player.inventory.filter(i=>itemDef(i.id).slot),recipes=RECIPES.filter(r=>r.station==='smith'),runes=['runePower','runeGuard','runePrecision'];return `${npcCard('Ragor','Kowal i runmistrz','berserker','Ulepszanie jest coraz droższe, a wysokiej jakości przedmioty wymagają więcej materiałów.')}<div class="forge-summary"><span>🔹 Odłamki: <b>${countItem('runeShard')}</b></span>${runes.map(id=>`<span>${itemDef(id).icon} ${countItem(id)}</span>`).join('')}</div><h3>⚒️ Obróbka sprzętu</h3><div class="panel-list">${gear.map(i=>{const d=itemDef(i.id),up=i.upgrade||0,q=upgradeQuote(i),eq=enchantQuote(i,!!i.enchant);return `<div class="panel-item smith-item"><div><b class="shop-item-name">${itemIconVisual(d.id,'shop-item-svg')} ${itemName(i)}</b><div class="muted">${rarityName(d.rarity)} • +${up} • ${i.enchant?`🔮 ${i.enchant.name}`:'bez enchantu'} • ${i.rune?`${itemDef(i.rune).icon} ${itemDef(i.rune).name}`:'wolne gniazdo runy'}</div></div><div class="smith-actions"><button class="secondary" data-upgrade="${i.uid}" ${up>=5?'disabled':''}>${up>=5?'MAX':`Ulepsz ${q.gold}🪙${q.scrap?` + ${q.scrap}🔩`:''}`}</button><button class="secondary" data-enchant="${i.uid}">${i.enchant?`Przerzuć ${eq.gold}🪙`:`Enchant ${eq.gold}🪙`}</button>${!i.rune?runes.map(r=>`<button class="ghost" data-socket="${i.uid}" data-rune="${r}" ${countItem(r)?'':'disabled'}>${itemDef(r).icon} Osadź</button>`).join(''):`<button class="ghost" data-unsocket="${i.uid}">↩️ Wyjmij runę</button>`}</div></div>`}).join('')}</div><h3>🧰 Receptury kuźni</h3><div class="panel-list">${recipes.map(r=>{const fee=craftFee(r);return `<div class="panel-item shoprow"><div><b class="shop-item-name">${itemIconVisual(r.result,'shop-item-svg')} ${r.name}</b><div class="muted">${Object.entries(r.ingredients).map(([id,q])=>`${q}× ${itemDef(id).icon} ${itemDef(id).name}`).join(' + ')} • opłata ${fee} 🪙</div></div><button class="secondary" data-craft="${r.id}">Wykuj</button></div>`}).join('')}</div>`}

function alchemistHTML(){return `${npcCard('Ilyra','Alchemiczka','mage','Alchemia zużywa materiały i niewielką opłatę za przygotowanie stanowiska.')}<div class="panel-list">${RECIPES.filter(r=>r.station==='alchemist').map(r=>{const fee=craftFee(r);return `<div class="panel-item shoprow"><div><b class="shop-item-name">${itemIconVisual(r.result,'shop-item-svg')} ${r.name}</b><div class="muted">${Object.entries(r.ingredients).map(([id,q])=>`${q}× ${itemDef(id).icon} ${itemDef(id).name}`).join(' + ')} • ${fee} 🪙</div></div><button class="secondary" data-craft="${r.id}">Uwarz</button></div>`}).join('')}</div>`}

function auctionHTML(){const offerIds=['blueBlade','forestBow','arcaneStaff','shadowRing','wolfCharm','ironHelm','emberRing','ravenBlade','wildBow','mistHood'];const ids=[0,1,2,3].map(i=>offerIds[Math.floor(seeded(daySeed()+i*91)*offerIds.length)]);return `${npcCard('Varo','Licytator','ranger','Dom aukcyjny NPC jest wygodny, ale nie jest najtańszą drogą do mocnego ekwipunku.')}<p class="muted">Oferty zmieniają się codziennie. Sprzęt epicki pojawia się rzadziej i kosztuje wyraźnie więcej niż crafting.</p><div class="panel-list">${ids.map((id,i)=>{const d=itemDef(id),factor=ECONOMY.auctionMin+seeded(daySeed()+i*17)*(ECONOMY.auctionMax-ECONOMY.auctionMin)+(rarityRank(d.rarity)>=3?.12:0),price=Math.ceil(d.value*factor);return `<div class="panel-item shoprow"><div><b class="rarity-${d.rarity} shop-item-name">${itemIconVisual(d.id,'shop-item-svg')} ${d.name}</b><div class="muted">${rarityName(d.rarity)} • ${price} 🪙</div></div><button class="secondary" data-auction-buy="${id}" data-price="${price}">Kup</button></div>`}).join('')}</div>`}

function guildHTML(){return `${npcCard('Edrin','Mistrz Gildii','knight','Sala gildii jest zamknięta do 10 poziomu. W tej wersji służy jako lokalna baza osiągnięć bohatera.')}<div class="panel-item"><b>🛡️ Gildia</b><p>${state.player.guild?`Należysz do gildii <b>${state.player.guild}</b>.`:'Nie należysz jeszcze do gildii.'}</p>${!state.player.guild?'<input id="guildName" placeholder="Nazwa gildii" style="width:100%"><button class="secondary" style="margin-top:8px" data-create-guild>Utwórz gildię bohatera</button>':'<div class="muted">Rozwijaj reputację i przygotuj bohatera do przyszłych wypraw gildyjnych.</div>'}</div>`}

function bindBuilding(id){
 document.querySelector('[data-rest]')?.addEventListener('click',()=>{const cost=Math.min(45,8+state.player.level*2);if(state.player.gold<cost)return toast(`Odpoczynek kosztuje ${cost} 🪙.`);state.player.gold-=cost;state.player.hp=state.player.maxHp;state.player.mana=state.player.maxMana;save();closeModal();refresh();toast(`Odpoczynek zakończony • -${cost} 🪙`) });
 ...
 document.querySelector('[data-accept]')?.addEventListener('click',e=>{const qid=e.target.dataset.accept;if(!state.quests.active.includes(qid))state.quests.active.push(qid);save();closeModal();refresh();toast('Zadanie przyjęte.');if(storyScene(qid))setTimeout(()=>openStoryScene(qid),180)});
...
function renderMore(el){el.innerHTML=`<div class="section-title"><h2>☰ Menu</h2><span class="pill">Build 1.11</span></div><div class="panel-list"><div class="panel-item mobile-install-card"><b>📲 Time4Heroes na telefonie</b><div class="muted">Zainstaluj grę na ekranie głównym.</div><button class="primary" data-install-app>${isStandalone()?'✅ Aplikacja zainstalowana':'Zainstaluj na telefonie'}</button></div><div class="panel-item"><b>🔊 Dźwięk i haptyka</b><div class="settings-toggles"><button class="secondary" data-audio>${state.settings.audio?'🔊 SFX: ON':'🔇 SFX: OFF'}</button><button class="secondary" data-ambient>${state.settings.ambient?'🎵 Ambient: ON':'🎵 Ambient: OFF'}</button><button class="secondary" data-haptics>${state.settings.haptics?'📳 Wibracje: ON':'📴 Wibracje: OFF'}</button></div></div><div class="panel-item"><b>💾 Zapis gry</b><div class="muted">Wersja przed multiplayerem: zapis lokalny, bez Supabase. Build 1.11 przejmuje save z 1.10 i starszych wersji.</div><div class="tabs" style="margin-top:8px"><button class="secondary" data-export>Eksportuj</button><button class="secondary" data-import>Importuj</button><input type="file" id="saveFile" accept="application/json" hidden></div></div><div class="panel-item"><b>🌙 Symulacja nocy</b><button class="secondary" data-night>${state.settings.forceNight?'Wyłącz symulację':'Włącz noc'}</button></div><div class="panel-item"><b>🎓 Samouczek</b><div class="muted">${state.tutorial.complete?'Ukończony. Możesz uruchomić go ponownie.':'Aktywny.'}</div><button class="secondary" data-restart-tutorial>Uruchom od początku</button></div><div class="panel-item"><b>🎓 New Player Experience — 1.11</b><div class="muted">Lepsze pierwsze 30–60 minut: prolog, prowadzenie krok po kroku, pierwszy loot, skill, sekret i treningowy loch.</div></div><div class="panel-item"><b>💰 Gospodarka 1.8</b><div class="muted">Sprzęt sprzedaje się za 28–48% wartości zależnie od rzadkości. Rozbiórka daje materiały, bossy i elity mają system gwarantowanego łupu po serii pecha.</div><div class="tiny" style="margin-top:6px">Sprzedane przedmioty: ${state.economy?.totalSold||0} • Rozebrane: ${state.economy?.totalSalvaged||0} • Pity elity: ${state.economy?.elitePity||0}/4 • Pity boss: ${state.economy?.bossPity||0}/3</div></div><div class="panel-item"><b>🔄 Reset</b><button class="danger" data-reset>Usuń zapis</button></div></div>`;el.querySelector('[data-install-app]')?.addEventListener('click',installApp);el.querySelector('[data-export]').onclick=exportSave;el.querySelector('[data-import]').onclick=()=>document.querySelector('#saveFile').click();document.querySelector('#saveFile').onchange=importSave;el.querySelector('[data-night]').onclick=()=>{state.settings.forceNight=!state.settings.forceNight;save();refresh()};el.querySelector('[data-audio]').onclick=()=>{state.settings.audio=!state.settings.audio;save();renderMore(el)};el.querySelector('[data-ambient]').onclick=()=>{state.settings.ambient=!state.settings.ambient;save();refreshAmbient();renderMore(el)};el.querySelector('[data-haptics]').onclick=()=>{state.settings.haptics=!state.settings.haptics;save();renderMore(el)};el.querySelector('[data-restart-tutorial]').onclick=()=>{state.tutorial={stage:0,complete:false,rewardGiven:true,flags:{},introSeen:true,finishReward:true};save();refresh()};el.querySelector('[data-reset]').onclick=()=>{if(confirm('Na pewno usunąć zapis?')){localStorage.removeItem(SAVE_KEY);state=null;render()}}}
function openShowcase(){openModal(`<div class="modal-head"><div><h2>Time4Heroes</h2><div class="muted">Build 1.11 • New Player Experience</div></div><button class="close" data-close>×</button></div><p>Dopracowane pierwsze 30–60 minut: prolog, czytelne cele, pierwszy skill, sekret i treningowy loch.</p>`)}
function exportSave(){const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='time4heroes-build-1.11-save.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
