import {CLASSES,MONSTERS,ITEMS,QUESTS,SKILLS,PETS,RECIPES,DUNGEONS,BUILDINGS} from './data.js?v=2001';
...
function renderQuests(el){
 ...
 const all=[...state.quests.active,...state.quests.done].map(id=>QUESTS.find(q=>q.id===id)).filter(Boolean),active=state.quests.active.map(id=>QUESTS.find(q=>q.id===id)).find(Boolean),chapter=active?.chapter||all.at(-1)?.chapter||'Przygoda';
 const contracts=(state.adventure.bounties||[]).filter(b=>b.accepted&&!b.claimed);
 el.innerHTML=`<div class="section-title"><h2>📜 Dziennik zadań</h2><span class="pill">${activeTaskCount()}/4 aktywne</span></div>${tutorialJournalHTML()}${storyProfileHTML()}<div class="panel-list">${all.map(q=>questHTML(q)).join('')||'<div class="panel-item">Brak aktywnych zadań fabularnych.</div>'}</div>${contracts.length?`<h3>📌 Zlecenia z tablicy</h3><div class="panel-list">${contracts.map(b=>`<div class="panel-item quest-card"><div class="quest-title-row"><b>${b.icon} ${b.name}</b><span class="story-badge">KONTRAKT</span></div><div class="muted">Pokonaj: ${MONSTERS.find(m=>m.id===b.target)?.name||b.target} • ${b.progress||0}/${b.need}</div><div class="quest-reward">${b.xp} XP • ${b.gold} 🪙 • ${b.rep} reputacji</div>${(b.progress||0)>=b.need?`<button class="secondary" data-claim-bounty="${b.id}">Odbierz nagrodę</button>`:''}</div>`).join('')}</div>`:''}<div class="quest-limit-note">Samouczek nie zajmuje miejsca. Pozostałe zadania: maksymalnie 4 jednocześnie.</div>`;
...
const CITY_INTERIORS={
 tavern:{title:'Karczma „Pod Krukiem”',npc:'Dorian',role:'Karczmarz • były wojownik',classId:'knight',bg:'assets/interior-tavern.png',quote:'„Miecz odwiesiłem na ścianę. Pamięć o potworach — nie.”'},
...
function renderTown(el){setAmbient('town');const cl=climate();el.innerHTML=`<div class="section-title"><div><h2>🏘️ Wioska Pod Krukiem</h2><div class="muted">${cl.icon} ${cl.weather} • ${cl.phase}. Każdy budynek jest teraz osobnym miejscem z własnymi interakcjami.</div></div><span class="pill">HUB 2.0A</span></div><div class="town-scene"><div class="town-sky"></div><div class="town-banner"><b>WIOSKA POD KRUKIEM</b><span>Wejdź do środka, podejdź do NPC albo skorzystaj z wyposażenia budynku.</span></div><div class="town-grid v03-town">${BUILDINGS.map(b=>{const u=buildingUnlock(b.id);return `<button class="building ${u.ok?'':'locked-building'}" data-building="${b.id}" ${u.ok?'':'disabled'}><span class="building-icon">${b.icon}</span><b>${b.name}</b><div class="tiny">${u.ok?'Wnętrze RPG':`🔒 ${u.reason}`}</div><em>${u.ok?'WEJDŹ →':'ZABLOKOWANE'}</em></button>`}).join('')}</div></div>`;el.querySelectorAll('[data-building]:not(:disabled)').forEach(b=>b.onclick=()=>openBuilding(b.dataset.building,'scene'))}
function buildingSceneHTML(id){const c=CITY_INTERIORS[id];const secondary=id==='tavern'?`<button class="interior-hotspot fireplace-hotspot" data-building-action="fireplace"><span>🔥</span><b>Kominek</b><small>Odpoczynek i regeneracja</small></button><button class="interior-hotspot board-hotspot" data-building-action="board"><span>📌</span><b>Tablica ogłoszeń</b><small>Questy i kontrakty</small></button>`:id==='smith'?`<button class="interior-hotspot prop-hotspot" data-building-action="service"><span>⚒️</span><b>Kowadło i palenisko</b><small>Ulepszanie • runy • crafting</small></button>`:id==='alchemist'?`<button class="interior-hotspot prop-hotspot" data-building-action="service"><span>⚗️</span><b>Stół alchemiczny</b><small>Mikstury • receptury</small></button>`:id==='shop'?`<button class="interior-hotspot prop-hotspot" data-building-action="service"><span>🧺</span><b>Lada i regały</b><small>Kupno towarów</small></button>`:id==='auction'?`<button class="interior-hotspot prop-hotspot" data-building-action="service"><span>📖</span><b>Księga aukcji</b><small>Dzisiejsze oferty</small></button>`:`<button class="interior-hotspot prop-hotspot" data-building-action="service"><span>🛡️</span><b>Stół gildii</b><small>Gildia i reputacja</small></button>`;
 return `<div class="city-interior room-${id}" style="--room-bg:url('${c.bg}')"><div class="room-atmosphere"></div><div class="interior-heading"><div><span>WNĘTRZE</span><h2>${c.title}</h2></div><small>Kliknij postać lub element pomieszczenia</small></div><div class="interior-stage"><button class="interior-npc" data-building-action="keeper">${npcVisual(c.npc,c.classId,'sprite-room-npc')}<span class="npc-counter"></span><strong>${c.npc}</strong><small>${c.role}</small></button>${secondary}</div><div class="interior-quote">${c.quote}</div></div>`}
...
function openBuilding(id,view='scene'){setAmbient(id==='tavern'?'tavern':'town');const unlock=buildingUnlock(id);if(!unlock.ok)return toast(`Odblokujesz to: ${unlock.reason}.`);if(id==='tavern')tutorialEvent('tavern');const c=CITY_INTERIORS[id]||CITY_INTERIORS.tavern;let body=buildingSceneHTML(id);if(view==='talk')body=buildingTalkHTML(id);else if(id==='tavern'&&view==='keeper')body=tavernKeeperHTML();else if(id==='tavern'&&view==='fireplace')body=tavernFireplaceHTML();else if(id==='tavern'&&view==='board')body=tavernBoardHTML();else if(view==='service')body=buildingServiceHTML(id);openModal(`<div class="location-scene scene-${id} city-modal"><div class="location-overlay"><div class="modal-head"><div><h2>${c.title}</h2><div class="muted">Wioska Pod Krukiem • Visual 2.0A</div></div><button class="close" data-close>×</button></div><div id="buildingBody">${body}</div></div></div>`);bindBuilding(id,view)}
...
function exportSave(){const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='time4heroes-build-2.0a-save.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
