...
const SAVE_KEY='georpg_build_03';
const OLD_SAVE_KEY='georpg_build_02';
const LEGACY_SAVE_KEY='georpg_build_01';
...
function load(){
 ...
 const old=rawLoad(OLD_SAVE_KEY);if(old){const migrated=normalizeState(old);localStorage.setItem(SAVE_KEY,JSON.stringify(migrated));return migrated}
 const legacy=rawLoad(LEGACY_SAVE_KEY);if(legacy){const migrated=normalizeState(legacy);localStorage.setItem(SAVE_KEY,JSON.stringify(migrated));return migrated}
 return null;
}
...
function renderCreate(){
 let selected='knight';
 app.innerHTML=`<div class="boot v03"><div class="card create"><div class="build-mark">BUILD 0.3 • VISUAL & COMBAT OVERHAUL</div><h1 class="logo">GEORPG</h1><p class="sub">GPS odkrywa świat. Bohater rozwija się także w domu.</p><h2>Stwórz bohatera</h2><div class="form-row"><label>Imię</label><input id="heroName" maxlength="18" value="Krzysztof"></div><div class="class-grid">${Object.entries(CLASSES).map(([id,c])=>`<button class="class-btn ${id===selected?'active':''}" data-class="${id}"><span class="class-icon">${c.icon}</span><b>${c.name}</b><div class="tiny">${c.desc}</div></button>`).join('')}</div><div id="classDesc" class="panel-item hero-preview" style="margin:12px 0"></div><button id="startGame" class="primary large">Rozpocznij przygodę</button></div></div>`;
 const desc=()=>{const c=CLASSES[selected];document.querySelector('#classDesc').innerHTML=`<div class="preview-avatar">${c.icon}</div><div><b>${c.name}</b><div class="muted">STR ${c.base.str} • AGI ${c.base.agi} • INT ${c.base.int} • VIT ${c.base.vit}</div><div>${c.desc}</div>${['hunter','ranger'].includes(selected)?'<div class="gold">🐺 Startujesz z chowańcem: Młody Wilk.</div>':''}</div>`};desc();
...
function topbar(){
 ...
 return `<header class="topbar"><div class="identity"><div class="mini-avatar">${c.icon}</div><div><b>${p.name}</b><div class="tiny">${c.name} • poziom ${p.level} <span class="build-chip">0.3</span></div></div></div><div class="bars"><div><div class="barwrap"><div class="bar hp" style="width:${100*p.hp/p.maxHp}%"></div><div class="barlabel">HP ${p.hp}/${p.maxHp}</div></div><div class="barwrap"><div class="bar xp" style="width:${100*p.xp/need}%"></div><div class="barlabel">XP ${p.xp}/${need}</div></div></div><div><div class="barwrap"><div class="bar mana" style="width:${100*p.mana/p.maxMana}%"></div><div class="barlabel">MANA ${p.mana}/${p.maxMana}</div></div><div class="tiny">ATK ${attackPower()} • Pancerz ${armorPower()} • Kryt ${critChance().toFixed(0)}%${pet?` • ${petDef(pet.id).icon} lvl ${pet.level}`:''}</div></div></div><div class="resource"><span>🪙 <strong>${p.gold}</strong></span><span class="weather">${cl.icon} ${cl.weather} • ${cl.phase}</span></div></header>`;
}
function bottomNav(){const tabs=[['map','🗺️','Mapa'],['char','🧙','Postać'],['inv','🎒','Ekwipunek'],['quests','📜','Questy'],['bestiary','📖','Bestiariusz'],['town','🍺','Miasto'],['social','👥','Gracze'],['more','⚙️','Więcej']];return `<nav class="bottom">${tabs.map(([id,ico,name])=>`<button class="navbtn ${id===currentTab?'active':''}" data-nav="${id}"><span>${ico}</span>${name}</button>`).join('')}</nav>`}
...
function renderSocial(el){const fake=[{n:'Ardan',l:7,c:'knight',d:42},{n:'Mirael',l:6,c:'mage',d:88},{n:'Borvik',l:9,c:'berserker',d:135}];el.innerHTML=`<div class="section-title"><h2>👥 Gracze w pobliżu</h2><span class="pill">DEMO OFFLINE</span></div><p class="muted">Interfejs jest gotowy pod multiplayer. Te postacie są lokalną demonstracją.</p><div class="panel-list">${fake.map((x,i)=>`<button class="panel-item" style="color:inherit;text-align:left" data-player="${i}"><b>${CLASSES[x.c].icon} ${x.n}</b> • lvl ${x.l}<div class="muted">${x.d} m od Ciebie</div></button>`).join('')}</div>`;el.querySelectorAll('[data-player]').forEach(b=>b.onclick=()=>openPlayer(fake[Number(b.dataset.player)]))}
function openPlayer(x){openModal(`<div class="modal-head"><div><h2>${CLASSES[x.c].icon} ${x.n}</h2><div class="muted">${CLASSES[x.c].name} • lvl ${x.l}</div></div><button class="close" data-close>×</button></div><div class="tabs"><button class="secondary" data-social="party">➕ Drużyna</button><button class="secondary" data-social="pvp">⚔️ PvP</button><button class="secondary" data-social="trade">🤝 Handel</button><button class="secondary" data-social="profile">👤 Profil</button><button class="secondary" data-social="friend">⭐ Znajomy</button></div><p class="muted">Prawdziwa synchronizacja graczy będzie wymagała logowania i backendu realtime.</p>`);document.querySelectorAll('[data-social]').forEach(b=>b.onclick=()=>toast(`„${b.textContent.trim()}” — UI gotowe, backend jeszcze niepodłączony.`))}

function renderMore(el){el.innerHTML=`<div class="section-title"><h2>⚙️ Ustawienia</h2><span class="pill">Build 0.3</span></div><div class="panel-list"><div class="panel-item"><b>💾 Zapis gry</b><div class="muted">Build 0.3 automatycznie migruje zapis 0.2 i 0.1. Zapis działa lokalnie w przeglądarce.</div><div class="tabs" style="margin-top:8px"><button class="secondary" data-export>Eksportuj</button><button class="secondary" data-import>Importuj</button><input type="file" id="saveFile" accept="application/json" hidden></div></div><div class="panel-item"><b>🌙 Symulacja nocy</b><div class="muted">Przydatna do testowania questów zależnych od pory dnia.</div><button class="secondary" data-night>${state.settings.forceNight?'Wyłącz symulację':'Włącz noc'}</button></div><div class="panel-item"><b>🧪 Tryb testowy mapy</b><div class="muted">Strzałki przesuwają bohatera o 20 m bez GPS.</div></div><div class="panel-item"><b>🆕 Co nowego w 0.3</b><div class="muted">Pełnoekranowa walka, paper-doll ekwipunku, rozgałęzione skille, filtry mapy, rozbiórka sprzętu, nowy hub miasta i bardziej czytelny loot.</div></div><div class="panel-item"><b>🔄 Reset</b><div class="muted">Usuwa zapis Build 0.3.</div><button class="danger" data-reset>Usuń zapis</button></div></div>`;
...
function exportSave(){const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='georpg-build-0.3-save.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
...
function startCombat(entity,opts={}){
 ...
 combat={entity,monster:m,level:lvl,maxHp,hp:maxHp,atk,log:[`${m.name} staje do walki.`],guard:0,debuff:0,debuffTurns:0,poison:0,poisonTurns:0,mark:0,dungeon:opts.dungeon||null};openCombat();
}
function openCombat(){const c=combat,p=state.player;if(!c)return;const cl=climate(),pet=petInstance();app.innerHTML=`<div class="battle-screen ${c.dungeon?'dungeon-battle':''}"><div class="battle-top"><div><span class="build-chip">WALKA 0.3</span><h2>${c.dungeon?'🕳️ Komnata lochu':'⚔️ Spotkanie w świecie'}</h2></div><div>${cl.icon} ${cl.weather} • ${cl.phase}</div></div><div class="battle-arena"><div class="arena-layer layer-back"></div><div class="combatant hero-side"><div class="combat-name"><b>${p.name}</b><span>${CLASSES[p.class].name} • lvl ${p.level}</span></div><div class="battle-bars"><div class="barwrap bigbar"><div class="bar hp" style="width:${100*p.hp/p.maxHp}%"></div><div class="barlabel">HP ${p.hp}/${p.maxHp}</div></div><div class="barwrap bigbar"><div class="bar mana" style="width:${100*p.mana/p.maxMana}%"></div><div class="barlabel">MANA ${p.mana}/${p.maxMana}</div></div></div><div class="battle-sprite hero-sprite"><div>${CLASSES[p.class].icon}</div><span class="shadow"></span></div>${pet?`<div class="battle-pet"><span>${petDef(pet.id).icon}</span><small>${petDef(pet.id).name} lvl ${pet.level}</small></div>`:''}</div><div class="battle-center"><div class="versus">VS</div><div class="turn-indicator">TURA GRACZA</div></div><div class="combatant enemy-side"><div class="combat-name"><b>${c.entity.elite?'⭐ ':''}${c.monster.name}</b><span>${c.monster.family} • lvl ${c.level}</span></div><div class="battle-bars"><div class="barwrap bigbar"><div class="bar hp enemyhp" style="width:${100*Math.max(0,c.hp)/c.maxHp}%"></div><div class="barlabel">HP ${Math.max(0,c.hp)}/${c.maxHp}</div></div><div class="status-row">${c.poisonTurns?`<span>☠️ Trucizna ${c.poisonTurns}</span>`:''}${c.debuffTurns?`<span>⬇️ Osłabienie ${c.debuffTurns}</span>`:''}${c.markTurns?`<span>🎯 Znak ${c.markTurns}</span>`:''}</div></div><div class="battle-sprite enemy-sprite"><div>${c.monster.icon}</div><span class="shadow"></span></div><div class="enemy-meta">ATK ${c.atk} • słabość: ${c.monster.weak||'nieznana'}</div></div></div><div class="battle-bottom"><div class="combat-log"><b>Dziennik walki</b>${c.log.slice(-6).map(x=>`<div>› ${x}</div>`).join('')}</div><div class="skill-hotbar"><button class="battle-skill basic" data-attack><span>⚔️</span><b>Atak</b><small>0 many</small></button>${p.skills.map(id=>skillDef(id)).filter(Boolean).map(s=>`<button class="battle-skill" data-skill="${s.id}" ${p.mana<s.mana?'disabled':''}><span>${s.icon}</span><b>${s.name}</b><small>${s.mana} many</small></button>`).join('')}<button class="battle-skill potion-skill" data-combat-potion ${countItem('potion')?'':'disabled'}><span>🧪</span><b>Mikstura</b><small>${countItem('potion')} szt.</small></button><button class="battle-skill flee-skill" data-flee><span>🏃</span><b>Ucieczka</b><small>${c.dungeon?'zablokowana':'70%'}</small></button></div></div></div>`;
...
function completeDungeon(){const d=DUNGEONS.find(x=>x.id===dungeonRun.id),bonus=220+state.player.level*35,gold=55+state.player.level*4;gainXp(bonus);state.player.gold+=gold;state.player.dungeonClears[d.id]=(state.player.dungeonClears[d.id]||0)+1;checkQuestProgress('dungeon',d.id);if(Math.random()<.45)addItem('crystal');if(Math.random()<.28)addItem('moonHerb');dungeonRun=null;save();refresh();toast(`${d.name} ukończona! +${bonus} XP • +${gold} 🪙`)}
