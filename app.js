import {CLASSES,MONSTERS,MONSTER_LOOT,ITEMS,QUESTS,SKILLS,PETS,RECIPES,DUNGEONS,BUILDINGS} from './data.js?v=3050';
...
const BIOMES={
 meadow:{name:'Łąki',icon:'🌾',color:'#79a94d',fill:.14,desc:'Otwarte tereny. Częste zwierzęta, gobliny i surowce.'},
 forest:{name:'Las',icon:'🌲',color:'#214d2b',fill:.20,desc:'Gęsty teren pełen wilków, pająków i ukrytych ścieżek.'},
 ruins:{name:'Ruiny',icon:'🏚️',color:'#686b70',fill:.16,desc:'Stare miejsca przyciągające nieumarłych i kultystów.'},
 marsh:{name:'Mokradła',icon:'🌫️',color:'#397169',fill:.18,desc:'Niebezpieczne bagna, trucizny i rzadkie składniki.'},
 highlands:{name:'Wzgórza',icon:'⛰️',color:'#806944',fill:.17,desc:'Trudniejszy teren z ogrami i silniejszymi bestiami.'}
};
...
function rebuildBiomeLayers(){
 ...
 const minX=cx-3,maxX=cx+3,minY=cy-3,maxY=cy+3;
 const idAt=(gx,gy)=>biomeAt(gx*size+size/2,gy*size+size/2);
 // Kolorujemy komórki bez obrysowania każdej z osobna, dzięki czemu sąsiednie pola tego samego biomu tworzą jedną wizualną plamę.
 for(let gx=minX;gx<=maxX;gx++)for(let gy=minY;gy<=maxY;gy++){
  ...
  const fill=L.rectangle([a,b],{pane:'biomePane',stroke:false,fillColor:bio.color,fillOpacity:bio.fill??.15,interactive:false}).addTo(realMap);
  leafletBiomeLayers.push(fill);
  // Granicę rysujemy tylko tam, gdzie po drugiej stronie zaczyna się inny biom.
  if(idAt(gx+1,gy)!==id){const p1=worldToLatLng(x2,y1),p2=worldToLatLng(x2,y2);if(p1&&p2)leafletBiomeLayers.push(L.polyline([p1,p2],{pane:'biomePane',color:'#ead8a6',weight:1.4,opacity:.52,dashArray:'5 7',interactive:false}).addTo(realMap))}
  if(idAt(gx,gy+1)!==id){const p1=worldToLatLng(x1,y2),p2=worldToLatLng(x2,y2);if(p1&&p2)leafletBiomeLayers.push(L.polyline([p1,p2],{pane:'biomePane',color:'#ead8a6',weight:1.4,opacity:.52,dashArray:'5 7',interactive:false}).addTo(realMap))}
 }
}
...
function renderInventory(el){
 ...
 const p=state.player,slots=[['helmet','Hełm','⛑️'],['amulet','Amulet','📿'],['weapon','Broń','⚔️'],['armor','Pancerz','🛡️'],['offhand','Druga ręka','🛡️'],['gloves','Rękawice','🧤'],['ring1','Pierścień I','💍'],['ring2','Pierścień II','💍'],['boots','Buty','🥾']],used=inventoryUsedSlots(),cap=inventoryCapacity(),bag=backpackEntries();
...
function exportSave(){const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='time4heroes-build-3.0.5-save.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
...
function defendAction(){if(!combat)return;resetCombatExchange();combat.turnDealtNote='OBRONA';combat.guard=Math.max(combat.guard,1);logCombat('🛡️ Przyjmujesz postawę obronną. Najbliższy cios zada znacznie mniej obrażeń.');haptic(12);enemyTurn()}
...
function enemyTurn(){
 ...
 if(combat.stunned>0){combat.stunned--;setCombatTaken(0,'OGŁUSZONY');logCombat('💫 Przeciwnik jest ogłuszony i traci akcję.');tickEnemyStatuses();save();openCombat();return}
 if(combat.freezeTurns>0&&Math.random()<.25){setCombatTaken(0,'ZAMROŻONY');logCombat('❄️ Zamrożenie spowalnia przeciwnika — traci akcję.');tickEnemyStatuses();save();openCombat();return}
 if(combat.intent){resolveBossIntent();tickEnemyStatuses();if(state.player.hp<=0)return loseCombat();save();openCombat();return}
 combat.enemyTurns++;
 const intentEvery=combat.bossPhase>=3?2:3;if(combat.isBoss&&combat.enemyTurns%intentEvery===0){combat.intent=chooseBossIntent();const info=bossIntentInfo(combat.intent);setCombatTaken(0,'BOSS SZYKUJE ATAK');logCombat(`⚠️ Boss przygotowuje: ${info.name}. ${info.hint}`);playSfx('boss');haptic([30,35,30]);tickEnemyStatuses();save();openCombat();return}
 const dodge=Math.random()*100<dodgeChance();if(dodge){combat.lastEnemyHit=null;setCombatTaken(0,'UNIK');logCombat('💨 Unikasz ataku przeciwnika.');tickEnemyStatuses();save();openCombat();return}
...
function combatPotion(){if(!countItem('potion'))return;resetCombatExchange();combat.turnDealtNote='MIKSTURA';playSfx('heal');removeItem('potion');state.player.hp=Math.min(state.player.maxHp,state.player.hp+55);logCombat('Wypijasz Miksturę życia (+55 HP).');enemyTurn()}
