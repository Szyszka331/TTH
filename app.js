import {CLASSES,MONSTERS,ITEMS,QUESTS,SKILLS,PETS,RECIPES,DUNGEONS,BUILDINGS} from './data.js?v=2002';
...
function buildingSceneHTML(id){
 ...
 const actions=id==='tavern'?`<button class="scene-hotspot npc-zone" data-building-action="keeper"><span>💬</span><b>Dorian</b><small>Rozmowa • anegdoty • stamina</small></button><button class="scene-hotspot fire-zone" data-building-action="fireplace"><span>🔥</span><b>Kominek</b><small>Odpocznij i zregeneruj siły</small></button><button class="scene-hotspot board-zone" data-building-action="board"><span>📜</span><b>Tablica ogłoszeń</b><small>Questy i kontrakty</small></button>`:`<button class="scene-hotspot npc-zone" data-building-action="keeper"><span>💬</span><b>${c.npc}</b><small>${c.role}</small></button><button class="scene-hotspot service-zone" data-building-action="service"><span>${id==='smith'?'⚒️':id==='alchemist'?'⚗️':id==='shop'?'🛒':id==='auction'?'📖':'🛡️'}</span><b>${id==='smith'?'Warsztat':id==='alchemist'?'Stół alchemiczny':id==='shop'?'Handel':id==='auction'?'Księga aukcji':'Stół gildii'}</b><small>Kliknij, aby wejść w interakcję</small></button>`;
