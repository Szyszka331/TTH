...
const SAVE_KEY='time4heroes_build_22';
const MIGRATION_KEYS=['time4heroes_build_21','time4heroes_build_115','time4heroes_build_114','time4heroes_build_111','time4heroes_build_110','time4heroes_build_19','time4heroes_build_18','time4heroes_build_17','time4heroes_build_16','time4heroes_build_15','time4heroes_build_14','time4heroes_build_13','time4heroes_build_12_core','time4heroes_build_11','time4heroes_build_10','time4heroes_build_09','time4heroes_build_08','georpg_build_07','georpg_build_06','georpg_build_05','georpg_build_04','georpg_build_03','georpg_build_02','georpg_build_01'];
...
function topbar(){
 ...
 return `<header class="topbar topbar-fantasy"><div class="brand-side"><div class="identity"><div class="mini-avatar">${classVisual(p.class,'sprite-mini')}</div><div><b>${p.name}</b><div class="tiny">${c.name} • poziom ${p.level} <span class="build-chip">2.2</span></div></div></div><div class="bars"><div><div class="barwrap"><div class="bar hp" style="width:${100*p.hp/p.maxHp}%"></div><div class="barlabel">HP ${p.hp}/${p.maxHp}</div></div><div class="barwrap"><div class="bar mana" style="width:${100*p.mana/p.maxMana}%"></div><div class="barlabel">MANA ${p.mana}/${p.maxMana}</div></div></div><div><div class="barwrap"><div class="bar xp" style="width:${100*p.xp/need}%"></div><div class="barlabel">XP ${p.xp}/${need}</div></div><div class="tiny">ATK ${attackPower()} • Pancerz ${armorPower()} • Kryt ${critChance().toFixed(0)}%${pet?` • 🐾 lvl ${pet.level}`:''}</div></div></div></div><div class="logo-center"><h1>Time4Heroes</h1><small>Świat czeka na odkrycie</small></div><div class="hud-right"><div class="resource resource-fantasy"><span>🪙 <strong>${p.gold}</strong></span><span>💎 <strong>${countItem('crystal')}</strong></span><span>⚡ <strong>${p.stamina??100}/${p.maxStamina??100}</strong></span></div><div class="world-meta"><span>${region}</span><span>${cl.icon} ${cl.weather}</span><span>🕒 ${now}</span></div></div></header>`;
}

function bottomNav(){const tabs=[['map','🗺️','Mapa',''],['hero','🧙','Bohater',''],['bag','🎒','Plecak','shortcut'],['town','🏰','Miasto',''],['quests','📜','Zadania','shortcut'],['menu','☰','Menu','']];return `<nav class="bottom core-nav">${tabs.map(([id,ico,name,kind])=>kind==='shortcut'?`<button class="navbtn ${currentTab==='hero'&&id==='bag'&&state.ui.heroView==='inv'?'active':''}${currentTab==='adventureHub'&&id==='quests'?' active':''}" data-shortcut="${id}"><span>${ico}</span>${name}</button>`:`<button class="navbtn ${id===currentTab?'active':''}" data-nav="${id}"><span>${ico}</span>${name}</button>`).join('')}</nav>`}
...
function shellSidebar(){
 const items=[
  {id:'map',label:'Mapa',icon:'🧭',type:'nav'},
  {id:'hero',label:'Bohater',icon:'🛡️',type:'nav'},
  {id:'bag',label:'Plecak',icon:'🎒',type:'shortcut'},
  {id:'town',label:'Miasto',icon:'🏰',type:'nav'},
  {id:'bestiary',label:'Bestiariusz',icon:'📖',type:'shortcut'},
  {id:'skills',label:'Umiejętności',icon:'🌳',type:'shortcut'},
  {id:'menu',label:'Menu',icon:'📜',type:'nav'}
 ];
 return `<aside class="left-rail fantasy-rail">${items.map(it=>{const active=(it.id==='map'&&currentTab==='map')||(it.id==='hero'&&currentTab==='hero'&&state.ui.heroView==='char')||(it.id==='bag'&&currentTab==='hero'&&state.ui.heroView==='inv')||(it.id==='town'&&currentTab==='town')||(it.id==='bestiary'&&currentTab==='adventureHub'&&state.ui.adventureView==='bestiary')||(it.id==='skills'&&currentTab==='hero'&&state.ui.heroView==='char')||(it.id==='menu'&&currentTab==='menu');return `<button class="rail-btn ${active?'active':''}" ${it.type==='nav'?`data-nav="${it.id}"`:`data-shortcut="${it.id}"`}><span>${it.icon}</span><small>${it.label}</small></button>`}).join('')}</aside>`;
}
...
function mapBackpackPreviewHTML(){
 const p=state.player;const cap=inventoryCapacity(),used=usedSlots();const items=p.inventory.slice(0,12);
...
function renderMap(el){setAmbient('forest');destroyRealMap();ensureLivingWorld();state.ui.mapPanelTab ||= 'quests';const p=state.player,hasGeo=!!(p.position.lat&&p.position.lng),virtual=!!p.position.virtualTravel;for(const e of state.world.entities)if(e.type==='monster'&&!e.alive&&e.respawn<=Date.now())e.alive=true;const tutorial=tutorialMapOverlay();const region=biomeInfoAtPlayer();el.innerHTML=`<div class="world-dashboard"><section class="dashboard-main-card fantasy-card"><div class="dashboard-map-header"><div><div class="dashboard-kicker">Mapa świata</div><h2>${region.name}</h2><p>${region.icon} Spokojny region. Gęste lasy i stare ruiny skrywają wiele tajemnic.</p></div>${mapQuickActionsHTML()}</div><div class="illustrated-map-frame"><div class="map-compass-strip">N <span>E</span> S <span>W</span></div><div class="illustrated-map-surface"><img src="assets/world-map-debowy.png" alt="Mapa Lasu Dębowego" class="illustrated-map-bg"><div class="discover-ring outer"></div><div class="discover-ring inner"></div><button class="player-token" title="${p.name}"><span>${classVisual(p.class,'sprite-mini')}</span><b>${p.name}</b></button>${mapDashboardEntities().map(dashboardMapEntityHTML).join('')}${tutorial}${!hasGeo?`<div class="gps-start-card dashboard-gps-card"><b>📍 Rozpocznij eksplorację</b><p>Włącz GPS, aby gra śledziła Twoją pozycję. Nadal możesz testować grę w trybie domowym.</p><div class="gps-card-actions"><button class="primary" data-map-gps>Włącz GPS</button><button class="secondary" data-map-demo-step="up">Tryb testowy</button></div></div>`:''}<div class="map-overlay-footer"><div class="scale-bar">100 m</div><button class="map-world-btn secondary" data-nav="town">Miasto i budynki</button></div><div class="map-ui-stack"><button class="map-ui-btn" data-map-zoom="in">＋</button><button class="map-ui-btn" data-map-zoom="out">－</button><button class="map-ui-btn" data-map-center>◎</button><button class="map-ui-btn" data-shortcut="quests">📜</button></div><div class="demo-pad"><button data-map-demo-step="up">▲</button><div><button data-map-demo-step="left">◀</button><button data-map-demo-step="right">▶</button></div><button data-map-demo-step="down">▼</button></div></div></div></section><aside class="dashboard-side-card fantasy-card">${mapRightPanelHTML()}</aside><div class="dashboard-bottom-row">${mapBackpackPreviewHTML()}${mapTownPreviewHTML()}</div></div>`;
 el.querySelectorAll('[data-dash-entity]').forEach(b=>b.addEventListener('click',()=>interactEntity(state.world.entities.find(e=>e.id===b.dataset.dashEntity))));
 ...
 el.querySelectorAll('[data-map-center]').forEach(b=>b.onclick=()=>toast(hasGeo?'Pozycja gracza jest już na środku mapy.':'Włącz GPS, aby wyśrodkować pozycję.'));
 el.querySelectorAll('[data-map-zoom]').forEach(b=>b.onclick=()=>toast('To stylizowana mapa świata — zoom zostanie rozwinięty w kolejnej paczce.'));
 ...
 bindShellControls(el);bindTutorialControls(el);
}
...
function exportSave(){const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='time4heroes-build-2.0b-save.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
