import {CLASSES,MONSTERS,MONSTER_LOOT,ITEMS,QUESTS,SKILLS,PETS,RECIPES,DUNGEONS,BUILDINGS} from './data.js?v=3010';
...
function exportSave(){const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='time4heroes-build-3.0.1-save.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
...
function battleLootRow(drop,index){const d=itemDef(drop.id);return `<label class="battle-loot-row rarity-card-${d.rarity}"><input type="checkbox" data-battle-loot="${index}"><span class="battle-loot-icon">${d.icon}</span><span class="battle-loot-copy"><b class="rarity-${d.rarity}">${d.name}</b><small>${drop.qty>1?`${drop.qty} szt. • `:''}${d.rarity} • ${d.type==='material'?'materiał':d.type==='consumable'?'zużywalny':d.slot?'ekwipunek':'przedmiot'}</small></span><span class="battle-loot-take">ZABIERZ</span></label>`}
